...
def setup_albert_models():
    """Set up locally trained ALBERT models for embedding and classification."""
    # Get the absolute path to the trained_albert folder
    model_path = os.path.join(os.path.dirname(__file__), 'trained_albert')

    # Load the trained tokenizer and ALBERT model
    tokenizer = AlbertTokenizer.from_pretrained(model_path)
    embedding_model = AlbertModel.from_pretrained(model_path)
    classification_model = AlbertForSequenceClassification.from_pretrained(model_path)
...
def process_documents(input_path):
    ...
    tokenizer, embedding_model, classification_model = setup_albert_models()  # Load trained ALBERT model
