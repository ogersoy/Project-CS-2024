...
from markdown_cleaner import process_file
...
def process_documents(input_path):
    ...
    embedding_model = setup_sentence_bert()  # Use Sentence-BERT for better embeddings
...
def answer_question(query, chunks, doc_embeddings, tokenizer, embedding_model, classification_model):
    ...
    # Step 1: Find the most relevant document chunk
    relevant_chunk = find_most_relevant_chunk(query, chunks, doc_embeddings, embedding_model)

    # Step 2: (Optional) Classify the relevance of the chunk to ensure its quality
    relevance = classify_relevance(query, relevant_chunk, tokenizer, classification_model)
    if relevance != "fully answers the question":
        print(f"Warning: The answer might not fully match the query. Relevance classified as: {relevance}")

    # Step 3: Return the most relevant text chunk for further processing by the language model
