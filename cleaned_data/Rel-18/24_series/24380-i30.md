...
d. shall include the MCPTT ID of the requesting floor participant in a User ID
field, if privacy is not requested; and
...
the ASCII name string is: MCNC (Notification control).
5\. For the MBS subchannel control protocol specified in the present document
the ASCII name string is: MCSC (Mission Critical MBS subchannel Control).
...
the total length in octets of the \ value, the
\ value, the \ value, and one or
more \ value items.
...
of the \ value item except padding.
...
The Message Type field identifies the floor control message that is
acknowledged.
...
consisting of the 5 bit message subtype as coded in table 8.2.2.1-1 preceeded
by \"000\".The first bit of the 5 bit subtype is set to zero.
The spare bits are set to zero.
...
The \ value is a binary value and is set
according to table 8.2.3.1-2.
The \ value is a binary value and has a value
indicating the total length in octets of the \ value, and
one or more pair of \ and \ value
items except padding.
The \ value is a binary value and includes the number of
Granted User ID entries in the list.
The \ value is a binary value and includes the value
...
\ values is not (1 + multiple of 4) bytes, then the sum shall
be padded to (1 + multiple of 4) bytes. The value of the padding bytes is set
to zero. The padding bytes are ignored by the receiver.
...
The \ value is a binary value and is set according to table
8.2.3.1-2.
The \ value is a binary value and has avalue indicating
the total length in octets of the the \ value, and one or more pair
of \ and \ value items except padding.
...
The \ value is a binary value and has a value
indicating the total length in octets of the \ value and
one or more \ value items (i.e. set of (\ value plus
...
The \ value is a binary value and has a value
indicating the total length in octets of the \ value, and
one or more pair of \ and \ value items
except padding.
...
formula below.
The \ value is a binary value that indicates the number of
Queued User ID entries in the list.
The \ value is a binary value that indicates the length
...
**[User ID:]{.underline}**
The User ID field is used in off-network and in messages sent by a non-
controlling function to a controlling function, and is coded as described in
clause 8.2.3.8.
...
\'10\' media bearer establishment failed and call continuesNOTE: Reason cause
\'10\' can be used when the specific participant's call leg is disconnected
due to bearer issue, however the call continues with the other participants
(e.g. Group call). The impacted participant may try to rejoin the ongoing
call.
All other values are reserved for future use.
...
## 8.6 MBS subchannel control
### 8.6.1 Introduction
The MBS subchannel control messages shall be coded as described in clause
8.6.2 where the MBS subchannel control message is part of the application-
dependent data.
...
### 8.6.2 MBS subchannel control messages
Table 8.6.2-1 provides a list of MBS subchannel control protocol messages.
Table 8.6.2-1: MBS subchannel control protocol messages
Message name Subtype Reference Direction
* * *
MapGroupToSessionStream 00000 clause 8.6.4 Server client
UnMapGroupFromSessionStream 00001 clause 8.6.5 Server client MBS Application
Paging 00010 clause 8.6.6 Server client Session Announcement 00011 clause
8.6.7 Server client NOTE: The participating MCPTT function is the server and
the MCPTT client is the client.
### 8.6.3 MBS subchannel control specific fields
...
* * *
                     Decimal    Binary
MBS Subchannel 000 00000000 Clause 8.6.3.3 MBS Session ID 001 00000001 Clause
8.6.3.4. MCPTT Group ID 002 00000010 Clause 8.6.3.2 Monitoring State 003
00000011 Subcaluse 8.6.3.5
#### 8.6.3.2 MCPTT Group ID field
The MCPTT Group ID field contains a SIP URI identifying the MCPTT group for
which media and floor control messages are going to be broadcasted over a MBS
subchannel.
The MCPTT Group ID field is coded as the MCPTT Group Identity field specified
in clause 8.3.3.5.
#### 8.6.3.3 MBS Subchannel field
The MBS Subchannel field describes which MBS subchannel to use for media and
for floor control.
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
The \ value is a binary value and shall be set
according to table 8.6.3.1-1.
The \ value is a binary value indicating the total
length in octets of the \ value, \ value,
spare, \ value and \ items.
The \ value shall consist of 4 bit parameter giving the
number of the\" m=audio\" m-line in the SIP MESSAGE request announcing the MBS
session described in 3GPP TS 24.379 [2].
The \ value shall consist of 4 bit parameter giving the
number of the \"m=application\" m-line in the SIP MESSAGE request announcing
the MBS session described in 3GPP TS 24.379 [2]. The \
value is set to \"0\" when the same subchannel is used for media and for floor
control.
The \ value indicates the IP version:
\'0\' IP version 4
...
The \ value is a 32-bit binary value giving the
port to be used if the\ value is greater than \'0\'. If
the \ value is equal to \'0\', the \ value is not included in the MBS Subchannel field.
The \ value is a 32-bit binary value giving the port to be
used. The \ value is always present in the MBS Subchannel
field.
The \ value is:
1\. a 32 bit binary value containing the IP v4 address if the \
indicates that the \ value is a IP v4 Address; or
...
value of the padding bytes should be set to zero. The padding bytes shall be
ignored.
#### 8.6.3.5 Monitoring state
Clause 8.4.3.6 apply also for MBS.
### 8.6.4 MapGroupToSessionStream message
...
### 8.6.5 UnMapGroupFromSessionStream message
...
**[MBS Subchannel:]{.underline}**
The MBS Subchannel field is coded as described in clause 8.6.3.3.
### 8.6.6 MBS Application Paging message
The MBS Application Paging message is sent by the participating function when
an existing conversation is to be moved to unicast sessions or a new
conversation is to be started on unicast sessions.
...
**[MCPTT Group ID:]{.underline}**
The MCPTT Group ID field is coded as described in clause 8.6.3.2.
### 8.6.7 Session Announcement message
...
The length shall be coded as specified in clause 8.1.2.
**[MBS Session ID:]{.underline}**
The MBS Session ID field is coded as described in clause 8.6.3.4. This field
is mandatory.
**[TMGI:]{.underline}**
The TMGI field is coded as described in clause 8.4.3.4. This field is
mandatory.
...
sending uncompressed packets, specified in RFC 3095 [21]. MBMS subchannels
...
# 10B MBS subchannel control procedure
All steps of clause 10 apply also for MBS, with the clarification that
terminology mapping specified in Annex X applies.
...
Table 11.1.4-3 shows the timers used in the participating MCPTT function for
MBS channel control.
Table 11.1.4-3: Timers in the participating MCPTT function for MBS channel
control.
+-------------+-------------+-------------+-------------+-------------+ | TIMER | TIMER VALUE | CAUSE OF | NORMAL STOP | ON EXPIRY | | | | START | | | +=============+=============+=============+=============+=============+ | T25 (MBS | Default | T | Release of | Send Unmap | | Co | value: | ransmission | the call. | Group From | | nversation) | | of Map | | Session | | | 30 seconds. | Group To | | Stream | | | | Session | | message. | | | Co | Stream | | | | | nfigurable. | message. | | | | | | | | | | | Obtained | Restarted | | | | | from the | when an RTP | | | | | \ | control | | | | | element of | message is | | | | | the | sent. | | | | | \ | | | | | | element of | | | | | | the | | | | | | \ | | | | | | element in | | | | | | 3GPP TS 24. | | | | | | 484 [13]. | | | | +-------------+-------------+-------------+-------------+-------------+ | T26 (Map | Default | T | Release of | Send Map | | Group To | value: | ransmission | the call | Group To | | Session | | of Map | (or MBS | Session | | Stream) | 500 | Group To | S | Stream | | | mi | Session | ubchannel). | message. | | | lliseconds. | Stream | | | | | | message. | | | | | Co | | | | | | nfigurable. | | | | | | | | | | | | Obtained | | | | | | from the | | | | | | \ | | | | | | element of | | | | | | the | | | | | | \ | | | | | | element of | | | | | | the | | | | | | \ | | | | | | element in | | | | | | 3GPP TS 24. | | | | | | 484 [13]. | | | | +-------------+-------------+-------------+-------------+-------------+ | T27 (Unmap | Default | T | Release of | Send Unmap | | Group From | value: | ransmission | the call. | Group To | | Session | | of Unmap | | Session | | Stream) | 200 | Group From | | stream | | | mi | Session | | message. | | | lliseconds. | stream | | | | | | message. | | | | | Co | | | | | | nfigurable. | | | | | | | | | | | | Obtained | | | | | | from the | | | | | | \ | | | | | | element of | | | | | | the | | | | | | \ | | | | | | element of | | | | | | the | | | | | | \ | | | | | | element in | | | | | | 3GPP TS 24. | | | | | | 484 [13]. | | | | +-------------+-------------+-------------+-------------+-------------+
## 11.2 Counters
...
The table 11.2.4-3 shows counters, their limits and the action on expiry for
the \'Participating MCPTT function MBS subchannel control state machine\'.
Table 11.2.4-3: Counters used in the \'Participating MCPTT function MBS
subchannel control state machine\'
+----------------+----------------+----------------+----------------+ | Counter | Limit | Associated | On reaching | | | | timer | the limit | +================+================+================+================+ | C27 | Default value: | T27 (Unmap | The Unmap | | | 3. | Group From | Group From | | (Unmap Group | | Session | Session Stream | | From Session | Configurable. | Stream) | message is no | | Stream) | | | more re-sent. | | | Obtained from | | | | | the | | | | | \ | | | | | element of the | | | | | \ | | | | | element in | | | | | 3GPP TS | | | | | 24.484 [13]. | | | +----------------+----------------+----------------+----------------+
...
floor control messages according to IETF RFC 3711 [16] and 3GPP TS 33.180 [18]
...
i. one \"m=audio ...\" media line containing the relevant media-line
attributes. Where:
...
i. one \"m=application ...\" media line to be used as the general purpose MBMS
subchannel; and
...
* * *
###### ## Annex D (Informative): Mapping of MBMS terms to MBS
