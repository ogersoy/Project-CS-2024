...
[28] 3GPP TS 29.212: \"Policy and Charging Control over Gx reference point\".
...
information possibly received from MME and/or SGSN during location update in
order to decide whether MME and/or SGSN need to be contacted before a response
is sent to the AS. The HSS shall provide the most recent IMS over PS support
indication and RAT type as indicated by the access nodes. If no information
could be retrieved from at least one access node, the HSS shall indicate that
IMS voice over PS support is unknown and no RAT type shall be indicated. See
Annex E.
...
2\. The HSS shall check that the IMS Public User Identity or Public Service
Identity in the request exists in HSS. If not, Experimental-Result shall be
set to DIAMETER_ERROR_USER_UNKNOWN in the Sh-Subs-Notif Response.
3\. If the User Identity does not apply to the Data-Reference indicated in the
request according to Table 7.6.1, Experimental-Result shall be set to
...
This information element contains:
  ...
  * the locations of the served subscriber in the MME and the SGSN if the requested domain is PS and the requested nodes is MME and SGSN.
...
specified in 3GPP TS 29.118 [30], the ECGI and TAI are included, rather than
Location number, Service area ID, Global Cell ID and Location area ID.
...
domain/node indicated by the Requested-Domain/Requested-Nodes (see 7.2), with
...
S6d/MAP operation towards the SGSN to retrieve this information.
...
GeodeticInformation, VLR Number, MSC Number, AgeOfLocationInformation,
...
SGSN Number, AgeOfLocationInformation, CurrentLocationRetrieved and
UserCSGInformation. They are defined in 7.6.
Class EPSLocationInformation has the attributes E-UTRANCellGlobalId,
TrackingAreaID, GeographicalInformation, GeodeticInformation, MME Name,
AgeOfLocationInformation, CurrentLocationRetrieved and UserCSGInformation.
...
enumeration whose possible values are defined in section 7.6.18.
...
instance of the class PSIActivation, 0 or 1 instance of the class DSAI and 0
or 1 instance of the class IPAddressSecureBindingInformation.
...
contents and usage.
Class SMSRegistrationInfo contains the IP-SM-GW-Number. See section 7.6.17 for
further details.
