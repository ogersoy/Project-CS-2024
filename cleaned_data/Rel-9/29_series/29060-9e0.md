...
PDP contexts for this PDP address or two IP addresses (one IPv4 and one IPv6
if PDP Type IPv4v6 is supported and used) and APN.
...
When receiving a T-PDU the GGSN checks if a PDP Context is established for
that PDP address or two IP addresses (one IPv4 and one IPv6 if PDP Type IPv4v6
is supported and used). If no PDP context has been previously established the
GGSN may store the T-PDU, try to initiate the Network-Requested PDP Context
...
static PDP information about the PDP address or two IP addresses (one IPv4 and
one IPv6 if PDP Type IPv4v6 is supported and used).
...
IE(s) for all non-GBR PDP Contexts of the APN.
...
APN-AMBR with NSAPI Optional 7.7.101 Higher bitrates than 16 Mbps flag
Optional 7.7.102 Private Extension Optional 7.7.46
* * *
...
Higher bitrates than 16 Mbps flag Optional 7.7.102 Additional MM context for
SRVCC Optional 7.7.107 Additional flags for SRVCC Optional 7.7.108 STN-SR
Optional 7.7.109 C-MSISDN Optional 7.7.110 Extended RANAP Cause Optional
...
IMSI, then the message shall be silently discarded and no further processing
of the IEs shall continue i.e. the Cause value is not processed.
...
\"New PDP type due to network preference\" indicates that the GGSN has
selected a PDP type different from the one sent by the MS.\"
...
response rej IMSI not known 194 MS is GPRS Detached 195 MS is not GPRS
...
228 For future use 229-240 Cause values reserved for GPRS charging protocol
use (see GTP\' in 3GPP TS 32.295 [33]) 241-255 NOTE: With this coding, bits 8
and 7 of the Cause Value respectively indicate whether the message was a
request or a response, and whether the request was accepted or rejected.
* * *
...
contexts that share the same PDP address or two IP addresses (one IPv4 and one
IPv6 if PDP Type IPv4v6 is supported and used) with the PDP context identified
by the NSAPI included in the Delete PDP Context Request Message shall be torn
...
Forward Relocation Request and SGSN Context Response and shall be set to 1 if
the IMSI present in the message is not authenticated and is for an emergency
attached MS.
...
| 1 | Type=193 (Decimal) |  |  |  |  |  |  |  |   
| 2-3 | Length |  |  |  |  |  |  |  |   
| 4-n | Spare | Spare | Spare | VB | Spare (NOTE 2) | Spare (NOTE 2) | CCRSI | Unauthenticated IMSI |   
Figure 7.7.93.1: Extended Common Flags Information Element
NOTE 2: This bit is used in newer versions of the specification.
...
For two digits MNCs, bits 5 to 8 of octet 6 are coded as \"1111\".
...
## 7.7.102 Higher bitrates than 16 Mbps flag
...
It is formatted as shown in Figure 7.7.102-1.
* * *
              Bits                                                            
     Octets   8                                   7   6   5   4   3   2   1   
     1        Type = 205 (decimal)                                            
...
* * *
Figure 7.7.102-1: Higher bitrates than 16 Mbps flag
### 7.7.103 (void)
### 7.7.104 (void)
### 7.7.105 (void)
### 7.7.106 (void)
...
Numbers, Evolved Allocation/Retention Priority II, APN-AMBR with NSAPI and the
...
may have a Length field value, which is different from the expected Length. In
this case,
...
PDP context Jan 2011 Rel-9 9.5.1 Missing length types in Table 37 were added
Feb 2011 Rel-9 9.5.2 History table updated CT#51 29.060 9.5.2 0789r1 Rel-9
9.6.0 APN-AMBR IE 0791r4 Subscribed UE-AMBR in mobility procedure 0794r1
Passing Charging Characteristics for IRAT 0799r1 Essential correction to the
encoding of Target RNC-ID CT#52 29.060 9.6.0 0822r1 Rel-9 9.7.0 Incorrect PCO
handling at the GGSN 0811r1 Higher bitrates than 16 Mbps flag 0807r2 Mapping
of ASN.1/PER parameters to GTPv1 IEs 0818 Correction on PDP Type Number CT#53
29.060 9.7.0 0828r1 Rel-9 9.8.0 PCO handling at the GGSN for pre-Rel 7 SGSNs
0824r1 PS handover procedure for SRVCC 0830r1 Essential correction to handling
of EPDN session for UICCless UE 0835r1 RANAP Cause code range alignment with
RANAP specification 0837 PDU Notification Request correction for dual address
PDP CT#54 29.060 9.8.0 0847 Rel-9 9.9.0 Defining the fixed number of octets
for extendable IEs CT#55 29.060 9.9.0 0859 Rel-9 9.10.0 Handover to CSG cell
with emergency bearer 0862 Pre-Rel-7 QoS description correction 0869r1 Missing
CSG Information Reporting Action IE in CPC and UPC response messages 0879r2
Higher bitrates than 16 Mbps flag CT#56 29.060 9.10.0 0904r2 Rel-9 9.11.0
Providing Serving Network Information to the GGSN 0887r1 GGSN TEID and IP
address in the Update PDP Context Response message