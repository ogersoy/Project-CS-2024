...
section 4.1.2.2.
d) The signature algorithm shall be \"sha1WithRSAEncryption\" [38] or
\"sha256WithRSAEncryption\" [48], and the RSA public key used for signing
shall not be longer than 2048 bits. For security reasons, the use of
\"sha1WithRSAEncryption\" is not recommended for newly created certificates.
...
shall be \"sha1WithRSAEncryption\" [38] or \"sha256WithRSAEncryption\" [48].
For security reasons, the use of \"sha1WithRSAEncryption\" is not recommended
for newly created CRLs and OCSP responses.
...
established.
NOTE 3: For interworking with pre-Release 9 elements, usage of SHA-1 in CRLs
and OCSP responses may be required for some time. However, it is likely that
in a future 3GPP release, CRLs and OCSP responses which use SHA-1 as the hash
