...
received from the CN the SRNC checks the state of the target UE. If the UE is
...
IODE IE. The RRC definition here is different from the one given in [24].
...
In this concept, different GNSS (e.g. GPS, Galileo, GLONASS) can be used
separately or in combination to perform the location of a UE. The aim of such
combination is to improve:
...
a 3rd party service provider that is available and therefore can be activated
