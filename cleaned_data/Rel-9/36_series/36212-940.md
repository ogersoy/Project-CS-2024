...
* * *
For the case that HARQ-ACK consists of more than two bits information, i.e.
...
DCI format 1D is used for the compact scheduling of one PDSCH codeword with
...
Table 5.3.3.1.2-1, one zero bit shall be appended to format 1D.
##### 5.3.3.1.5 Format 2
...
subframe _n_ is according to the latest PMI(s) reported by the UE on PUSCH,
not coming from PUCCH, on or before subframe _n_ -4.
...
CRC attachment, including any padding bits appended to format 0\. The
parameter _tpc-Index_ provided by higher layers determines the index to the
...
the time span, in units of OFDM symbols, of the DCI in that subframe. The CFI
takes values CFI = 1, 2 or 3. For system bandwidths $N_{\text{RB}}^{\text{DL}}
> \text{10}$, the span of the DCI in units of OFDM symbols, 1, 2 or 3, is
given by the CFI. For system bandwidths $N_{\text{RB}}^{\text{DL}} \leq
\text{10}$, the span of the DCI in units of OFDM symbols, 2, 3 or 4, is given
by CFI+1.
