...
Identity shall be included in the same implicit registration set as the
wildcarded Public User Identity it matches, but it may have a different
service profile than the wildcarded Public User Identity.
...
reference points is specified in TS 24.229 [10a].
...
NOTE: The example flows illustrate only the allocation of the TrGW before
sending the SDP offer.
If the IBCF is configured per local policy to use proactive transcoding, the
IBCF shall add codecs to the offer such that in the terminating UE codec
selection process, priority shall be given to the codecs inserted by the
originating UE over the codecs inserted by the IBCF.
...
from the originating UE prior to selecting a codec inserted by the network. A
TrGW is allocated.
4\. The IBCF generates a new SIP message towards User B network based on the
received SIP message where additional codec(s) have been added indicating low
preference, and where the transport address and port information has been
altered to indicate the addresses associated by the TrGW.
