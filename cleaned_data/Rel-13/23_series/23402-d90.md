...
\- The UE shall be able to discovery WLANs that support S2a connectivity. This
...
against the groups of selection criteria in the active WLANSP rule. When a
group of selection criteria includes the HomeNetwork attribute and is set,
then the UE (a) shall create a list of available WLANs that directly interwork
with the home operator (as specified in clause 4.8.2.1.6) and (b) shall apply
the group of selection criteria to all the WLANs in this list. Otherwise, when
the HomeNetwork attribute is not set or is not present, the UE shall apply the
group of selection criteria to all available WLANs. The UE may need to perform
ANQP procedures (as specified in the HS2.0 Rel‑2 specification [75]) or other
...
##### **6.9.2.2.1a: Serving RNS Relocation Procedure, Combined Hard Handover
and SRNS Relocation Procedure, and Combined Cell/URA Update and SRNS
Relocation Procedure using S4** {#a-serving-rns-relocation-procedure-combined-
hard-handover-and-srns-relocation-procedure-and-combined-cellura-update-and-
srns-relocation-procedure-using-s4 .H6}
...
{#a-enhanced-serving-rns-relocation-procedure-using-s4 .H6}
Steps A.1 and A.2 of the procedure in clause 5.7.2 occur instead of the steps
shown in the box (step B and C) in figure 44b1 and steps B and C in figure
44b2 in TS 23.060 [21], clause 6.9.2.2.5A.
##### **\- 6.12.1A: UE Initiated Service Request Procedure Using S4** {#a-ue-
initiated-service-request-procedure-using-s4 .H6}
Steps A.1 and A.2 of the procedure in clause 5.7.2 occur instead of the steps
shown in the box (B1) in TS 23.060 [21], clause 6.12.1A, figure 50A.
##### **\- 6.13.1.1.2: Iu mode to A/Gb mode Intra-SGSN Change using S4** {#iu-
mode-to-agb-mode-intra-sgsn-change-using-s4 .H6}
Steps A.1 and A.2 of the procedure in clause 5.7.2 occur instead of the steps
...
{#agb-mode-to-iu-mode-intra-sgsn-change-using-s4 .H6}
Steps A.1 and A.2 of the procedure in clause 5.7.2 occur instead of the steps
shown in the box (A1) in TS 23.060 [21], clause 6.13.1.2.2, figure 53-2.
##### **\- 6.13.2.1.2: Iu mode to A/Gb mode Inter-SGSN Change using S4** {#iu-
mode-to-agb-mode-inter-sgsn-change-using-s4 .H6}
Steps A.1 and A.2 of the procedure in clause 5.7.2 occur instead of the steps
...
{#agb-mode-to-iu-mode-inter-sgsn-change-using-s4 .H6}
Steps A.1 and A.2 of the procedure in clause 5.7.2 occur instead of the steps
shown in the box (B1) in TS 23.060 [21], clause 6.13.2.2.2, figure 55-3.
##### **\- 8.1.4A Paging response for GPRS Downlink Transfer with no
established User plane on S4** {#a-paging-response-for-gprs-downlink-transfer-
with-no-established-user-plane-on-s4 .H6}
Steps A.1 and A.2 of the procedure in clause 5.7.2 occur instead of the steps
...
##### **\- 9.2.2.1A: A PDP Context Activation using S4** {#a-a-pdp-context-
activation-using-s4 .H6}
...
##### **\- 9.2.2.1.1A, Figure 66a: Secondary PDP Context Activation Procedure,
PDP Creation Part using S4** {#a-figure-66a-secondary-pdp-context-activation-
procedure-pdp-creation-part-using-s4 .H6}
...
Request Part** {#a-ms-initiated-eps-bearer-modification-procedure-
using-s4-request-part .H6}
...
Execution Part** {#b-ms-initiated-eps-bearer-modification-procedure-
using-s4-execution-part .H6}
...
Response Part** {#c-ms-initiated-eps-bearer-modification-procedure-
using-s4-response-part .H6}
...
##### **\- 9.2.2.3A: Network Requested PDP Context Activation Procedure using
S4** {#a-network-requested-pdp-context-activation-procedure-using-s4 .H6}
...
Part** {#a-sgsn-initiated-eps-bearer-modification-procedure-request-part .H6}
...
##### **\- 9.2.3.1B: SGSN-Initiated EPS Bearer Modification Procedure,
Response Part** {#b-sgsn-initiated-eps-bearer-modification-procedure-response-
part .H6}
...
{#a-pdn-gw-initiated-eps-bearer-modification-procedure .H6}
Step A.1 of the procedure in clause 5.4.1 occurs instead of the steps shown in
...
##### **\- 9.2.4.1A.1: MS-and SGSN Initiated PDN connection Deactivation
Procedure using S4** {#a.1-ms-and-sgsn-initiated-pdn-connection-deactivation-
procedure-using-s4 .H6}
...
{#a.2-ms--and-sgsn-initiated-bearer-deactivation-using-s4 .H6}
...
procedures in TS 23.060 [21] employ this procedure:
##### **\- 9.2.4.3A: PDN GW-Initiated PDP Context Deactivation Procedure using
S4** {#a-pdn-gw-initiated-pdp-context-deactivation-procedure-using-s4 .H6}
Step A.1 in clause 5.4.1 corresponds to the steps shown in box (A1) of TS
23.060 [21], clause 9.2.4.3A, figure 77a.
##### **\- 9.2.4.3B: PDN GW-Initiated PDP Context Deactivation Procedure using
S4** {#b-pdn-gw-initiated-pdp-context-deactivation-procedure-using-s4 .H6}
...
##### **\- 6.9.1.2.2a: Inter SGSN Routeing Area Update and Combined Inter SGSN
RA / LA Update using S4** {#a-inter-sgsn-routeing-area-update-and-combined-
inter-sgsn-ra-la-update-using-s4 .H6}
...
##### **\- 6.9.2.1a: Routeing Area Update Procedure using S4** {#a-routeing-
area-update-procedure-using-s4 .H6}
...
based on USIM credentials, the 3GPP AAA server may store WLAN Location
...
this session in case of signalling congestion (over SWx). The indication is
used by the ePDG to apply specific policies related with emergency PDN
connection (e.g. stored in Emergency Configuration Data).
...
NOTE: No procedure for additional authentication and authorisation with an
...
information for the emergency PDN connection. It instead uses Emergency
Configuration Data to get information on the APN and possibly PDN GW and / or
QoS (APN-AMBR, default QoS) to use for the emergency PDN connection.
...
instance, the 3GPP AAA server does not update the HSS with the identity of the
PDN GW in that case.
...
EAP-AKA \' authentication signaling [72] is extended in order to negotiate the
connection mode: Single-Connection mode, Multi-Connection mode or Transparent
Single-Connection mode, and to carry additional information needed for single
connection mode.
...
\- Step 2. This step is the same as step 2 in 16.2.1 with the following
addition: If the UE indicates single-connection, then the UE indicates also
handover via EAP-AKA\' to 3GPP AAA.
\- Step 3. This step is the same Aas step 3 in 16.2.1 with the following
...
clause 16.2.1, where in this case Multi-Connection Mode is requested by the
UE.
...
changes apply:
On step 9 of clause 8.2.1.1 and step 11 of 8.2.1.3: the Charging Id provided
by the PGW to the default and dedicated bearers in 3GPP access is the Charging
Id previously assigned to the corresponding default and dedicated bearers
(i.e. bearer with the same QCI and ARP) of the PDN connection in the non-3GPP
access on the S2a interface, although the Charging Id still applies to the
non-3GPP access.
