...
NOTE 5: In case of E-UTRAN a QCI\'s Priority level may be used as the basis
...
traffic steering. Alternately, the Application Identifier may be used for
...
determines and updates the impacted PCC rule (as described in 6.12.2) and
provides the updated PCC rule to the PCEF. The updated PCC rule triggers the
sending of a Routing Rules creation / modification / deletion to the UE (as
described above for Network-initiated IP flow mobility).
...
to calculate the ARP priority. The ARP pre-emption capability and the ARP pre-
emption vulnerability are set according to operator policies and regulatory
requirements. The priority sharing indicator is stored for later use.
...
NOTE 5: For IP flow mobility feature enabled, the TDF doesn\'t have accurate
...
node (PCEF or TDF).
NOTE 6: The PCRF ensures that the number of requests/following policy
...
keys is reported).
NOTE 7: The PCRF maintains usage thresholds for each Monitoring key and IP‑CAN
...
NOTE 8: If the PCRF decides to deactivate all PCC rules or ADC rules
...
operator policy.
NOTE 9: Sponsored data connectivity is not supported in the roaming with
...
functions.
NOTE 10: In order to allow the PCRF to select and provision an application
...
PCEF shall discard any previously received Subsequent APN-AMBR instances
whenever the PCRF provides a new Authorized APN-AMBR or Subsequent APN-AMBR.
...
NOTE 5: The maximum bit rate is an average value, which is measured over some
...
NOTE 6: Assigning the same Charging key for several applications implies that
...
based charging.
NOTE 7: The Service Identifier need not have any relationship to service
...
NOTE: In this release of the specification, traffic steering control
...
IMSI and APN.
The RUCI report message includes the RUCI which is defined in clause 6.1.X.1.
...
NOTE 5: If the SCEF recieves only one transfer policy, the AF is not required
to confirm.
...
reporting RAT type change. The characteristics of the air interface,
communicated as the radio access type, have changed. PCEF PCRF/OCS
* * *
...
CAN type , RAT Type and an indication that access is trusted to the PCRF (over
Gx). The TWAN ID may include the identifier of the operator of the TWAN as
defined in clause 16 of TS 23.402 [18]. This occurs at step 3 of Figure 7.2-1
(IP‑CAN Session Establishment). This information is also provided by the PCEF
to the charging entities as described in TS 32.251 [9].
...
The IP-CAN type change event trigger defined in clause 6.1.4 and RAT type
event trigger applies. The PCEF reports to the PCRF when a Create Session
Request is received including information that the UE moved to a trusted WLAN
access. This information is also provided by the PCEF to the charging entities
as described in TS 32.251 [9].
...
\- The IP-CAN type changes defined in clause 6.1.4 and RAT type changes event
trigger applies. The PCEF reports to PCRF when a Create Session Request is
received including information that the UE moved to an untrusted WLAN access.
This information is also provided by the PCEF to the charging entities as
described in TS 32.251 [9]. As described for the case of an IP‑CAN Session
Establishment, the PCEF provides also the PCRF with location information it
may have received from the ePDG and the ePDG IP address used in IKEv2 tunnel
procedures.
\- The Access Network Information Report event trigger defined in clause 6.1.4
