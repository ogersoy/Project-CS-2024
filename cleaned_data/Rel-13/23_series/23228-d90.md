...
P-CSCF can therefore be locally configured to allow resource sharing in both
directions if the P-CSCF knows that the UE will not send media to a remote UE
on hold.
...
of a new Rx session with the PCRF, indicate that resources may be shared in
uplink and/or downlink direction by assigning an uplink and/or downlink tag to
each media component of an IMS session unless it is an IMS emergency session.
...
previously assigned for this UE. The PCRF may then authorize resource sharing
based on these tags. For further information, see TS 23.203 [54].
Whenever resource sharing is activate, the P-CSCF shall ensure that the UE can
...
The P-CSCF may indicate to the PCRF that the resource allocation for a media
flow is allowed to use the same priority as other media flows of the same
media type for the UE engaged in multiple sessions by providing a priority
sharing indicator in addition to the application identifier and the service
priority. For MCPTT, the service priority and the priority sharing indicator
are defined in TS 23.179 [92].
...
indicator for a media flow, the P-CSCF shall convey to the PCRF the priority
sharing indicator as described in TS 23.203 [54].
NOTE 1: The Application Server is not supposed to include the priority sharing
indicator for an emergency session.
NOTE 2: The priority sharing enables the usage of one bearer per QCI for a UE
having multiple sessions; otherwise sessions with different service priorities
would end up in different bearers.
...
PCRF providing the session information. The derived session information shall
indicate the priority of the MPS session which depends if the Service User\'s
priority level is known at this stage. The PCC interaction between the P-CSCF
and the PCRF is described in TS 23.203 [54].
...
priority of the MPS session and interact with the PCRF providing the session
information,
...
## M.3.1 Description
### M.3.1.1 General
The architecture and flows in this clause are only showing EPS. The principles
shown are also applicable for other access networks.
...
The HSS may not be aware whether the UE is currently camping on 3GPP or Non
3GPP access and may need to request Location Information from the MME, the
SGSN and from the TWAN (via the AAA server in this last case).
...
procedures according to TS 33.310 [4], using certificates. The certificates
...
# U.1 Overview
## U.1.0 General
Web Real-Time Communication (WebRTC) is specified in IETF Draft, IETF RFC 8825
[84] and WebRTC 1.0 [85]. This Annex specifies a network-based architecture
for the support of WebRTC client\'s access to IMS. Any requirements for
specific audio and video codecs from IETF RFC 8825 [84] (directly and
indirectly referenced) do not apply for WebRTC access to IMS; the codecs that
shall be supported for WebRTC access to IMS are described in TS 26.114 [76].
...
the SDP offer associated with media multiplexing shall be be removed at the
entry of the IMS network.
...
\- The eP-CSCF shall ensure via signalling that RTP and RTCP flows of an RTP
stream are not multiplexed onto the same port if entities anchoring the
session media path in the IMS domain do not support that capability.
...
TS 33.328) for media protocols specific to WebRTC, including media consent,
and DTLS-SRTP as key exchange mechanism for media components using SRTP.
...
When MSRP is transported over the data channel, the eIMS-AGW shall provide an
MSRP B2BUA to allow interoperation with existing MSRP peer endpoints.
...
to initiate a HTTPS connection to the WWSF. The TLS connection provides one-
...
way authentication of the server based on the server certificate. The browser
downloads and initializes the WIC from the WWSF. The WWSF or WAF authenticates
the user using a common web authentication procedure. The WWSF determines the
Private User Identity and Public User Identity for the WIC and returns the
security token which is issued by the WAF to the WIC. The IMS identities may
be provided to the WIC in addition to the security token.
2\. The WIC opens a WSS (secure WebSocket) connection using cross-origin
mechanism to the eP-CSCF. Standard cross-origin resource sharing procedures
are used to ensure that the WIC originated from a WWSF authorized to access
this eP-CSCF.
...
plane interworking is disabled, except when LI is needed. When LI is
performed, media plane interworking is performed according to TS 33.328 [83].
...
that purpose. The eP-CSCF-1 shall not apply OMR procedures according to Annex
Q.
...
information, but no ICE related information ). To enable the check in step 6
wether intermediates which do not support switching to the encapsulated media
are inserted into the media path, the eP-CSCF-1 also encapsulates the address
IP2.
...
configures the eIMS-AGW2 to transparently forward those media.
...
transparent media have been selected.
...
policies.
###### ## Annex W (Informative): Change history