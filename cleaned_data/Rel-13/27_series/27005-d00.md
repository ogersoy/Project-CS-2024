...
values of \.
\: integer type, indicating multiple message deletion request as
follows:
[0]{.underline} Delete the message specified in \
...
Figure 3.5.7-1 is an example of a TE-TA-ME setup when messages are sent to
network or stored to ME. The volatile memory may as well be in the ME, or a
non-volatile memory may be used instead when constructing messages.
...
Table 4.6-1: +CNMA Action Command Syntax
...
STATUS-REPORT) which is routed directly to the TE (refer command +CNMI table
3.4.1-3 and table 3.4.1-5). This acknowledgement command shall be used when
+CSMS parameter \ equals 1. In PDU mode, it is possible to send
either positive (RP-ACK) or negative (RP-ERROR) acknowledgement to the
network. Parameter \ defines which one will be sent. Optionally (when
\ is greater than zero) an acknowledgement TPDU (SMS-DELIVER-REPORT
for RP-ACK or RP-ERROR) may be sent to the network. The entering of PDU is
done similarly as specified in command Send Message +CMGS, except that the
format of \ is used instead of \ (i.e. SMSC address field is not
present). PDU shall not be bounded by double quotes. TA shall not send another
