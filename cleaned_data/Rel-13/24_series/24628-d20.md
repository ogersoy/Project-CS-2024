...
NOTE 1: When multiple early dialogs exist with authorization as \"sendrecv\"
...
\"sendonly\".
NOTE 2: if an early dialog exists where a SIP 18x response to the SIP INVITE
...
# G.1 General
An AS using gateway model specified in RFC 3960 [6] sends reliable provisional
response including a P-Early-Media header field containing either the
\"sendrecv\" or the \"sendonly\" and an SDP answer based on the information
received from MRF in order to send an announcement from the AS to the
originating UE.
