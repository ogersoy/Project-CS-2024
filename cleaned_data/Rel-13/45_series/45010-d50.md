...
synchronized together such that the timing difference between different BTSs
...
timeslots. This implementation option is illustrated in figure 5.7.4. When
...
\- the MS shall use a TA value of 0 for the HANDOVER in a PHYSICAL INFORMATION
message that TA shall be used. Before a TA is received from the new BTS no
valid \"used TA\" shall be signalled to the new BTS.ACCESS bursts sent. When a
TA is received
...
1) 1) the CTS-MS detects 3 successive CTSBCH-FB, decodes the three associated
CTSBCH-SB and stores the two timeslot shifts values between the three
successive CTSBCH-FB;
2) 2) the CTS-MS checks that the three FPBI (see 3GPP TS 45.002) extracted
from the three CTSBCH-SB are identical and that the three CTSBCH shifting
flags all indicate CTSBCH shifting active;
3) 3) the CTS-MS extracts the TNSCN from the FPBI according to the rule
