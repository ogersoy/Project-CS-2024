...
1 PCRF initiates a session towards the TDF. The PCRF provisions the applicable
ADC Rules for the corresponding TDF session by sending a Diameter TS-Request
to the TDF, including user identity information, the UE Ipv4 address and/or UE
Ipv6 prefix and, if available, PDN identifier, IP-CAN type, RAT type and
additional parameters as defined in clause 4b.5.1.1 of 3GPP TS 29.212 [9].
PCRF may also subscribe to the Event Triggers (e.g. APPLICATION_START and
APPLICATION_STOP).
...
to the UM3GPP TS QoS parameters (see clause 6.4.1.1).
...
The Translation/Mapping function in the GGSN shall derive the Authorized
UM3GPP TS QoS parameters from the Authorized IP QoS parameters received from
the PCRF according to the rules in table 6.4.1.
...
Authorized IP QoS parameters per PDP Context to Authorized UM3GPP TS QoS
parameters according to clause 6.4.1.1 and then compares the requested UM3GPP
TS QoS parameters against the corresponding Authorized UM3GPP TS QoS
parameters. The following criteria shall be fulfilled:
...
upgrade) the requested UM3GPP TS QoS parameters to the values that were
authorized.
### 6.4.2 3GPP- EPS
...
with a Gn/Gp SGSN.
The Translation/Mapping function in the P-GW shall derive the Authorized
UM3GPP TS QoS parameters from the Authorized IP QoS parameters derived for the
bearer applying the rules in table 6.4.2.
...
parameters per bearer to Authorized UM3GPP TS QoS parameters according to
clause 6.4.2.1 and then compares the requested UM3GPP TS QoS parameters
against the corresponding Authorized UM3GPP TS QoS parameters. The following
...
or upgrade) the requested UM3GPP TS QoS parameters to the values that were
authorized.
...
described in clause 6.14 of IETF RFC 3588 [14].
...
3GPP TS 23.179 [57] can be independent from a PLMN and is not guaranteed to
...
procedure as specified in IETF RFC 3588 [14], and include the PCRF identity in
...
Establishment of Diameter sessions may occur at the following cases:
\- S9 session 141vailability141
\- AF session establishment
The DRA client (AF/BBERF/PCEF) shall follow the procedure below if an
appropriate cached route table entry created from previous DRA (redirect)
interactions does not exist. Cached route table entries are created from the
Redirect-Host, Redirect-Host-Usage and Redirect-Max-Cache-Time AVPs as
described in clause 6.12, 6.13 and 6.14 of IETF RFC 3588 [14].
...
sent over Rx as defined in 3GPP TS 29.214 [10].
...
The IMS calls out procedures for NAT traversal for media and 170vailabili
...
> 3c. The BPCF acknowledges to the V-PCRF by sending the TEA/RAA command.
  1. For the non-roaming case, triggered by step 2, the BPCF initiates an S9a session establishment with the PCRF by sending a CCR to the PCRF with the CC-Request-Type AVP set to the value INITIAL_REQUEST. The BPCF provides the data as described in 3GPP TS 29.215 [22], clause A.5.1.2.
...
IETF RFC 3588 [14], and include the PCRF identity in the Redirect-Host AVP in
...
6\. The V-PCRF-1 returns a TEA 215vailab.
...
decisions when overload control is applied. This includes the definition of
Diameter message priority related AVPs and the Diameter message priority
related behaviour.
