...
[44] 3GPP TS 23.179: \"Functional architecture and information flows to
...
sessions as described in subclause 4.4.8.
...
in the PCEF. The AA-Answer message shall also include the IP-CAN-Type AVP if
the PCRF has previously requested to be updated with this information in the
PCEF/BBERF. In that case, the AA-Answer message shall also include the RAT
type information within the RAT-Type AVP and AN-Trusted AVP when applicable
for the specific IP-CAN Type. In addition, if IP flow mobility applies to
...
AA-Answer message shall include the IP-CAN-Type AVP if the PCRF has previously
requested to be updated with this information in the PCEF/BBERF. In that case,
the AA-Answer message shall also include the RAT type information within the
RAT-Type AVP and AN-Trusted AVP when applicable for the specific IP-CAN Type.
In addition, if IP flow mobility applies to service data flows as specified in
3GPP TS 29.212 [8], such that a subset of the flows within the AF session are
affected, the PCRF shall also include IP-CAN-type and RAT type information (if
applicable) to IP flow mobility related if the PCRF has previously requested
to be updated with this information in the PCEF/BBERF flows. The IP flow
...
AVP (if applicable) AN-Trusted AVP (if applicable) and AN-GW-Address AVP (if
applicable) for the UE's new IP-CAN/RAT. If the PCRF is informed of an IP-CAN
type change due to IP flow mobility as specified in 3GPP TS 29.212 [8], where
a subset of the flows within the AF session are affected, the PCRF shall
...
accumulated usage as provided by the PCEF to the AF in a RA-Request (RAR)
...
the accumulated usage as provided by the PCEF to the AF in ST-Answer (STA)
...
Action AVP set to INDICATION_OF_FAILED_RESOURCES_ALLOCATION.
...
Indicator AVP set to PRIORITY_SHARING_ENABLED. Upon reception of this
...
subclauses 4.5.27 and 4a.5.17 and handle the media flow excluded from priority
...
NOTE 1: The AVP header bit denoted as ‘M’, indicates whether support of the AVP is required. The AVP header bit denoted as ‘V’, indicates whether the optional Vendor-ID field is present in the AVP header. For further details, see IETF RFC 3588 [10]. NOTE 2: The value types are defined in IETF RFC 3588 [10]. NOTE 3: AVPs marked with a supported feature (e.g. "ProvAFsignalFlow", "SponsoredConnectivity", "Rel10" or "NetLoc") are applicable as described in subclause 5.4.1 NOTE 4: Volume Usage monitoring control functionality is applicable for SponsoredConnectivity supported feature. Time Based Usage monitoring control is applicable for SCTimeBasedUM supported feature. |  |  |  |  |  |  |  |  |   
...
The Result-Code AVP values defined in Diameter Base IETF RFC 3588 [10] are
...
from SDP according to 3GPP TS 29.213 [9] clause 6.2. However for multiplexed
RTP/RTCP flows (as negotiated using the\"a=rtcp-mux\" SDP attribute defined in
IETF RFC 5761 [42]), a P-CSCF supporting RTP/RTCP transport multiplexing shall
set the Flow-Status set to \"ENABLED\" to prevent that RTCP is blocked. If the
P-CSCF chooses to modify the values, the P-CSCF shall store the last received
SDP.
...
the UE is attached to using the procedure specified in clause 4.4.1.
...
the PCRF according to clause A.1 and shall include in this AA-request:
\- the \"ACCESS_NETWORK_INFO_REPORT\" value within the Specific-Action AVP;
and
\- the required access network information within the Required-Access-Info
AVP.
...
### A.13.1 General
Within the framework of MCPTT, when the SIP Core (3GPP TS 23.179 [44]) is
...
the Media-Component-Description AVP in the AAR command. See 3GPP TS 23.179
