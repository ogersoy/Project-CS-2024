...
distance from the DUT, , will vary from lab to lab. In the candidate solution
4 typically two antennas will be used. The DUT will be rotated around its
centre line by using the electronically-controlled turntable system or using a
manual turning approach. The angle of turning is designated as . An anechoic
chamber of sufficient size () m is used and this will vary from lab to lab.
Some uplink signal amplifier may be used to ensure good isolation between Tx
...
where
is the compensation factor for antenna probe _i_ to be adjusted by channel
emulator.
is the measured attenuation via antenna probe _i_ , and,
is the highest attenuation over all antenna probes.
3) Adjust the channel emulator outputs by the compensation factor.
4) Calculate the pathloss of each route as follows:
where is the reference antenna gain.
...
2) Only one probe antenna is used at a time.
Rotate the UE using an angle step, of 45° and measure the UE reference
sensitivity. For each antenna probe and angle of step,, and polarization,
record the UE reference sensitivity in Table B.3.1-1.
...
= 8 for angle step of 45°, and,
is the recorded reference sensitivity for each angle step .
...
for measurement antenna installed inside the chamber. _N_ is the total number
...
the measured pattern. The detailed path loss calibration process is as below,
which is from Annex B in [4].
**Path loss calibration process**
is the attenuation between P and B, see figure B.3-1.
Where is cable loss from A to B. The cable AC connecting the substitution
antenna should be such that its influence upon radiation pattern measurements
is minimal. is the attenuation between points A and B. is the efficiency or
gain of the calibration antenna at the frequency of interest.
...
full spherical scanning is performed to determine.
...
minimum sphere surface surrounding the DUT antenna is.To compare the two-stage
method results with the multi-probe method, the power at the temporary antenna
connectors is set to be the same as would have been received when using the
multi-probe method.
...
9) Rotate the DUT by degrees and repeat steps 6 to 8 until all rotation angles
have been measured
...
And the the antenna correlation coefficient is
...
8) Rotate the antennas by degrees and repeat steps 5 to 7 until all rotation
