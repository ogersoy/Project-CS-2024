...
procedures are described in 4.1.2.17217.2 and 4.1.2.1717.3.
...
attachment
> The MDT activation procedure after UE attachment in 5GC is the same as in
> EPC, When UDM activates the trace, for MDT job, to the AMF the following
> configuration parameters shall be included in the message:
\- Area Scope (TA, Cell).
\- Trace Reference.
...
location procedure. This is required if the signallingbased MDT is started
...
AMF receive the user consent information it shall store it in its subscriber
database.
The AMF shall also check the roaming status of the user. If the user is within
his home operator's PLMNs and the user has given his consent, the AMF shall
send the Management based MDT PLMN List IE to the gNB during the UE context
setup procedure. Otherwise the AMF shall not send theManagement based MDT PLMN
List IE to the gNB.
...
for the UE.**
**In case of signalling based trace activation (subscription based MDT), the
gNB shall propagate the Trace Session parameters together with the MDT
specific parameters to the target cell regardless of whether the source or
target cell is part of the configured area scope in case of an Intra-PLMN
handover over Xn or N2.**
...
target cell is part of the configured area scope.
For signalling based MDT configuration (i.e. subscription based MDT), when a
UE that has been configured with MDT hands over to another gNB (i.e. in
connected mode) and the Signalling Based MDT PLMN List conditions mentioned
above are satisfied:
\- with an Xn handover: the MDT configuration shall be passed to the gNB in
the Xn handover request for continuity of MDT data collection. The new gNB
shall stop the MDT collection if the new conditions are not within the
criteria for MDT data collection.
...
+----------------------+----------------------+----------------------+ | SMF | Start triggering | Stop triggering | | | events | events | +----------------------+----------------------+----------------------+ | UE initiated PDU | NAS signalling from | Sending of | | Session | the UE, N16 | Nsmf_PDUSess | | Establishment | signalling from | ion_CreateSMContext | | procedure | V-SMF | response | | | | | | | Receipt of | | | | Nsmf_PDUSess | | | | ion_CreateSMContext | | | | request from AMF | | | | | | | | NAS signalling from | | | | the UE, N16a | | | | signalling from | | | | I-SMF | | +----------------------+----------------------+----------------------+ | UE initiated PDU | NAS signalling from | Sending of | | Session Modification | the UE, N16 | Nsmf_PDUSess | | procedure | signalling from | ion_UpdateSMContext | | | V-SMF | response | | | | | | | Receipt of | | | | Nsmf_PDUSess | | | | ion_UpdateSMContext | | | | request from AMF | | | | | | | | NAS signalling from | | | | the UE, N16a | | | | signalling from | | | | I-SMF | | +----------------------+----------------------+----------------------+ | UE initiated PDU | NAS signalling from | Sending of | | Session Release | the UE, N16 | Nsmf_PDUSessi | | procedure | signalling from | on_ReleaseSMContext | | | V-SMF | response | | | | | | | Receipt of | | | | Nsmf_PDUSessi | | | | on_ReleaseSMContext | | | | request from AMF | | | | | | | | NAS signalling from | | | | the UE, N16a | | | | signalling from | | | | I-SMF | | +----------------------+----------------------+----------------------+ | UE initiated PDU | N11 request from AMF | Sending of N11 | | Session UP | (may come from a NAS | response to AMF | | activation / | SR or Registration) | | | deactivation | | | +----------------------+----------------------+----------------------+ | Mobility of a PDU | NAS signalling from | Sending of NAS | | Session between 3GPP | the UE (Request Type | signalling to UE | | and N3GPP access to | = \"Existing PDU | | | 5GC | Session\") | | +----------------------+----------------------+----------------------+ | Mobility of a PDU | N11 request from AMF | Sending of | | Session from EPC | (comes from a N26 | Nsmf_PDUSess | | | forward relocation | ion_UpdateSMContext | | | request) | response | +----------------------+----------------------+----------------------+
* * *
...
is supported.
This parameter is optional when file based tracing in UMTS is supported.
...
This parameter is mandatory when streaming trace and/or MDT is supported.
The parameter may be present only if the IP address of TCE is not present.
...
DL M6 for DL M5 for DL M4 for DL M2 M1 spare M7 for UL M6 for UL M5 for UL M4
for UL M9  
spare  
spare
* * *
...
also TS 37.320 clauses 5.2.1.1 and 5.4.1.1 [30].
* * *
...
to which the Trace records shall be transferred.
...
parameter is applicable only for LTE and NR. In LTE tthe bit1 (GNSS) may be
selected only if the M1 measurement is selected in the \"List of
measurements\" parameter (defined in clause 5.10.3).
For LTE, iif the positioning method parameter indicates both E-Cell ID and
GNSS positioning, the eNB may use E-Cell ID measurement collection only if the
UE does not provide GNSS based location information.
...
measurements parameter (defined in clause 5.10.3) in NR is set to 1. The
parameter is used only in case of RAN side measurements whose configuration is
determined by RRM.
This measurement parameter defines the collection period that should be used
to collect available measurement samples in case of RRM configured
...
log availability in the RRCConnectionSetupComplete message to the eNB/RNC.
When the eNB/RNC receives this indication it can request the MDT log (if the
UE is still in the same RAT type where the MDT configuration was done) by
sending the UEInformationRequest message to the UE. The MDT logs are sent to
the network in the UEInformationResponse message. At the reception of the
