...
* * *
Information element Status Description MC service group ID M (NOTE) MC service
group ID of the group NOTE: If the Result information element indicates
failure then the value of MC service group ID information element has no
meaning.
* * *
...
group ID of the temporary group MC service list O List of MC services whose
service communications are to be enabled on this temporary group Result M
Indicates the success or failure of group regroup NOTE: Shall be present if
...
the media player.
NOTE 3: This procedure requires modifications to the 3GPP TS 23.468 [18], to
include the new requests/responses mentioned in steps 1 and 2. It requires a
BM-SC enhancement and can therefore only be used if supported by BM-SC.
...
ROHC information.
Editor\'s note: This procedure requires modifications to the 3GPP TS 23.468
[18], to include the new requests/responses mentioned in steps 1 and 2. It
requires a BM-SC enhancement and can therefore only be used if supported by
BM-SC.
...
affiliation status of the user for the requested MC service group(s).
4\. The MC service server provides a de-affilication notification with the
...
7\. The MC service server provides a de-affilication notification with the
group information to the MC service client.
...
(e.g. a request to upgrade the existing MC service call to an MC servie
...
managed by this MC service server. 6. The MC service server utilizes the
elevated priority when requesting resources for new or existing MC service
communication.
