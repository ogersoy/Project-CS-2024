...
The NID shall consist of an assignment mode and an NID value as shown in
figure 12.7-1.
...
the labels \"epdg.epc\".
The result of the 5GS Tracking Area Identity based Emergency ePDG FQDN will
be:
...
NOTE 1: The N3IWF FQDN configured in the UE can have a different format than
those specified in the following clauses.
...
28.3.2.2.5.
Editor\'s note: It is FFS whether N3IWF FQDN for emergency service is
supported.
...
\"pub.3gppnetwork.org\". The third and fourth labels together shall uniquely
identify the PLMN. The first two labels shall be \"n3iwf.5gc\". The result of
the N3IWF FQDN will be:
...
Visited Country FQDN of N3IWF will be:
...
PLMN as specified in clause 6.3.6.2a of 3GPP TS 23.501 [119].
...
clause 12.7. The first and second labels shall be \"n3iwf.5gc\". The resulting
Visited Country FQDN for SNPN N3IWF will be:
...
country.pub.3gppnetwork.org\".
NOTE 2: The identity (i.e. the corresponding DNS record) of an SNPN\'s N3IWF
...
visited country.
NOTE 1: If the visited country mandates the selection of the N3IWF in this
...
properly configured for multiple SNPNs using the same locally assigned NID. If
the visited country mandates the selection of the N3IWF in this country, the
NAPTR record(s) associated to the Visited Country FQDN of SNPNs that use a
locally assigned NID can be provisioned with the replacement field containing
an FQDN that cannot be resolved to an IP address.
...
[124]. The structure of the Telescopic FQDN is as specified below:
\.\,
where:
...
the definition and use of the network specific identifier.
...
register to 5GCN via a trusted non-3GPP access network (see clause 4.12a in
...
network (see clause 4.12 b in 3GPP TS 23.502 [120]), the N5CW device shall use
a NAI in the following format:
\"\\@nai.5gc-
...
trusted non-3GPP access as described in clause 6.3.12 of 3GPP TS 23.501 [119].
...
In roaming scenarios however, the NAI may contain both the VPLMN ID and the
HPLMN ID, in which case a decorated NAI shall be used (see clause 28.7.7.2).
...
domain to the NF Set ID of an AMF set defined in this clause. NF Instances of
an NF Set are equivalent and share the same MCC, MNC, NID (for SNPN), NF type
and NF Set ID.
