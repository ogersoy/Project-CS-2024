...
26 CA_1-5-7 1 5 7 CA_1-7-20 1 7 20 CA_1-18-28 1 18 28 CA_1-19-21 1 19 21
CA_2-4-5 2 4 5 CA_2-4-12 2 4 12 CA_2-4-13 2 4 13 CA_2-4-29 2 4 29 CA_2-5-12 2
...
{width="5.988194444444445in" height="2.6777777777777776in"}
...
carrier aggregation.
{width="6.6930555555555555in" height="3.015277777777778in"}
...
transmitter.
Rated total output power(P~rated,t~) of the base station is the mean power for
BS operating in single carrier, multi-carrier, or carrier aggregation
...
base station to transmit a signal according to E-TM1.1.
For a BS declared to be capable of multi-carrier and/or CA operation, set the
base station to transmit according to E-TM1.1 on all carriers configured using
in the applicable test configuration and corresponding power setting specified
in sub-clause 4.10 and 4.11.
...
Note 2: For CRS Ês \ For a BS declared to be capable of multi-carrier and/or CA operation, set
> the BS to transmit according to E-TM1.1 on all carriers configured using the
...
filter of bandwidth equal to the Transmission bandwidth configuration
BW~config~ centred on the assigned channel frequency. 70μs average window
centre is set from 35μs after end of one transmitter ON period + 17μs to 35μs
before start of next transmitter ON period -- 17μs.
3) For BS supporting contiguous CA, measure the mean power spectral density
over 70μs filtered with a square filter of bandwidth equal to the Aggregated
Channel Bandwidth **BW~Channel_CA~ centred on (F~edge_high~+F~edge_low~)/2.**
70μs average window centre is set from 35μs after end of one transmitter ON
period + 17μs to 35μs before start of next transmitter ON period -- 17μs.
For a multi-band capable BS, with separate antenna connector, the antenna
...
carrier frequency 3.0GHz \ For a BS declared to be capable of multi-carrier and/or CA operation, set
> the BS to transmit according to E-TM 3.1 on all carriers configured using
...
according to the test procedure in subclause 6.3.2.4.2 and test requirements
in subclause 6.3.2.5.
In addition, for a multi-band capable BS, the following step shall apply:
...
% 256QAM 4.5% NOTE: The EVM requirement for 256QAM applies to Home BS, Local
Area BS and Medium Range BS.
* * *
...
> Aggregation set the base station to transmit using the applicable test
> configuration and corresponding power setting specified in sub-clause 4.10
> and 4.11.
>
...
Annex I.1.1.
##### 6.5.4.4.2 Procedure
Set-up BS transmission at manufacturer's declared rated output power. Channel
...
bandwidth shall be less than or equal to the Aggregated Channel Bandwidth as
...
first adjacent channel inside any sub-block gap with a gap size W~gap~ ≥
15MHz. The ACLR requirement for the second adjacent channel applies inside any
sub-block gap with a gap size W~gap~ ≥ 20 MHz. The CACLR requirement in
subclause 6.6.2.6 applies in sub block gaps for the frequency ranges defined
in Table 6.6.2-5/6.
...
base station to transmit a signal according to E-TM1.1 at manufacturer's
...
b) Measure CACLR inside sub-block gap or Inter RF Bandwidth gap as specified
in subclause 6.6.2.6, if applicable.
3) Repeat the test with the channel set-up according to E-TM1.2.
In addition, for a multi-band capable BS, the following step shall apply:
4) For multi-band capable BS and single band tests, repeat the steps above per
involved band where single band test configurations and test models shall
apply with no carrier activated in the other band. For multi-band capable BS
...
corresponding filter bandwidth ACLR limit W~gap~ ≥ 15 MHz 2.5 MHz 3.84 Mcps
UTRA RRC (3.84 Mcps) 44.2 dB W~gap~ ≥ 20 MHz 7.5 MHz 3.84 Mcps UTRA RRC (3.84
Mcps) 44.2 dB NOTE: The RRC filter shall be equivalent to the transmit pulse
shape filter defined in TS 25.104 [15], with a chip rate as defined in this
table.
* * *
...
the Base Station RF Bandwidth edge (inside the gap) Assumed adjacent channel
carrier (informative) Filter on the adjacent channel frequency and
corresponding filter bandwidth ACLR limit W~gap~ ≥ 15 MHz 2.5 MHz 5MHz E-UTRA
Square (BW~Config~) 44.2 dB W~gap~ ≥ 20 MHz 7.5 MHz 5MHz E-UTRA Square
(BW~Config~) 44.2 dB
* * *
...
gap sizes listed in Table 6.6.2-5,
...
the Base Station RF Bandwidth edge (inside the gap) Assumed adjacent channel
carrier (informative) Filter on the adjacent channel frequency and
corresponding filter bandwidth CACLR limit 5 MHz ≤ W~gap~ \ As a general rule, the resolution bandwidth of the measuring equipment
...
##### 6.6.3.4.2 Procedure
1) For a BS declared to be capable of single carrier operation only, set the
BS transmission at manufacturer's declared rated output power. Channel set-up
shall be according to E-TM 1.1.
> For a BS declared to be capable of multi-carrier and/or CA operation, set
> the base station to transmit according to E-TM1.1 on all carriers configured
...
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz) for Category A
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz) for Category A
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz) for Category B
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz) for Category B
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz) for Category B
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz, emissions shall not exceed the
...
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz)
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz)
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz)
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz, emissions shall not exceed the maximum
...
Frequency offset of measurement filter ‑3dB point, ∆f Frequency offset of
measurement filter centre frequency, f_offset Test requirement Measurement
bandwidth (Note 3) 0 MHz ≤ ∆f \3GHz, emissions shall not exceed the
...
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz)
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \3GHz)
+----------------+----------------+----------------+----------------+ | Frequency | Frequency | Test | Measurement | | offset of | offset of | requirement | bandwidth | | measurement | measurement | (Note 1, 2) | (Note 3) | | filter ‑3dB | filter centre | | | | point, ∆f | frequency, | | | | | f_offset | | | +----------------+----------------+----------------+----------------+ | 0 MHz ≤ ∆f \1GHz
* * *
...
requirement Measurement bandwidth (Note 3) 1.4 MHz 0 MHz ≤ ∆f \ 3) Detection mode: True RMS.
>
> 4) Configure the BS with transmitter(s) active.
##### 6.6.4.4.2 Procedure
1) For a BS declared to be capable of single carrier operation only, set the
BS to transmit a signal according to E-TM1.1 at manufacturer's declared rated
output power.
> For a BS declared to be capable of multi-carrier and/or CA operation, set
> the base station to transmit according to E-TM1.1 on all carriers configured
> using the applicable test configuration and corresponding power setting
...
public safety operations
* * *
Operating Band Frequency range Maximum Level Measurement Bandwidth Note 26 851
- 859 MHz -13 dBm 100 kHz Applicable for offsets > 37.5kHz from the channel
edge
* * *
...
lower than that of the rated total output power in the operating band.
...
Annex I.1.2.
#### 6.7.4.2 Procedures
1) For a BS declared to be capable of single carrier operation only, generate
the wanted signal according to E-TM1.1 at manufacturer's declared rated output
power.
For a BS declared to be capable of multi-carrier and/or CA operation, set the
base station to transmit according to E-TM1.1 on all carriers configured using
the applicable test configuration and corresponding power setting specified in
...
#### 7.2.4.2 Procedure
1) For FDD BS start BS transmission according to E-TM 1.1 at manufacturer's
declared rated output power
2) Set the test signal mean power as specified in table 7.2-1 for Wide Area
BS, in Table 7.2-2 for Local Area BS, in Table 7.2-3 for Home BS and in Table
7.2-4 for Medium Range BS.
...
> 7.4-1 for Wide Area BS, in Table 7.4-2 for Local Area BS, in Table 7.4-3 for
> Home BS and in Table 7.4-4 for Medium Range BS on one side of the F~C~.
>
> 2) Adjust the signal generator for the interfering signal as specified in
> Table 7.4-1 for Wide Area BS, in Table 7.4-2 for Local Area BS, in Table
> 7.4-3 for Home BS and in Table 7.4-4 for Medium Range BS at opposite side of
> the F~C~ and adjacent to the wanted signal.
>
> 3) Measure the throughput according to Annex E.
>
...
Table 7.4-1 Wide Area BS in-channel selectivity
...
station under test to the level specified in Table 7.5-3 for Wide Area BS, in
Table7.5-4 for Local Area BS, in Table 7.5-5 for Home BS and in Table 7.5-6
for Medium Range BS.
...
Table 7.5-3 for Wide Area BS, in Table7.5-4 for Local Area BS, in Table 7.5-5
for Home BS and in Table 7.5-6 for Medium Range BS.
3) Measure the throughput according to Annex E, for multi-carrier and/or CA
operation the throughput shall be measured for relevant carriers specified by
the test configuration specified in subclause 4.10 and 4.11.
...
4) Measure the throughput according to Annex E, for multi-carrier and/or CA
operation the throughput shall be measured for relevant carriers specified by
the test configuration specified in subclause 4.10 and 4.11.
...
specified in Table 7.2-1 for each channel bandwidth and further specified in
Annex A.
For Medium Range BS, the wanted and the interfering signal coupled to the BS
antenna input are specified in Tables 7.5-1 and 7.5-2 for narrowband blocking
and in Table 7.5-6 for ACS. The reference measurement channel for the wanted
signal is specified in Table 7.2-4 for each channel bandwidth and further
...
for ACS. The reference measurement channel for the wanted signal is specified
...
Table 7.5-6: Adjacent channel selectivity for Medium Range BS
...
7.6-1a, 7.6-1b, 7.6-1c and 7.6-2 in clause 7.6.5.
...
#### 7.6.4.2 Procedure
1) For FDD BS declared to be capable of sinle carrier operation only, start BS
transmission according to E-TM 1.1 at manufacturer's declared rated output
power.
For a FDD BS declared to be capable of multi-carrier and/or CA operation, set
the BS to transmit according to E-TM 1.1 on all carriers configured using the
applicable test configuration and corresponding power setting specified in
...
Area BS, in Tables 7.6-1a, 7.6-2 and 7.6-4 for Local Area BS, in Table 7.6-1b
and 7.6-2 for Home BS and in Table 7.6-1c, 7.6.2 and 7.6-5 for Medium Range
BS. The E-UTRA interfering signal shall be swept with a step size of 1 MHz
starting from the minimum offset to the channel edges of the wanted signal as
specified in Table 7.6-2. The CW interfering signal shall be swept with a step
size of 1 MHz within the range specified in Table 7.6-1 and 7.6-3 for Wide
Area BS, in Table 7.6-1a and 7.6-4 for Local Area BS, in Table 7.6-1b for Home
BS and in Table 7.6-1c and 7.6-5 for Medium Range.
3) Measure the throughput of the wanted signal at the BS receiver according to
Annex E, for multi-carrier and/or CA operation the throughput shall be
measured for relevant carriers specified by the test configuration specified
in subclause 4.10 and 4.11.
...
#### 7.7.4.2 Procedure
1) For a FDD BS declared to be capable of single carrier operation only, start
BS transmission according to E-TM 1.1 at manufacturer's declared rated output
power.
For a FDD BS declared to be capable of multi-carrier and/or CA operation, set
the BS to transmit according to E-TM 1.1 on all carriers configured using the
...
Medium Range BS P~REFSENS~ + 6dB* -47  
...
+----------------------+----------------------+----------------------+ | E-UTRA | Interfering signal | Type of interfering | | | centre frequency | signal | | channel bandwidth of | offset from the | | | the lowest/highest | lower/upper Base | | | carrier received | Station RF Bandwidth | | | [MHz] | edge [MHz] | | +----------------------+----------------------+----------------------+ | 1.4 | ±2.1 | CW | +----------------------+----------------------+----------------------+ | | ±4.9 | 1.4MHz E-UTRA signal | +----------------------+----------------------+----------------------+ | 3 | ±4.5 | CW | +----------------------+----------------------+----------------------+ | | ±10.5 | 3MHz E-UTRA signal | +----------------------+----------------------+----------------------+ | 5 | ±7.5 | CW | +----------------------+----------------------+----------------------+ | | ±17.5 | 5MHz E-UTRA signal | +----------------------+----------------------+----------------------+ | 10 | ±7. 375 | CW | +----------------------+----------------------+----------------------+ | | ±17.5 | 5MHz E-UTRA signal | +----------------------+----------------------+----------------------+ | 15 | ±7.25 | CW | +----------------------+----------------------+----------------------+ | | ±17.5 | 5MHz E-UTRA signal | +----------------------+----------------------+----------------------+ | 20 | ±7.125 | CW | +----------------------+----------------------+----------------------+ | | ±17.5 | 5MHz E-UTRA signal | +----------------------+----------------------+----------------------+
...
HARQ pattern
* * *
Parameter Value Number of TTIs for a TTI bundle 4 RV sequence for 4 TTIs
within a TTI bundle 0, 2, 3, 1 HARQ round trip time **12 ms** Maximum number
of HARQ transmissions for a TTI bundle 5
* * *
3) The multipath fading emulators shall be configured according to the
corresponding channel model defined in annex B.
...
{width="6.000694444444444in" height="0.70625in"}
...
1.4 MHz 3 MHz 5 MHz 10 MHz 15 MHz 20 MHz 1 2 Normal EVA 5 Low -4.1 -4.1 -4.1
-4.1 -4 -4 ETU 300* Low -7.3 -7.3 -7.4 -7.3 -7.4 -7.3 4 EVA 5 Low -7.8 -7.7
-7.7 -7.7 -7.8 -7.8 ETU 300* Low -9.7 -9.7 -9.7 -9.8 -9.7 -9.7 8 EVA 5 Low
-10.8 -10.7 -10.8 -10.8 -10.7 -10.8 ETU 300* Low -11.9 -11.8 -11.9 -11.8 -11.8
-11.9 Note*: Not applicable for Local Area BS and Home BS.
* * *
NOTE: If the above Test Requirement differs from the Minimum Requirement then
the Test Tolerance applied for this test is non-zero. The Test Tolerance for
this test and the explanation of how the Minimum Requirement has been relaxed
...
Low - - - -7.7 -7.9 -7.9 8 Normal EPA 5 Low - - - -11.1 -11.2 -11.2 EVA70 Low
- - - -10.9 -11.1 -11.0
* * *
NOTE: If the above Test Requirement differs from the Minimum Requirement then
the Test Tolerance applied for this test is non-zero. The Test Tolerance for
...
Low - - - -6.6 -6.7 -6.7 8 Normal EPA 5 Low - - - -10.5 -10.3 -10.5 EVA70 Low
- - - -10.3 -10.4 -10.4
* * *
...
Low - - - -4.4 -4.5 -4.5 8 Normal EPA 5 Low - - - -8.2 -8.2 -8.3 EVA70 Low - -
- -8.1 -8.2 -8.1
* * *
NOTE: If the above Test Requirement differs from the Minimum Requirement then
the Test Tolerance applied for this test is non-zero. The Test Tolerance for
...
- - - -2.3 -2.5 -2.6 8 Normal EPA 5 Low - - - -6.7 -6.7 -6.7 EVA70 Low - - -
-6.4 -6.5 -6.6
* * *
NOTE: If the above Test Requirement differs from the Minimum Requirement then
the Test Tolerance applied for this test is non-zero. The Test Tolerance for
...
-7.7 -7.7 -8.5 -8.7 -8.7 -8.7 EVA 70 Low -8.2 -8.4 -8.5 -8.5 -8.6 -8.7 8
Normal EPA 5 Low -10.6 -10.7 -11.1 -11.2 -11.1 -11.2 EVA 70 Low -10.9 -11.0
-11.0 -11.0 -11.0 -11.0
* * *
NOTE: If the above Test Requirement differs from the Minimum Requirement then
the Test Tolerance applied for this test is non-zero. The Test Tolerance for
...
-16.5 -16.5 AWGN 1340 Hz -15.6 -15.2 -17.5 -17.5 8 AWGN 0 -19.0 -18.8 -20.6
-20.7 ETU 70 Low 270 Hz -15.0 -14.5 -16.4 -16.4 AWGN 625 Hz -17.4 -17.1 -19.0
-19.1 AWGN 1340 Hz -18.4 -18.1 -20.2 -20.2
* * *
NOTE: If the above Test Requirement differs from the Minimum Requirement then
the Test Tolerance applied for this test is non-zero. The Test Tolerance for
this test and the explanation of how the Minimum Requirement has been relaxed
...
pattern
* * *
Reference channel A11-1 Allocated resource blocks 3 DFT-OFDM Symbols per
subframe 12 Modulation QPSK Code rate 11/27* Payload size (bits) 328 Transport
block CRC (bits) 24 Code block CRC size (bits) 0 Number of code blocks - C 1
Coded block size including 12bits trellis termination (bits) 1068 Total number
of bits per sub-frame 864 Total symbols per sub-frame 432 Note *: code rate
per TTI
* * *
...
from Band 1 and are applied for performance verification in all frequency
bands.
...
Where
\- {width="0.3020833333333333in" height="0.21805555555555556in"} is the
spatial correlation matrix at the UE with same polarization,
\- {width="0.34375in" height="0.21805555555555556in"} is the spatial
...
2x8 case
* * *
In Table B.5A.3-2, {width="0.17708333333333334in"
height="0.20833333333333334in"} is a {width="0.3333333333333333in"
...
requirements shall be met within a window W\ | | | | | 1GH | []{.underline} | | | | z]{.underline} | | | | | | []{.underline} | | | | For 1.4MHz BW: | | | | | | 1.5dB, f ≤ | | | | Offsets \ | | | | | 1GH | | | | | z]{.underline} | | | | | | | | | | For 1.4MHz BW: | | | | | | | | | | Offsets \ | | | | | 1GH | | | | | z]{.underline} | | | | | | | | | | All BWs: | | | | | | | | | | [Additional | | | | | Reqts bands | | | | | 12,13,1 | | | | | 4]{.underline} | | | | | | | | | | All BWs: | | | | | | | | | | [Additional | | | | | Reqts bands | | | | | 2 | | | | | 0]{.underline} | | | | | | | | | | All BWs: | | | +----------------+----------------+----------------+----------------+ | 6.6.4.5.1 | [Category | 0dB | Formula: | | Transmitter | A]{.underline} | | | | spurious | | | Minimum | | emissions, | 9 kHz ≤ f \
...
{width="6.5in" height="1.4638888888888888in"}
...
{width="5.511111111111111in" height="3.111111111111111in"}
...
conditions
{width="5.848611111111111in" height="3.595138888888889in"}
