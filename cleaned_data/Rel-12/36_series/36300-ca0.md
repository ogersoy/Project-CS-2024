...
\- Like PCell, PSCell cannot be de-activated (see subclause 11.2);
...
\- Both MeNB and SeNB know the SFN and subframe offset of each other by OAM,
e.g., for the purpose of DRX alignment and identification of measurement gap.
When adding a new SCG SCell, dedicated RRC signalling is used for sending all
required system information of the cell as for CA described in sub-clause 7.5,
...
## 8.3 Sidelink communication related identities
...
Destination Layer-2 ID and LCID for identification of the RLC UM entity and
PDCP entity in the receiver;
...
used directly as the Destination Layer-2 ID in the MAC layer.
# 9 ARQ and HARQ
...
switch for handover as specified in section 10.1.2.2.1 are applicable for the
...
¾. The MeNB initiates the RRC connection reconfiguration procedure. The UE
...
> The SeNB uses the procedure to perform configuration changes of the SCG
> within the same SeNB, e.g. to trigger the release of SCG SCell(s) (other
> than PSCell), SCG bearer(s) and the SCG part of split bearer(s) (upon which
> the MeNB may release the bearer or reconfigure it to an MCG bearer), and to
...
> SeNB cannot initiate an SCG SCell addition except for the case of SI update
> of an SCG SCell. Figure 10.1.2.8.2-2 shows an example signalling flow for an
...
> The SeNB can decide whether the Random Access procedure is required, i.e.
> SCG change.
...
> step 1, the following MeNB initiated SeNB Modification procedure triggered
> by the MeNB in step 2 cannot be used for anything that would require a new
> SCG configuration (as _SCG-Config_ cannot be subsequently signalled by the
> SeNB).
...
¾. If required, the MeNB indicates in the _RRCConnectionReconfiguration_
message towards the UE that the UE shall release the entire SCG configuration.
In case the UE is unable to comply with (part of) the configuration included
in the _RRCConnectionReconfiguration_ message, it performs the reconfiguration
failure procedure.
...
½. The MeNB initiates the change of SeNB by requesting the target SeNB to
...
source SeNB. If data forwarding is needed the MeNB provides data forwarding
addresses to the source SeNB. Either direct data forwarding or indirect data
forwarding is used for SCG bearer. Only indirect data forwarding is used for
Split bearer. Reception of the SeNB Release Request message triggers the
source SeNB to stop providing user data to the UE and, if applicable, to start
data forwarding.
...
MCG bearer, RLC configured for SCG is released. During SCG change, S-K~eNB~
key is refreshed. To perform SCG change within the same SeNB, the SeNB
Modification procedure as described in section 10.1.2.8.2 is used and in this
...
follow the principles specified in sub-clause 10.4a.
...
the UE, eNB and MME implicitly knows the CDMA2000 network from the UE's RPLMN.
All UEs not supporting the per-PLMN signalling inter-work with the same
...
been replaced by the term Roaming and Access Restrictions, see section 10.4a.
...
HeNBs by using X2 handover in some cases (see section 4.6.1). If membership
verification is required for X2 mobility, the procedure described in Section
...
>
> 2) The UE sends an \"entering\" proximity indication when it determines it
> may be near a CSG member cell (based on autonomous search procedures). The
...
> information when the UE is not in the geographical area where its CSG member
> cells are located.
>
...
> and the selected target PLMN received in the Handover Required message and
> the stored CSG subscription data for the UE (see 3GPP TS 23.401 [17]). If
> the access control procedure fails, the MME ends the handover procedure by
> replying with the Handover Preparation Failure message. If the Cell Access
> Mode is present, the MME determines the CSG Membership Status of the UE
> handing over to the hybrid cell and includes it in the Handover Request
> message.
>
...
> to constantly check the CSG ID and associated PLMN ID of all cells it sees.
...
One deactivation timer is maintained per SCell but one common value is
configured per CG by RRC.
...
per CG. For split bearers, the PDCP data is considered in BSR only in one CG
that is configured by RRC.
...
one HARQ RTT. Based on the above the minimum active time is of length equal to
...
receiving MBMS service(s) via MBSFN when changing cell(s). E-UTRAN procedures
provide support for service continuity with respect to mobility within the
same MBSFN area. Within the same geographic area, MBMS services can be
provided on more than one frequency and the frequencies used to provide MBMS
services may change from one geographic area to another within a PLMN.
...
from the target cell MCCH.
To avoid the need to read MBMS related system information and potentially MCCH
on neighbour frequencies, the UE is made aware of which frequency is providing
which MBMS services via MBSFN through the combination of the following MBMS
assistance information:
\- user service description (USD): in the USD (see 3GPP TS 26.346 [49]), the
...
SAIs, see definition in section 15.3 of 3GPP TS 23.003 [26]) belonging to the
MBMS service area (see definition in 3GPP TS 23.246 [48]);
...
by acquiring MCCH and may apply these procedures even though a MBMS service is
not provided via MBSFN;
...
the following:
\- if the serving cell provides _SystemInformationBlockType15_ , the UE
considers that a frequency is providing the MBMS service via MBSFN if and only
if one of the MBMS SAI(s) of this frequency as indicated in
_SystemInformationBlockType15_ of the serving cell is indicated for this MBMS
service in the USD;
\- if the serving cell does not provide _SystemInformationBlockType15_ , the
UE in RRC_IDLE state may consider that a frequency included in the USD for the
MBMS service is providing this MBMS service as long as the UE reselects cells
where _SystemInformationBlockType13_ is provided.
In RRC_IDLE, the UE applies the normal cell reselection rules with the
following modifications:
\- the UE which is receiving MBMS service(s) via MBSFN and can only receive
these MBMS service(s) via MBSFN while camping on the frequency providing these
MBMS service(s) is allowed to make this frequency highest priority;
\- the UE which is interested in receiving MBMS service(s) via MBSFN and can
only receive these MBMS service(s) via MBSFN while camping on the frequency
providing these MBMS service(s) is allowed to make this frequency highest
priority when it intends to receive these MBMS service(s);
...
to UE implementation.
In RRC_CONNECTED, the UE that is receiving or interested to receive MBMS via
MBSFN informs the network about its MBMS interest via a RRC message and the
network does its best to ensure that the UE is able to receive MBMS and
...
change of user interest or of service availability).
...
The UE shall support MBMS reception on any serving cell and on any cell that
may be additionally configured as serving cell according to the UE
capabilities.
...
request to the MME. This message can be transmitted before the step 4. Only in
case of distributed MCE architecture radio resource setup is scheduled
according to the parameter \"time of MBMS data transfer\" which indicates an
absolute start time of data delivery, otherwise according to the \"minimum
time to MBMS data transfer\" parameter.
3\. MCE sends the MBMS Session Start Request message to the eNBs in the
targeted MBMS service area. If the MBMS Session Start message includes the
MBMS Service Area Identity with value 0 as defined in TS 23.003 [26], the MCE
shall consider that all those eNBs supporting the PLMN as indicated by the
received MBMS Session Start Request message are involved. The MCE then
determines in which MBSFN area(s) the service should be delivered.
...
session, e.g., the MBMS Service Area information to the eNB.
...
MME provides the information of the MBMS session, e.g., QoS and MBMS Service
Area, to the MCEs.
In this release the MBMS Session Update procedure only supports the update of
MBMS Service Area, the update of the allocation and retention priority of the
MBMS session and the update of time of MBMS data transfer where the last one
is used in the distributed MCE architecture only.
...
session, e.g., QoS and MBMS Service Area, to the MCEs.
...
RSRP measurement reports may be exploited for inter-eNB CoMP. For example, the
RSRP measurement reports can be used to determine and/or validate CoMP
hypotheses and benefit metrics.
...
irrespective of received power preference indication.
# 17 Void
## 17.1 Void
# 18 UE capabilities
...
uplink, no capability information is sent early in e.g. _RRCConnectionRequest_
...
subclause 10.2.2. The source RAN is not mandated to acquire other RAT
capabilities (i.e. other than the source and target RAT capabilities) in order
to start a handover preparation. This is described in subclause 19.2.2.5.6.
...
\- the MME locally changes the state of the UEs which used this signalling
connection to the ECM-IDLE state as described in TS 23.401 [17];
...
the modification of already established E-RAB configurations. The current
version of the specification supports the modification of the transport
information only. This procedure is used for DC if the SCG bearer option is
applied.
...
The eNB may also trigger EAB as specified in TS 23.401 [17] subclause
4.3.7.4.1 and TS 23.251 [54] subclause 4.6.
...
local configuration), it responds with the X2 REMOVAL FAILURE message.
...
certain scenarios (see 3GPP TR 36.816 [50]).
### 23.4.2 Solutions
...
A UE that supports IDC functionality indicates this capability to the network,
and the network can then configure by dedicated signalling whether the UE is
allowed to send an IDC indication. The IDC indication can only be triggered
...
removing SCells from the set of serving cells, or in case of uplink CA
operations, allocate uplink PRB resources on CC(s) whose inter-modulation
distortion and harmonics does not fall into the frequency range of the victim
system receiver.
...
predictable pattern of unscheduled periods by means of DRX mechanism.
...
supported uplink CA combinations suffering from IDC problems. The IDC
indication is also used to update the IDC assistance information, including
for the cases when the UE no longer suffers from IDC problems. In case of
inter-eNB handover, the IDC assistance information is transferred from the
...
This section describes the mechanisms to support traffic steering between
E-UTRAN and WLAN.
...
access network selection and traffic steering rules defined in section 23.6.2
and TS 36.304 [11]. The access network selection and traffic steering rules
defined in section 23.6.2 and TS 36.304 [11] are applied only to the WLANs of
...
defined in TS 36.304 [11], it performs traffic steering between E-UTRAN and
WLAN with APN granularity.
...
Communications.
A low complexity UE indicates UE Category 0 and has reduced Tx and Rx
capabilities compared to other UE of different categories.
A low complexity UE may access a cell only if SIB1 indicates that access of
low complexity UEs is supported. If the cell does not support low complexity
UEs, a low complexity UE considers the cell as barred.
The eNB determines that a UE is a low complexity device based on the LCID for
CCCH and the UE capability.
...
In this subclause, the UE radio protocol architecture for sidelink
communication is given for the user plane and the control plane.
...
for the user plane in subclause 6.
...
Layer-2 ID and ProSe Layer-2 Group ID combination. Parameters for logical
channel prioritization are not configured.
...
Figure 23.10.2.2-1: Control-Plane protocol stack for sidelink communication
...
A set of resource pools for sidelink control informationwhen the UE is out of
coverage for sidelink communication is pre-configured in the UE. The first
pool in the list is used for both transmission and reception of sidelink
control information while the other pools in the list are used for reception
of sidelink control information.
...
A set of resource pools for data when the UE is out of coverage for sidelink
communication is pre-configured in the UE. The first pool in the list is used
for transmission and reception of data while the other pools in the list are
used for reception of data.
...
direct radio signals via PC5. Sidelink discovery is supported only when the UE
...
discovery messages may act as a synchronisation source by transmitting a
synchronisation signal based on the resource information for synchronisation
signals provided in SIB19.
...
\- The eNB may provide a resource pool for UE autonomous resource selection
...
eNB that it wants to perform sidelink discovery announcement;
...
discovery message monitoring in SIB19. The SIB19 may contain detailed sidelink
discovery configuration used for announcing in neighbour cells of intra-
frequency as well.
...
A UE if authorised by the NW can announce discovery message only on serving
cell. The UE can monitor discovery resources in the same as well as other
frequencies than the serving cell, in same or different PLMNs.
  * The serving cell may provide in SIB19 a list of frequencies along > with PLMN ID on which the UE may aim to monitor discovery message;
  * The serving cell does not provide detailed sidelink discovery > configuration for other carrier frequencies. The UE shall read > SIB19 and other relevant SIBs on other carriers if it wants to > perform discovery message monitoring on those carriers;
...
\- The UE performs intra-frequency sidelink discovery announcement in
subframes in which a sidelink discovery resource pool is configured. The
sidelink discovery announcement shall not affect Uu transmission and/or
reception;
...
+--------+------------------+------------------+------------------+ | **#** | **Network |** Corresponding | **Remarks** | | | Action**| subclause** | | +--------+------------------+------------------+------------------+ | **1** | Intra-PCell | 10.1.2.8.2.1 | Causes also | | | Handover | Intra-MeNB | intra-PSCell | | | | handover | Change or | | | | involving SCG | Intra-SeNB | | | | change | Change. | | | | | | | | | | Also used for | | | | | KeNB refresh. | | | | | | | | | | Done via | | | | | Handover like in | | | | | Rel-8; SCG can | | | | | be retained | | | | | during the | | | | | Handover. | +--------+------------------+------------------+------------------+ | **2** | Intra-PSCell SCG | 10.1.2.8.2 SeNB | Also used for | | | Change | Modification | S-KeNB refresh. | | | | (MeNB initiated | | | | | SeNB | Does not require | | | | Modification) | intra-PCell | | | | | Handover. | | | | 10.1.2.8.2 SeNB | | | | | Modification | | | | | (SeNB initiated | | | | | SeNB | | | | | Modification) | | +--------+------------------+------------------+------------------+ | **3** | Intra-MeNB | 10.1.2.8.2.1 | Causes also | | | Handover | Intra-MeNB | intra-PSCell | | | | handover | Change or | | | | involving SCG | Intra-SeNB | | | | change | Change. | | | | | | | | | | Done via | | | | | Handover like in | | | | | Rel-8; SCG can | | | | | be retained | | | | | during the | | | | | Change. | +--------+------------------+------------------+------------------+ | **4** | Intra-SeNB SCG | 10.1.2.8.2 SeNB | Requires SCG | | | Change | Modification | change but does | | | | (MeNB initiated | not require | | | | SeNB | intra-PCell | | | | Modification) | Handover. | | | | | | | | | 10.1.2.8.2 SeNB | | | | | Modification | | | | | (SeNB initiated | | | | | SeNB | | | | | Modification) | | +--------+------------------+------------------+------------------+ | **5** | Inter-MeNB | 10.1.2.8.5 MeNB | Done via | | | Handover | to eNB Change | Handover like in | | | | | Rel-8; SCG is | | | | | also released. | +--------+------------------+------------------+------------------+ | **6** | Inter-SeNB SCG | 10.1.2.8.4 | Requires SCG | | | Change | Change of SeNB | change. Only | | | | | supported when | | | | | the MeNB does | | | | | not change at | | | | | the same time. | +--------+------------------+------------------+------------------+ | **7** | SCG addition by | 10.1.2.8.1 SeNB | Only the MeNB | | | MeNB | Addition | can request an | | | | | SCG to be added. | +--------+------------------+------------------+------------------+ | **8** | SCG SCell | 10.1.2.8.2 SeNB | Only the MeNB | | | addition by MeNB | Modification | can request to | | | | (MeNB initiated | add SCells to | | | | SeNB | the SCG.If the | | | | Modification) | SeNB wants to | | | | | use the new | | | | | SCell as PSCell | | | | | it needs to | | | | | initiate an SeNB | | | | | modification. | +--------+------------------+------------------+------------------+ | **9** | Non-last SCG | 10.1.2.8.2 SeNB | For requesting | | | SCell release by | Modification | the release of | | | MeNB | (MeNB initiated | the last SCG | | | | SeNB | SCell i.e. | | | | Modification) | PSCell, the MeNB | | | | | has to request | | | | | SCG release | | | | | instead. | +--------+------------------+------------------+------------------+ | **10** | Non-last SCG | 10.1.2.8.2 SeNB | For requesting | | | SCell release by | Modification | the release of | | | SeNB | (SeNB initiated | the last SCG | | | | SeNB | SCell i.e. | | | | Modification) | PSCell, the SeNB | | | | | has to request | | | | | SCG release | | | | | instead. | | | | | | | | | | The SeNB can | | | | | request to | | | | | release the | | | | | PSCell only if | | | | | it also chooses | | | | | a new PSCell. | +--------+------------------+------------------+------------------+ | **11** | SCG release by | 10.1.2.8.3 SeNB | The MeNB can | | | MeNB | Release (MeNB | always request | | | | initiated SeNB | the release of | | | | Release) | the SCG and the | | | | | SeNB has to | | | | | comply. | +--------+------------------+------------------+------------------+ | **12** | SCG release by | 10.1.2.8.3 SeNB | The SeNB can | | | SeNB | Release (SeNB | always request | | | | initiated SeNB | the release of | | | | Release) | the SCG and the | | | | | MeNB has to | | | | | comply | +--------+------------------+------------------+------------------+ | **13** | SCG bearer | 10.1.2.8.2 SeNB | If this is the | | | addition by MeNB | Modification | first bearer to | | | | (MeNB initiated | be added in the | | | | SeNB | SeNB, SCG | | | | Modification) | addition is used | | | | | instead. | | | | (10.1.2.8.1 MeNB | | | | | initiated SeNB | | | | | Addition) | | +--------+------------------+------------------+------------------+ | **14** | SCG bearer | 10.1.2.8.2 SeNB | Allowed for both | | | release | Modification | the MeNB and the | | | | (MeNB initiated | SeNB, but the | | | | SeNB | SeNB cannot | | | | Modification) | request to | | | | | release the last | | | | 10.1.2.8.2 SeNB | SCG bearer if | | | | Modification | that would | | | | (SeNB initiated | result in no | | | | SeNB | bearers in SeNB | | | | Modification) | - SCG release | | | | | should be used | | | | | instead. | | | | | | | | | | The SeNB cannot | | | | | initiate the | | | | | release of an | | | | | SCG bearer and | | | | | make changes to | | | | | _SCG-Config_ at | | | | | the same time | | | | | e.g. change the | | | | | configuration of | | | | | the physical | | | | | layer. | +--------+------------------+------------------+------------------+ | **15** | Split bearer | 10.1.2.8.2 SeNB | | | | addition by MeNB | Modification | | | | | (MeNB initiated | | | | | SeNB | | | | | Modification) | | +--------+------------------+------------------+------------------+ | **16** | Split bearer | 10.1.2.8.2 SeNB | Allowed for both | | | release | Modification | the MeNB and the | | | | (MeNB initiated | SeNB, but the | | | | SeNB | SeNB cannot | | | | Modification) | request to | | | | | release the last | | | | 10.1.2.8.2 SeNB | split bearer if | | | | Modification | that would | | | | (SeNB initiated | result in no | | | | SeNB | bearers in SeNB | | | | Modification) | - SCG release | | | | | should be used | | | | | instead. | | | | | | | | | | The SeNB cannot | | | | | initiate the | | | | | release of an | | | | | SCG bearer and | | | | | make changes to | | | | | _SCG-Config_ at | | | | | the same time | | | | | e.g. change the | | | | | configuration of | | | | | the physical | | | | | layer. | +--------+------------------+------------------+------------------+ | **17** | MCG to SCG | 10.1.2.8.2 SeNB | Either the MeNB | | | bearer type | Modification | or the SeNB can | | | change (and vice | (MeNB initiated | request to | | | versa) | SeNB | release an SCG | | | | Modification) | bearer. | | | | | | | | | 10.1.2.8.2 SeNB | Done via SCG | | | | Modification | change, i.e. L2 | | | | (SeNB initiated | reset is | | | | SeNB | required for the | | | | Modification) | UE. | | | | | | | | | | The SeNB can | | | | | only request | | | | | change from SCG | | | | | to MCG by | | | | | requesting | | | | | release of SCG | | | | | bearer. | | | | | | | | | | The MeNB makes | | | | | the decision on | | | | | whether to | | | | | release the DRB | | | | | entirely or | | | | | change it to MCG | | | | | bearer. | +--------+------------------+------------------+------------------+ | **18** | MCG to Split | 10.1.2.8.2 SeNB | The SeNB can | | | Bearer Change | Modification | always request | | | (and vice versa) | (MeNB initiated | to release the | | | | SeNB | SCG part of a | | | | Modification) | split bearer. | | | | | | | | | 10.1.2.8.2 SeNB | Done via SCG | | | | Modification | change, i.e. L2 | | | | (SeNB initiated | reset is | | | | SeNB | required for the | | | | Modification) | UE. | | | | | | | | | | MeNB makes the | | | | | decision on | | | | | whether to | | | | | release the DRB | | | | | entirely or | | | | | change it to MCG | | | | | bearer. | +--------+------------------+------------------+------------------+
