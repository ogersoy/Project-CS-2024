...
of these APN policies are equally applicable, e.g. APNPolicy1 specifies
...
or more usage monitoring instances applicable at PCC Rule level.
...
then the PCEF shall, at the monitoring time, reset the usage threshold to the
...
defined in 3GPP TS 29.214 [10], the PCRF shall send a Gx RAR command to the
corresponding Gx session with a P-CSCF Restoration Indication to the PCEF.
...
PCRF shall consider the requirements described in clause 4.5.19.1.1:
...
consider the requirements described in clause 4.5.19.1.1:
...
When receiving the flow based usage thresholds from the AF, the PCRF shall use
the sponsor identity to generate a monitoring key and request usage monitoring
...
the required access network information to the PCRFas follows:
...
During bearer deactivation procedure, when the NetLoc feature is supported,
the PCEF shall provide the access network information to the PCRF by including
the user location information within the 3GPP-User-Location-Info AVP (if user
location information was requested by the PCRF and was provided to the PCEF),
the information on when the UE was last known to be in that location within
User-Location-Info-Time AVP (if user location information was requested by the
PCRF and the corresponding information was provided to the PCEF), the PLMN
identifier within the 3GPP-SGSN-MCC-MNC AVP (if the user location information
was requested by the PCRF but was not provided to the PCEF) and the timezone
...
application detection and control. When the start or stop of the application's
traffic, identified by TDF-Application-Identifier, is detected, if PCRF has
previously subscribed to the APPLICATION_START/APPLICATION_STOP Event-
Triggers, unless a request to mute such a notification (Mute-Notification AVP)
is part of the corresponding PCC Rule, the PCEF shall report the information
regarding the detected application's traffic in the Application-Detection-
Information AVP in the CCR command even if the application traffic is
discarded due to enforcement actions of the PCC rule.
...
{#a.5.12.1.2.1-provisioning-of-qos-rules-at-gxx-session-establishment .H6}
...
{#a.5.12.1.2.2-provisioning-of-qos-rules-for-emergency-services .H6}
When the PCRF receives IMS service information from the AF for an Emergency
...
the stop of a detected application's traffic and transfer of service data flow
descriptions for detected applications, if deducible, and transfer of
...
Once the start or stop of the application's traffic, matching one of those ADC
Rules, is detected, if PCRF has previously subscribed to the
...
interface and report the detected 77ultiple77g77 information via the
...
application's traffic to another controlled address) for a dynamic ADC rule to
the TDF. The Provisioning shall be performed using the ADC rule provisioning
procedure. The redirect instruction shall be encoded using a Redirect-
Information AVP within the ADC-Rule-Definition AVP of the dynamic ADC rule.
...
more usage monitoring instances applicable at ADC Rule level.
...
measurements simultaneously for a given application(s).
When the time based usage monitoring is supported, the PCRF may optionally
...
address and report the detected 83ultiple83g83 information via the
...
detected application's traffic in the Application-Detection-Information AVP in
the CCR command even if the application traffic is discarded due to
...
TDF session.
\- When the maximum allowed bit rate (QoS) applies to a detected application's
traffic, see clause 4b.5.1.3.
...
on 87ultiple simultaneous traffic flows for the same Ipv6 prefix (i.e. two or
...
NetLoc-Access-Support | 2824 | 5.3.111 | Unsigned32 | V | P |  | M | Y | All | NetLoc  
...
RAN-NAS-Release-Cause | 2819 | 5.3.106 | OctetString | V | P |  | M | Y | 3GPP-EPS | Both RAN-NAS-Cause  
...
same request with the new value. The RAT-Type AVP and AN-Trusted AVP shall
also be provided when applicable to the specific IP-CAN Type (e.g. 3GPP IP-CAN
...
functionality feature (IFOM) as described in subclause 5.4.1. Not applicable
for FBA.
APPLICATION_START (39)
This value shall be used in CCA and RAR commands by the PCRF to indicate that
the PCEF shall inform the PCRF when the start of the application's traffic for
the application, required for detection, has been identified, unless a request
to mute such a notification (Mute-Notification AVP) is part of the
...
that the PCEF shall inform the PCRF when the stop of the application's traffic
...
it is applicable to functionality introduced with the ABC feature as described
in subclause 5.4.1.
...
Network-Charging-Identifier-Value AVP and, when applicable, the related PCC
...
This value indicates that redirection is disabled for a detected application's
traffic.
...
*[ RAT-Type ]
*[ AVP ]
### 5.3.106 RAN-NAS-Release-Cause AVP (3GPP-EPS access type)
The RAN-NAS-Release-Cause AVP (AVP code 2819) is of type OctetString, and
indicates the RAN or NAS release cause code information. The AVP shall be
coded as per the RAN/NAS Cause in clause 8.103 of 3GPP TS 29.274 [22],
starting with Octet 5.
...
entering or leaving. It shall be encoded using full hexadecimal
representation.
...
AVP may be absent from the Gx commands. As defined in subclause 7.1.1 of 3GPP
...
the PCEF within Supported-Features AVP(s) with the 'M' bit cleared. Otherwise,
the CCR shall include the supported features within the Supported-Features
...
+-------------------+-------------------+-----+-------------------+ | Feature bit | Feature | M/O | Description | +-------------------+-------------------+-----+-------------------+ | 0 | UMCH | O | This feature | | | | | indicates support | | | | | for Usage | | | | | Monitoring | | | | | Congestion | | | | | Handling. If the | | | | | TDF supports this | | | | | feature, the | | | | | benaviour shall | | | | | be as specified | | | | | in clauses | | | | | 4b.5.7.7. | +-------------------+-------------------+-----+-------------------+ | 1 | Trusted-WLAN | O | This feature | | | | | indicates the | | | | | support for the | | | | | Trusted WLAN | | | | | access as defined | | | | | in | | | | | 3GPP | | | | | TS 23.402 [23]. | +-------------------+-------------------+-----+-------------------+ | 2 | TimeBasedUM | O | This feature | | | | | indicates support | | | | | for Time based | | | | | Usage Monitoring | | | | | Control. If the | | | | | TDF supports this | | | | | feature, the | | | | | benaviour shall | | | | | be as specified | | | | | in corresponding | | | | | clauses in this | | | | | specification. | +-------------------+-------------------+-----+-------------------+ | 3 | P | O | This feature | | | endingTransaction | | indicates support | | | | | for the race | | | | | condition | | | | | handling as | | | | | defined in | | | | | 3GPP | | | | | TS 29.213 [8]. | +-------------------+-------------------+-----+-------------------+ | 4 | ABC | O | This feature | | | | | indicates support | | | | | for Application | | | | | Based Charging. | +-------------------+-------------------+-----+-------------------+ | 5 | CNO-ULI | O | This feature | | | | | indicates support | | | | | for Presence | | | | | Reporting Area | | | | | Information | | | | | Reporting. If the | | | | | TDF supports this | | | | | feature, the PCRF | | | | | shall behave as | | | | | described in | | | | | Annex B.3.16. | +-------------------+-------------------+-----+-------------------+ | NOTE: | | | | | | | | | | **Feature bit:** | | | | | The order number | | | | | of the bit within | | | | | the Feature-List | | | | | AVP where the | | | | | least significant | | | | | bit is assigned | | | | | number \"0\". | | | | | | | | | | **Feature:** A | | | | | short name that | | | | | can be used to | | | | | refer to the bit | | | | | and to the | | | | | feature, e.g. | | | | | \"EPS\". | | | | | | | | | | **M/O:** Defines | | | | | if the | | | | | implementation of | | | | | the feature is | | | | | mandatory (\"M\") | | | | | or optional | | | | | (\"O\") in this | | | | | 3GPP Release. | | | | | | | | | | **Description:** | | | | | A clear textual | | | | | description of | | | | | the feature. | | | | +-------------------+-------------------+-----+-------------------+
...
The TSA command, indicated by the Command-Code field set to 8388637and the 'R'
bit cleared in the Command Flags field, is sent by the TDF to the PCRF in
response to the TS-Request command.
Message Format:
\ ::= \
\
{ Vendor-Specific-Application-Id }
...
The PCEF shall wait for the corresponding CCA before replying to the IP-CAN
...
the IP-CAN session establishment procedure.
...
in clause 5.4 in the 3GPP 3GPP TS 29.213 [8] taking into account the default
bearer QoS change and any PCC/QoS Rule operation requested by the PCRF.
NOTE: The network configuration can ensure that at least one PCC/QoS Rule is
...
available, is included in the TWAN-Identifier AVP. If the NetLoc-Trusted-WLAN
is supported, the procedure described in clause 4.5.22 shall apply with the
exception of the user location information that, if available, is included in
the TWAN-Identifier AVP.
...
on 214ultiple simultaneous traffic flows for the same Ipv6 prefix (i.e. two or
more traffic flows from Ipv6 addresses of the same IP-CAN session) the TDF
could not be aware that those flows belong to the same IP-CAN session until a
response is received from the PCRF, containing the Ipv6 prefix. This leads to
using separate TDF sessions for the Ipv6 addresses for the same IP-CAN
...
AVPs shall maintain their 'M', 'P' and 'V' flag settings. Where RADIUS VSAs
are re-used, unless otherwise stated, they shall be translated to Diameter
AVPs as described in IETF RFC 4005 [12] with the exception that the 'M' flag
...
with the following exceptions:
o In the routed RG with NAT mode, the IP-CAN session is per RG in the PCEF/IP
...
O In the bridge RG mode and in routed RG mode without NAT there may be a
