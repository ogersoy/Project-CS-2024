...
> request to the P-CSCF‑vA1 according to TS 24.229 [4].
...
The P-CSCF-vA1 selects an IBCF (IBCF-vA1) to be the exit IBCF towards the home
IMS network hA of the user A and sends the INVITE request according to TS
...
IBCF-vA1 updates the SDP as described within TS 29.079 [5].
The IBCF-vA1 selects, based on local policy, to route the INVITE request via
IC network A and sends the INVITE request according to TS 24.229 [4] to the
IC-A1. The IBCF-vA1 determines the next hop IP address from the URI in the
...
updates the SDP as described within TS 29.079 [5].
The IC-A1 selects an entry point (IBCF-hA1) of the home IMS network hA and
sends the INVITE request according to TS 24.229 [4] to the IBCF-hA1.
> The IC-A1 may add a Route header field entry pointing to the entry point of
...
\"+g.3gpp.trf\" header field parameter in a Feature-Caps header field, the
IBCF-hA1 updates the SDP as described in TS 29.079 [5].
The IBCF-hA1 sends the INVITE request according to TS 24.229 [4] to the
S-CSCF-hA1.
In this example, the IBCF-hA1 does not add any \"transit-ioi\" header field
parameter in the P-Charging-Vector header field. However, the IBCF-hA1 adds
the \"transit-ioi\" header field parameter based on local policy, when the
...
> All other SIP headers are set due to procedures described within TS 24.229
> [4].
...
sends the 183 (Session Progress) response according to TS 24.229 [4] towards
the UE‑A via the S-CSCF‑hA1, AS‑hA1, IBCF‑hA1, IC‑A1, IBCF‑vA1, P-CSCF‑vA1
path as described within TS 24.229 [4].
...
fulfilled according to TS 24.229 [4].
...
response, to the IBCF**‑** hA2 according to TS 24.229 [4].
...
SDP behaviour for loopback including OMR is described within TS 29.079 [5].
NOTE: For clarity, the SIP 100 (Trying) messages are not shown in the
signalling flow.
...
shown. The rest is as described in TS 24.229 [4].
> **2\. INVITE request (P-CSCF-vA1 to IBCF-vA1) - see example in table
> 5.4.2-2.**
>
...
> home IMS network hA and sends the INVITE request according to TS 24.229 [4]
> to IBCF-vA1.
...
> sends the INVITE request according to TS 24.229 [4] to the IC-A1. The IBCF-
> vA1 determines the next hop IP address from the URI in the first entry of
> the Route header field either by configured information or using a DNS query
> according to IETF RFC 3263 [13]. If the subsequent interconnection network
...
> IC-A1 updates the SDP as described within TS 29.079 [5].
>
> The IC-A1 selects an entry IBCF-hA1 of the home IMS network hA and sends the
> INVITE request according to TS 24.229 [4] to the IBCF-hA1.
>
> The IC-A1 may add a Route header field entry pointing to the entry point of
...
> When the IBCF-hA1 receives the initial INVITE request containing the
> \"+g.3gpp.trf\" header field parameter in a Feature-Caps header field, the
> IBCF-hA1 updates the SDP as described within TS 29.079 [5].
>
> In this example, the IBCF-hA1 does not add any \"transit-ioi\" header field
> parameter in the P-Charging-Vector header field. However, the IBCF-hA1 adds
> the \"transit-ioi\" header field parameter based on local policy, when the
...
> The INVITE request is sent according to TS 24.229 [4] to the IBCF-hA1.
...
> the IBCF-hA1 updates the SDP as defined within TS 29.079 [5].
>
> The IBCF-hA1 selects to route the INVITE request via an IC network A and
> sends the INVITE request according to TS 24.229 [4] to the IC-A1. From
...
> is possible. The IC-A1 updates the SDP according to TS 29.079 [5].
>
> The IC-A1 selects an entry IBCF in the visited network and sends the INVITE
> request according to TS 24.229 [4] to the IBCF-vA1. Seen from principle
...
> updates the SDP according to TS 29.079 [5].
>
> The IBCF-vA1 sends the INVITE request according to TS 24.229 [4] to the TRF-
> vA1.
>
> The content of the Record-Route header field and the Via header field will
> be based on local policy and updated accordingly.
...
request according to TS 24.229 [4] to the IBCF-vA1 (which could be also
...
> **SDP MIME body: The IBCF-vA1 adds the** a=visited-realm according to TS
> 29.079 [5] indicating that an optimal media route has occurred and replaces
> the received IP address with its own IP address.
>
...
The IBCF-vA1 sends the 183 (Session Progress) response according to TS 24.229
[4] towards the S**‑** CSCF**‑** hA via the IC-A1 and IBCF-hA1. Neither the
IC-A1 nor IBCF-hA1 updates the SDP.
...
> IC** ‑**A1) handling.**
The procedures of TS 24.229 [4] apply. The main changes are:
...
The UPDATE request and the 200 (OK) response to the UPDATE request are sent
according to the rules described in TS 29.079 [5].
...
> the I-CSCF‑hA1 according to TS 24.229 [4].
...
> URI as described in TS 24.229 [4].
>
> **Via:** The I-CSCF‑hA1 adds a Via header field.
>
...
The S-CSCF-hA1 sends the INVITE request according to TS 24.229 [4] to the
IBCF-hA1.
...
of the visited IMS network vA and sends the INVITE request according to TS
24.229 [4] to the IBCF-vA1.
...
>
> The P-CSCF‑vA1 applies trust domain according to TS 24.229 [4] clause 4.4.
> This will result in that some header fields will be removed. In our example
> the P-Charging-Vector header field and the P**‑** Access**‑** Network**‑**
> Info header field are removed from the outgoing INVITE request.
>
> The P-CSCF‑vA1 sends the INVITE request according to TS 24.229 [4] to the
> UE-A.
...
When the IBCF‑hA2 receives the 183 (Session Progress) response, the IBCF‑hA2
sends the 183 (Session Progress) response according to TS 24.229 [4] towards
the remote UE as described within clause 5.3 or clause 5.4 of this document
and regarding the rules defined in TS 24.229 [4].
...
The PRACK request and the 200 (OK) response to the PRACK request are sent
without any SDP offer/SDP answer between the UE-A and the IBCF-hA2 according
to TS 24.229 [4]. The messages are sent via all intermediate nodes with the
exception of I-CSCF-hA1.
...
the UE-A according to TS 24.229 [4].
...
TS 24.628 [22] describes how to include announcements into the path for IMS.
...
respectively.
NOTE 1: If precondition mechanism is used, the procedures according to TS
24.229 [4] apply.
...
path as shown in the example in TS 29.079 [5] clause A.6.
...
P-Charging-Vector header field.
The TS 24.237 [20] defines two types of MSC servers. One MSC server enhanced
for ICS and another MSC server enhanced for SIP supporting PS to CS SRVCC. The
message flows are the same if PS to CS SRVCC occurs so in the following
subclauses both variants of MSC servers are referred to as \"MSC server\". The
only difference between the two types is that when the MSC server is enhanced
for ICS, the MSC server will register the user following the procedures in
clause 5.2 when the PS to CS SRVCC access transfer procedure is completed.
...
the MESSAGE request according to TS 24.229 [4] to the IC-A1. The IBCF-hA1
...
and sends the MESSAGE request according to TS 24.229 [4] to the IBCF-vA1.
...
(OK) response according to TS 24.229 [4]. No SRVCC specific parameters are
...
currently specified in TS 24.237 [20].
...
The MSC server-vA1 sends the ACK to the ATCF-vA1 according to TS 24.229 [4].
...
SDP as described within TS 29.079 [5].
The IBCF-vA1 selects, based on local policy, to route the INVITE request due
to ATU-STI via interconnect network A and sends the request according to TS
24.229 [4] to the IC-A1. The IBCF-vA1 determines the next hop IP address from
the SIP request URI, either by configured information or using a DNS query
according to IETF RFC 3263 [13]. If the subsequent interconnection network is
SIP-aware, the determined IP address will belong to the SIP proxy IC-A1.
...
the IC-A1 updates the SDP as described within TS 29.079 [5].
The IC-A1 selects an entry point (IBCF-hA1) of the home IMS network hA and
sends the INVITE request according to TS 24.229 [4] to the IBCF-hA1.
...
updates the SDP as described in TS 29.079 [5].
...
request due to ATU-STI to the selected I-CSCF according to TS 24.229 [4].
In this example, the IBCF-hA1 does not add any \"transit-ioi\" header field
parameter in the P-Charging-Vector header field. However, the IBCF-hA1 adds
the \"transit-ioi\" header field parameter based on local policy, when the
\"transit-ioi\" identifying the preceding interconnection network is not
...
5.11.3-11.**
The S-CSCF-hA1 forwards the re-INVITE request according to procedures in TS
24.229 [4].
...
hA1 according to TS 24.229 [4]. The 200 (OK) response does not contain any
SRVCC specific information**.**
**14-15. ACK request (SCC AS-hA1 to IBCF-hA2 via S-CSCF-hA1).**
The SCC AS-hA1 sends the ACK to the IBCF-hA2 according to TS 24.229 [4]. The
ACK request does not contain any SRVCC specific information.
...
S-CSCF‑hA1).**
The ATCF-vA1 sends the ACK to the SCC AS-hA1 according to TS 24.229 [4]. The
ACK request does not contain any SRVCC specific information.
...
conclusion is that it differs so the SCC AS-hA1 needs to send a re-INVITE
request towards the remote UE.
The SCC AS-hA1 handles OMR attributes according to TS 24.237 [20] clause
6A.4.4.
