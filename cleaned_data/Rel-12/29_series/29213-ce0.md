...
TS 29.212 [9], TS 29.214 [10], TS 29.215 [22], and TS 29.219 [28] shall be
...
\- The steps numbered as \"number+letter\" (e.g. \"3a\") will be executed, for
the roaming case, instead of steps numbered as \"number\" (e.g. \"3\"), as
indicated in the explanatory text below the signalling flows.
...
restricted to Emergency Services is described in TS 29.212 [9].
\- Subscription-related information is not relevant for Emergency Sessions;
therefore Sp reference point does not apply.
\- With the UDC-based architecture as defined in TS 23.203 [2] and TS 23.335
[25], SPR, whenever mentioned in the present specification, refers to UDR. The
Ud interface as defined in TS 29.335 [26] is the interface between the PCRF
and the UDR.
...
Restoration procedure as defined in TS 29.212 [9].
NOTE: Only the interaction with OCS for spending limits 13vailabil over Sy
...
{width="6.6875in" height="7.823611111111111in"}
...
as defined in clause 4.5.1 of TS 29.212 [9] and may send charging
...
prevent the use thereof, the PCE F shall indicate th at the support for
...
cannot make the PCC rule decisions, as described in TS 29.212 [9].
...
active PCC rule for the IP-CAN session as specified in TS 23.203 [2].
...
the access network information. If RAN-NAS-Cause feature is supported, the
PCEF informs the H-PCRF about the access network information and the failure
...
access network information and/or RAN/NAS release cause(s) if available.
...
1 through step 7: as specified in Figure 4.2.1.1.1: UE Initiated IP-CAN
...
If the AF resides in the VPLMN, the V‑PCRF proxies AF session signalling over
S9 between the V-AF and the H‑PCRF.
\- In order to perform PCEF initiated IP-CAN Session Termination Procedures,
step 1 through step 8: as specified in Figure 4.2.2.1.1: PCEF Initiated IP-CAN
Session Termination -- AF Located in the HPLMN are executed.
...
8\. The H-PCRF responds by sending an STA to the V-PCRF.
9\. The V-PCRF proxies the STA to the V-AF.
10\. Step 13 through step 16: as specified in Figure 4.2.2.1.1: PCEF Initiated
IP-CAN Session Termination -- AF Located in the HPLMN are executed, as needed.
...
If the AF resides in the VPLMN, the V‑PCRF proxies AF session signalling over
S9 between the V-AF and the H‑PCRF.
\- In order to perform PCRF initiated IP-CAN Session Termination Procedures,
step 1 through step 11: asspecified in Figure 4.2.3.1.1: PCRF Initiated IP-CAN
Session Termination -- AF Located in the HPLMN are executed.
...
\"UPDATE_REQUEST\". For an IP-CAN Session modification where an existing IP-
CAN bearer is modified, the PCEF supplies the specific event that caused the
IP-CAN Session modification within the Event-Trigger AVP and the PCC rule
name(s) and their status within the Charging-Rule-Report AVP. For an IP-CAN
Session modification where an existing IP-CAN bearer is terminated, the PCEF
supplies the affected PCC rule name(s), their status set to inactive, the rule
failure code and, if RAN-NAS-Cause feature is supported and if available, the
RAN/NAS cause(s) within the Charging-Rule-Report AVP and the access network
information. In the case where the UE initiates a resource modification
request procedure, the PCEF includes the Packet-Filter-Information AVP,
Packet-Filter-Operation AVP and QoS-Information AVP, if applicable. In the
case of PCEF supporting Application Detection and Control feature, when the
start or stop of the application\'s traffic, identified by TDF-Application-
Identifier, is detected, if PCRF has previously subscribed to the
APPLICATION_START/APPLICATION_STOP Event-Triggers, the PCEF shall report the
information regarding the detected application\'s traffic in the Application-
Detection-Information AVP in the CCR command.
...
(e.g. for IMS refer to TS 23.228 [36], replies with a Diameter RAA and may
...
If RAN-NAS-Cause feature is supported and RAN/NAS release cause(s) and/or
access network information were received in step 3, the H-PCRF sends this
information to the AF in step 11.
...
Result-Code AVP set to the value PCC_RULE_EVENT, identify the failed PCC rules
as specified in TS 29.215 [22], and additionally may provide the acceptable
QoS Information for the service.
...
a Care-of Address in MIP registrations (see TS 23.402 [21], clause 6.3).\ For
case 2b, as defined in clause 4.0, the BBERF detects that the UE requests an
IP-CAN session to be established (see TS 23.402 [21], clauses 4.5.2 and 5.6.1)
or, at BBERF relocation, to be resumed with a certain APN (see TS 23.402 [21],
clauses 5.7.1 and 5.7.2) or the UE requests a pre-registration with this BBERF
...
procedure as defined in clause 4a.5.16 of TS 29.212 [9] to request the access
network information.
...
defined in clause 4a.5.7 of TS 29.212 [9].
...
4a.5.6 of TS 29.212 [9] for the non-roaming case. Based on the information
received the H-PCRF reclassifies primary/non-primary BBERFs according to the
procedures defined in clause 4a.5.7 of TS 29.212 [9].
...
mode selected as defined in clause 4a.5.9 of TS 29.212 [9]. The H-PCRF
...
routing rules as defined in clause 4a.5.7 of TS 29.212 [9]
...
TS 29.212 [9]. The H-PCRF includes the AN-GW-Address AVP identifying the BBERF
involved in the exchange of the IP flows described by the received IP flow
mobility routing rules
7b. The V-PCRF enforces visited operator policies regarding QoS authorization
requested by the H-PCRF as indicated by the roaming agreements.
7c. If the V-PCRF denies an authorization, it informs the H-PCRF and may
provide the acceptable QoS Information for the service by including in the RAA
...
rules as defined in clause 4a.5.7 of TS 29.212 [9]
...
according to clause 4a.5.6 of TS 29.212 [9]. Otherwise based on the
information received the V-PCRF identifies multiple BBERF sessions for a
particular IP-CAN session.
5\. Based on the information received the V-PCRF reclassifies primary/non-
primary BBERFs according to the procedures defined in clause 4a.5.7 of TS
29.212 [9].
...
this case, the H-PCRF receives an IP flow mobility event by the PCEF and
derives QoS rules based on the IP flow mobility routing rules.
Figure 4.5.2.3.1: PCEF-initiated IP-CAN session modification -- IP flow
mobility.
1\. The PCEF receives a message or indication that a handover occurred.
2\. The PCEF initiates an IP-CAN Session Modification procedure by sending a
CCR using the CC-Request-Type AVP set to the value UPDATE_REQUEST to the
...
the IP flow mobility routing rule as defined in clause 4a.5.7 of TS 29.212 [9]
...
routed through multiple BBERF(s) as described in clause 4a.5.7 of TS 29.212
[9].
> If either the AN_GW_CHANGE or the IP-CAN_CHANGE event is subscribed by
> H-PCRF and this event trigger is received steps 6\~9 are executed. Otherwise
> steps 6\~9 are skipped.
...
```{=html}
``` 4\. The PCRF stores the information and acknowledges the session
establishment by sending a CCA. The PCRF may include the Ipv6 prefix within
the Framed-Ipv6-Prefix AVP if the established TDF session is Ipv6 address
related.
...
on 74vailabi simultaneous traffic flows for the same Ipv6 prefix (i.e. two or
...
TDF reports the failure to the PCRF as defined in sub clause 4b.5.5 of TS
29.212 [9].
...
2\. The H-PCRF sends a Diameter SLR command including the Subscription-Id AVPs
(e.g. IMSI) and optionally the list of policy counter identifier(s) within
Policy-Counter-Identifier(s) AVPs. The request also includes the SL-Request-
Type AVP which set to the value INTERMEDIATE_REQUEST (1).
...
6.2.2.2 and the IP-CAN specific annexes of TS 23.203 [2]).
...
the default EPS bearer. If there are active non-MPS services, the QCI/ARP of
the PCC/QoS rules related to the non-MPS services shall also be changed.
...
the MPS session according to clauses 4.5.19.1.3 and 4a.5.14.1.3 in TS 29.212
[9] for Gx/Gxx respectively. If the Rx authorization indicates Group
Communication Service prioritization as described in TS 23.468 [34], then the
PCRF shall allow the prioritization of the Group Communication session
according to clause 4.5.19 in TS 29.212 [9].
...
CAN bearer. The BBF should not bind rules with the PS to CS session continuity
indicator to the same bearer as the rules without the PS to CS session
...
UE, as described in TS 29.212 [9].
...
packet flows in uplink direction as described in clause 15.3.3.4 of TS 23.060
[3]) and provides it to the UE.
NOTE 6: For GPRS and EPS, the state of TFT packet filters, as defined in TS
23.060 [3], for an IP-CAN session requires that there is at most one bearer
...
If MPD (TS 26.247 [30]) is used, the AF may apply the mapping described in
Annex X in TS 26.247 [30]. For IMS, the mapping rules in Clause 6.2 shall be
used at the P-CSCF. The AF passes service information to the PCRF over the Rx
interface. Clause 6.2 specifies the QoS parameter mapping functions at the AF
applicable for all IMS P-CSCFs regardless of the access technology.
...
the recommendations for conversational (TS 26.236 [7]) or streaming
applications (TS 26.234 [6]) should also be taken into account when the UE
derives the IP and UMTS QoS parameters. If SDP is used as SDI, e.g. for IMS,
the UE should apply clause 6.5.1.and should also apply mapping rules for the
authorised QoS parameters in clause 6.5.2 to derive the maximum values for the
different requested bit rates and traffic classes. In case the UE multiplexes
several IP flows onto the same PDP Context, it has to combine their IP and
UMTS QoS parameters. If an IP BS manager is present, the Translation/Mapping
function maps the IP QoS parameters to the corresponding UMTS QoS parameters.
...
1\. The AF can map from SDI within the AF session signalling to service
information passed to the PCRF over the Rx interface (see clause 6.2 if SDP is
used as SDI).
2\. The PCRF shall map from the service information received over the Rx
interface to the Authorized IP QoS parameters that shall be passed to the
PCEF/BBERF via the Gx/Gxx interface. The mapping is performed for each IP
...
PCRF (see TS 29.212 [9] for details). The PCRF may supply Authorized IP QoS
Parameters per PDP context together with the PCC rules. The GGSN maps the
...
PCRF (see TS 29.212 [9] for details). The PCRF may supply Authorized IP QoS
Parameters applicable for the provided PCC rules. The P-GW calculates the
...
QoS Parameters. See TS 26.236 [7] for Conversational Codec Applications and TS
26.234 [6] for Streaming Codec Applications.
...
Diameter sessions established over the Gx, S9, Gxx, Rx and for unsolicited
application reporting, the Sd reference points for a certain IP-CAN session
reach the same PCRF when multiple and separately addressable PCRFs have been
...
and/or Ipv6 prefix, the APN(if available), the PCEF identity (if
available),and the selected PCRF address for a certain IP-CAN Session.
...
received in the message from the BBERF/PCEF/AF/TDF/V-PCRF with the
...
it supports (e.g., Gx, Gxx, Rx, S9 and for unsolicited application reporting,
Sd) by including the value of the application identifier in the Auth-
Application-Id AVP and the value of the 3GPP (10415) in the Vendor-Id AVP of
the Vendor-Specific-Application-Id AVP contained in the Capabilities‑Exchange-
...
shall be aware of Gx and Gxx Diameter termination requests as defined in TS
29.212 [9] in order to detect whether release of DRA bindings is required.
...
Identity in the IP-Domain-Id AVP (TS 23.203 [2]).
...
the UE\'s Ipv4 address and/or Ipv6 prefix (TS 23.203 [2]).
...
establishment of an IP-CAN session as defined in clauses 4.5.1, 4a.5.1 of TS
29.212 [9]) is sent by the Client and received by a DRA (proxy).
...
5\. PCRF-1 returns a Diameter Answer as defined in clauses 4.5, 4a.5 of TS
29.212 [9] to the DRA (proxy).
...
modification of an IP-CAN session) as defined in clauses 4.5.1, 4a.5.1 of TS
29.212 [9] or clause 4.4 of TS 29.214 [10]) is sent by the Client and received
by the DRA (proxy).
...
5\. PCRF-1 returns a Diameter Answer as defined in clauses 4.5, 4a.5 of TS
29.212 [9] or clause 4.4 of TS 29.214 [10]) to the DRA (proxy).
...
5\. Clients returns a Diameter Answer as defined in clauses 4.5, 4a.5 of TS
29.212 [9] or clause 4.4 of TS 29.214 [10]) to the DRA (proxy).
...
TS 29.212 [9]) is sent by the Client to the DRA (proxy). The message uses the
same Session-Id AVP value of the active Diameter session established between
the Client and PCRF-1.
...
Answer, (e.g., as defined in clauses 4.5, 4a.5 of TS 29.212 [9]) to DRA
(proxy).
...
4a.5.1 of TS 29.212 [9]) with user information (e.g., UE-NAI) is sent by the
Client and received by the DRA (redirect).
...
target PCRF.
6\. PCRF-1 returns a Diameter Answer, as defined in clauses 4.5, 4a.5 of TS
29.212 [9], to the Client.
...
update the DRA binding information only if the UE\'s address(es) is updated
and the DRA (redirect) is maintaining PCRF routing information per IP-CAN
session. For visited access case, the V-PCRF shall send the Diameter session
...
4a.5.3 (Gxx) of TS 29.212 [9]) is sent by the Client to the DRA (redirect).
3\. A Diameter Termination Request (e.g., as defined in clauses 4.5.7 (Gx) and
4a.5.3 (Gxx) of TS 29.212 [9]) is sent by the Client to PRCF-1. The message
uses the same Session-Id AVP value of the active Diameter session established
...
(e.g., as defined in clauses 4.5.7 (Gx) and 4a.5.3 (Gxx) of TS 29.212 [9]) to
the Client.
...
In this clause, the terms \"client\" and \"server\" are relative to the
session context. As an example, for the Gx interface, the client is the PCEF
and the server is the PCRF. The term \"node\" can refer to either a client or
a server. The term \"transaction\" refers to a Diameter request and its
associated answer. The term \"ongoing transaction\" refers to a transaction
that has an outstanding answer.
...
establishment as defined in TS 29.212 [9] for Gx, Gxx and Sd and TS 29.215
[22] for S9.
...
result code of DIAMETER_PENDING_TRANSACTION as defined in TS 29.212 [9].
...
(CCR with a CC-Request-Type set to TERMINATION_REQUEST) or a request for
session release (e.g. RAR with Session-Release-Cause). In both cases, the
...
sent over Rx as defined in TS 29.214 [10].
...
service with appropriate SIP signalling, or to allow the P-CSCF to request
network provided location information for inclusion in the SDP offer. This is
described in figure B.2.1.2.
...
access network information for inclusion in the SDP offer. The P-CSCF forwards
the SDP offer in SIP signalling.
...
procedure, as defined in the TS 23.380 [35]. The AAR command contains a Rx-
Request-Type AVP with value set to PCSCF_RESTORATION and can contain the IP
...
> For the non-roaming case and the home routed case, the H-PCRF receives the
> AAR command, i.e. step 1 is executed.
...
subsequent P-CSCF Restoration procedure as specified in TS 23.380 [35].
...
> command from the H-PCRF, the PCEF acknowledges this message by sending an
> RAA command to the H-PCRF, i.e. step 4 is executed.
...
The IMS calls out procedures for NAT traversal for media and 154vailabili
...
Annex F of TS 24.229 [5]. The hosted NAT will modify the source IP address and
source port of uplink IP packets, and the destination IP address and port of
downlink IP packets. The IP addess and port information provided by the UE are
thus not appropriate to configure PCC rules.
...
procedures in Annex F of TS 24.229 [5]) and that ICE is not used (using
...
Contexts as specified in TS 23.060 [3].
...
Evolved ARP to Rel-99 ARP as specified in clause B.3.3.3 of TS 29.212 [9].
...
in clause 4.1.5 of TS 23.402 [21].
...
> scenario, the BBERF provides the data as described in TS 29.212 [9], clause
> E.5.2.
For case 1, the PCEF provides the data as described in TS 29.212 [9], clause
E.5.1.
...
point.
  1. For GTP/PMIP-based S2b of the WLAN scenario (non-roaming case) the PCRF sends a TER command to BPCF to trigger an S9a session establishment procedure triggered by the Gateway Control session establishment or Indication of IP-CAN session establishment in step 1.
...
described in TS 29.215 [22], clause A.6.1.1.1 or A.6.3.1.0.
...
BPCF.
> The V-PCRF provides the data as described in TS 29.215 [22], clause A.5.1.1.
  1. The BPCF acknowledges to the PCRF by sending the TEA /RAAcommand.
...
> 3c. The BPCF acknowledges to the V-PCRF by sending the TEA/RAA command.
  1. For the non-roaming case, triggered by step 2, the BPCF initiates an S9a session establishment with the PCRF by sending a CCR to the PCRF with the CC-Request-Type AVP set to the value INITIAL_REQUEST. The BPCF provides the data as described in TS 29.215 [22], clause A.5.1.2.
...
the value INITIAL_REQUEST. The BPCF provides the data as described in TS
...
the received information.
  1. The H-PCRF stores the information received in the CCR.
  2. For the non-roaming case, the H-PCRF acknowledges the S9a session establishment by sending a CCA to the BPCF.
...
the BPCF.
  1. Step 7 through step 15: as specified in Figure 4.1.1: IP-CAN session establishment are executed. Step 13 is only applicable to case 2b of H(e)NB scenario.
...
INITIAL_REQUEST. The BPCF shall provide the data as described in TS 29.215
[22], clause A.5.1.2.1.
...
> Subsession-Operation AVP set to the value ESTABLISHMENT.
  1. The H-PCRF stores the information received in the CCR.
  2. Perform step 5 through 11: as specified in Figure 4.1.1: IP-CAN session establishment. Additionally, an indication on whether policy control for NSWO traffic should be performed for the UE may be retrieved from the SPR. The H-PCRF enables policy control for NSWO traffic for that UE based on operator policies and user profile information that may depend on e.g. NSWO-APN being used by the UE.
  3. For the non-roaming case, the H-PCRF provisions the PCC Rules to the BPCF using CCA if policy control is enabled.
...
> released PDN connection.
  1. For the non-roaming case, the BPCF sends a RAA to acknowledge the RAR.
...
3c. The BPCF sends a RAA to the V-PCRF.
  1. For the non-roaming case, the BPCF sends a CCR to the H-PCRF to indicate the S9a session termination if the H-PCRF requests that the BPCF terminates the S9a session in step 2. The BPCF requests the termination of the S9a session using the CC-Request-Type set to the value TERMINATION_REQUEST.
...
to the value TERMINATION_REQUEST.
  1. For the non-roaming case, the H-PCRF sends a CCA to acknowledge the CCR.
...
> 5c. The V-PCRF sends a CCA to acknowledge the CCR.
  1. Perform the step 3 through step 7: as defined in Figure 4.2.1.1: UE-initiated IP-CAN session termination-AF located in HPLMN,
...
TERMINATION to request the termination of the conresponding S9 subsession.
  1. The H-PCRF identifies the AF sessions that are bound to IP flows of the removed IP-CAN Session.
  2. For the non-roaming case, the H-PCRF acknowledges the S9a* session termination by sending a CCA to the BPCF.
...
theBPCF.
  1. If there is an active TDF session between TDF and H-PCRF, the TDF session termination is initiated as defined in clause 4.6.2.
  2. Perform step 8 through step 12: as specified in Figure 4.2.1.1: UE-Initiated IP-CAN Session Termination -- AF located in the HPLMN
...
> 3 The H-PCRF sends a Diameter CCA to the BPCF to acknowledge the CCR
> command.
...
port number (if available) to the BPCF.
  1. The BPCF shall perform QoS validation and translates the QoS information as received (i.e. QCI, MBR, GBR and ARP) into access specific QoS parameters applicable in the Fixed Broadband Access.
...
> 2 The H-PCRF sends a Diameter RAR to request that the BPCF installs,
> modifies or removes QoS Rules (e.g. SDF template, QCI, ARP, MBR and GBR).
When the UE is in roaming case, steps 2a \~ 2c are executed instead of step 2:
2a. For Visited Access scenario, the H-PCRF may provision the PCC rule(s) to
the V-PCRF. For Home Routed scenario the H-PCRF may provision the QoS rule(s)
...
PCRF address for a certain IP-CAN Session or a certain user.
...
the selected PCRF address for a certain UE.
...
the selected PCRF address per UE-NAI or per UE-NAI and APN.
...
defined in TS 29.215 [22] in order to release the DRA binding information.
### E.6.3.4 Proxy DRA
...
triggered by the (V-) PCRF, the BPCF may use the (V-) PCRF address provided
...
IETF RFC 3588 [14], and include the PCRF address (Diameter Identity) in the
Redirect-Host AVP in the Diameter reply sent to the BPCF .
If proxy agent is used for DRA, the DRA should use the proxy procedure as
specified in IETF RFC 3588 [14]. For PA2 solution (described in clause 7.1)
...
as described in IETF RFC3588 [14].
...
in IETF RFC 3588 [14]. The target V-PCRF identity is included in the Redirect-
Host AVP.
...
6\. The V-PCRF-1 returns a TEA 199vailab.
...
of S9 session establishment procedure according toclause E.6.4.3.1), this
...
defined in Broadband Forum TR-146 [31].
...
> Subscriber IP session in Fixed Broadband Access network can be the Access
> Line Identifier (Physical-Access-ID AVP and Logical-Access-ID AVP) or the
> username (Subscription-Id AVP), for example when the Subscriber IP session
> is a PPP Session.
...
establishment as described in TS 29.212 [9] Annex G.5.2.
...
modification as described in TS 29.212 [9] Annex G.5.4.2.
...
shall provide parameters to the PCEF in the IP Edge at IP-CAN session
modification as described in TS 29.212 [9] Annex G.5.4.1.
# F.5 PCRF Addressing
### F.5.1 General
...
\- Gxx reference point is not used.
\- The Subscriber Identifier specified in TS 23.203 [2] clause S.5.1.2 is used
as user identity.
...
TS 22.153 [37] and emergency have the highest priority. If required by the
...
QoS change and any PCC Rule/QoS Rule operation requested by the PCRF.
#