...
When the MSC Server receives a CC Recall as a REFER request as described in TS
...
[42] or TS 24.611 [31].
Erasure of the CB service is not specified for NAS signalling in TS 24.088
[30], therefore interworking procedures for service erasure are not
applicable.
##### 5.7.4.5.2 Activation/deactivation
When the MSC Server receives a REGISTER message with an ActivateSS or
DeactivateSS invoke operation for any call barring supplementary service code
as described in TS 24.088 [30], the MSC Server shall send a RELEASE COMPLETE
message with an implementation-specific error code.
NOTE: This is because the password registration procedures required to support
CB activation and deactivation using NAS signalling do not have equivalencies
in TS 24.623 [42] or TS 24.611 [31].
...
applied.
When the MSC Server receives a REGISTER message with an InterrogateSS invoke
operation for the BAOC or BAIC supplementary service code as described in TS
24.082 [24], the MSC Server shall generate and send an HTTP GET request to
fetch the instance of the Communication Barring document as specified in TS
24.623 [42].
When a response to the HTTP GET request is received, the MSC Server shall
apply the following interworking:
\- If a 200 OK response is received which includes a Communication Barring
document as defined in TS 24.611 [31], the MSC Server shall send a RELEASE
COMPLETE message as follows:
...
described in TS 24.082 [24], the MSC Server shall send a RELEASE COMPLETE
message with an implementation-specific error code as these supplementary
service codes cannot be mapped to the CB rules defined in TS 24.611 [31].
...
include Communication Diversion document as described in TS 24.604 [23] in the
HTTP PUT request as follows:
\- The MSC Server shall set the active attribute to \"true\";
\- If the supplementary service code is CFU as described in TS 24.082 [24],
the MSC Server shall include a forwarding rule where the condition element is
empty or no condition element is included, as defined in TS 24.604 [23];;
\- If the supplementary service code is CFB as described in TS 24.082 [24],
the MSC Server shall include a forwarding rule for the busy condition defined
in TS 24.604 [23];
\- If the supplementary service code is CFNRy as described in TS 24.082 [24],
the MSC Server shall include a forwarding rule for the no-answer condition
defined in TS 24.604 [23];
\- If the supplementary service code is CFNRc as described in TS 24.082 [24],
the MSC Server shall include a forwarding rule for the not-reachable condition
defined in TS 24.604 [23];
...
NOTE: The Communication Diversion document described in TS 24.604 [23] defines
XML elements which have no functional equivalent in the service configuration
signalling defined in TS 24.082 [24]. The inclusion of these elements and the
values assigned to them is subject to operator policy.
...
described in TS 24.604 [23] in an HTTP PUT request, modified as follows:
...
forwarding rule for the busy condition defined in TS 24.604 [23], if present;
\- If the supplementary service code is CFNRy, the MSC Server shall remove the
forwarding rule for the no-answer condition defined in TS 24.604 [23], if
present;
\- If the supplementary service code is CFNRc, the MSC Server shall remove the
forwarding rule for the not-reachable condition defined in TS 24.604 [23], if
present;
...
\- If a 200 OK is received, the MSC Server shall send a RELEASE COMPLETE
message as follows:
\- If an SS-Status parameter is required as specified in TS 24.082 [24] clause
1.3.1, the MSC Server shall indicate an SS-Status of not active;
\- If a non 200 OK response is received or if a 200 OK response is received
which does not include a Communication Diversion document, the MSC Server
shall send a RELEASE COMPLETE message with an implementation-specific error
code.
...
service code as described in TS 24.082 [24], the MSC Server shall include a
Communication Diversion document as described in TS 24.604 [23] in the HTTP
...
\"active\" attribute to \"false\";
\- If the supplementary service code is CFU as described in TS 24.082 [24],
the MSC Server shall include a forwarding rule with an empty condition element
or no condition element included;
\- If the supplementary service code is CFB as described in TS 24.082 [24],
the MSC Server shall include a forwarding rule for the busy condition defined
in TS 24.604 [23];
\- If the supplementary service code is CFNRy as described in TS 24.082 [24],
...
described in TS 24.082 [24], the MSC Server shall generate and send an HTTP
GET request to fetch the instance of the Communication Diversion document as
specified in TS 24.604 [23].
When a response to the HTTP GET request is received, the MSC Server shall
apply the following interworking:
\- If a 200 OK is received which includes a Communication Diversion document
as defined in TS 24.604 [23], the MSC Server shall send a RELEASE COMPLETE
message as follows:
...
Activation in the IMS network as specified in TS 24.642 [52] is performed
...
requests using the methods in TS 24.642 [52].
...
When the MSC Server receives a REGISTER message with an InterrogateSS invoke
operation for the CCBS supplementary service code as described in TS 24.093
[53], the MSC Server shall generate and send an HTTP GET request to fetch the
instance of the Communication Completion document as specified in TS 24.642
[52].
When an HTTP 200 OK response is received which includes a Communication
Completion document as defined in TS 24.642 [52], the MSC Server shall:
...
in TS 24.642 [52];\ \ when a response to the HTTP GET request is received the
MSC server shall:
a) if an HTTP 200 OK is received which includes a Communication Completion
Request Records document as defined in TS 24.642 [52] where there is one or
more instances of the \"cc-entry\" element, assign to each of these elements a
CCBS Index and send a RELEASE COMPLETE message indicating an SS-Status of
provisioned and include the list of outstanding CCBS requests; and
...
The MSC Server applies the procedures specified in TS 23.009 [37] for Intra-
MSC handover/relocation in the CS domain. After the handover/relocation is
completed, the MSC Server shall process the interworking between the NAS
signalling with the SIP signalling as specified in sub-clauses 5.3 through
5.6.
### 5.8.2 Inter-MSC Handover/Relocation
The MSC Server applies the procedures specified in TS 23.009 [37] for Inter-
MSC handover/relocation in the CS domain. After the handover/relocation is
completed, the anchor MSC Server shall process the interworking between the
...
IuFP is defined is defined in TS 25.415 [16]. IuCS layer 2 and layer 3 are
defined in TS 25.414 [15]. The IuCS layer 1 is defined in TS 25.411 [17].
...
the IuCS and Mb reference points as specified in TS 29.414 [18] clause 7.4.
...
Layer 1 for the TDM-based A-interface is defined in TS 48.004 [19].
...
The IP-based A-interface user plane transport is defined in TS 48.103 [38].
...
Reserve RTP Connection Point procedure as defined in TS 23.205 [39] and in TS
29.232 [11], with additional information elements as specified in the present
subclause.
...
MboIP as defined in TS 23.205 [39] and in TS 29.232 [11], with additional
information elements as specified in the present subclause. The local IP
address and UDP port are used by the CS-MGW to receive user plane data.
...
Configure RTP Resources procedure as defined in TS 29.232 [11] to request the
CS-MGW to configure the bearer.
...
Connection Point procedure defined in in TS 23.205 [39] shall be applied with
additional information elements as defined in table 7.1.2.2.1.
> Table 7.1.2.2.1: Additions to Reserve RTP Connection Point Procedure
+-------------+-------------+-------------+-------------+-------------+ | > Procedure | > Initiated | > | > | Information | | | | Information | Information | element | | | | > element | > element | description | | | | > name | > required | | +-------------+-------------+-------------+-------------+-------------+ | Reserve RTP | (G | | | | | Connection | )MSC-Server | | | | | Point | | | | | +-------------+-------------+-------------+-------------+-------------+ | | | | | | +-------------+-------------+-------------+-------------+-------------+ | Reserve RTP | MGW | | | | | Connection | | | | | | Point | | | | | | | | | | | | Ack | | | | | +-------------+-------------+-------------+-------------+-------------+ | | | | | | +-------------+-------------+-------------+-------------+-------------+
...
The MSC Server and the CS-MGW shall act in accordance with clause 6.1.3 in TS
23.205 [39] and apply the coding in accordance with TS 29.232 [11].
#### 7.1.3.3 A interface over IP
The MSC Server and the CS-MGW shall act in accordance with clause 6.1.5 in TS
23.205 [39] and apply the coding in accordance with TS 29.232 [11].
#### 7.1.3.4 A interface over TDM
The MSC Server and the CS-MGW shall act in accordance with clause 6.1.1.4 in
TS 23.205 [39] and apply the coding in accordance with TS 29.232 [11].
...
case the MSC Server shall use the Send Tone procedure in accordance with TS
23.205 [39] and TS 29.232 [11].
...
the Stop Tone procedure as defined in TS 23.205 [39] and TS 29.232 [11].
...
\- use Change Through-Connection procedure as defined in TS 29.232 [11] during
any one of the Prepare Bearer and Reserve Circuit procedures as defined in TS
29.232 [11]; or
\- use Configure the RTP Connection Point procedure as defined in TS 29.232
[11] during Prepare IP bearer procedure as defined in TS 29.232 [11].
If the MSC Server wants to configure the CS-MGW so that the bearer will be
both-way through connected the MSC Server shall use Change Through-Connection
procedure as defined in TS 29.232 [11].
...
subclauses 9.2.3.4.1 to 9.2.3.4.4 in TS 29.163 [46].
NOTE: For the references to TS 29.163 [46], the O-MGCF and I-MGCF in TS 29.163
[46] is to be understood as MSC Server, the IM-MGW is to be understood as CS-
MGW, and the Configure IMS Resources procedure is to be understood as
Configure RTP Resources procedure.
...
CS-MGW the procedure in accordance with clause 6.1.1.12 of TS 23.231 [40] and
TS 29.232 [11] shall be applied.
...
procedure as specified in TS 23.205 [39] and TS 29.232 [11], with additional
information elements as specified in the present subclause. Within this
procedure, the MSC Server shall indicate the received speech codecs from the
remote UE and request a local IP address and UDP port from the CS-MGW and the
MSC Server may also indicate that the IP interface type is for MboIP as
defined in TS 29.232 [11]. The local IP address and UDP ports are used by the
CS-MGW to receive user plane data. The MSC Server shall include the local IP
address and UDP port number in the SDP answer for the selected codec.
...
Reserve and Configure RTP Connection Point procedure defined in TS 23.205 [39]
...
The MSC Server and the CS-MGW shall act in accordance with clause 6.2.3 in TS
23.205 [39] and apply the coding in accordance with TS 29.232 [11].
#### 7.2.3.3 A interface over IP
The MSC Server and the CS-MGW shall act in accordance with clause 6.2.4 in TS
23.205 [39] and apply the coding in accordance with TS 29.232 [11].
...
defined in TS 23.205 [39] and TS 29.232 [11].
...
Connection procedure as defined in TS 29.232 [11].
### 7.2.6 Announcement
...
procedure in accordance with TS 23.205 [39] and TS 29.232 [11].
If the MSC Server wishes to stop the sending of an announcement the MSC Server
shall apply the Stop Announcement procedure as defined in TS 23.205 [39] and
TS 29.232 [11].
### 7.2.7 Failure handling in the MSC server
If any procedure between the MSC Server and the CS-MGW has not completed
successfully or the MSC Server receives a Bearer Released procedure from the
CS-MGW the procedure in accordance with clause 6.2.1.3.13 of TS 23.231 [40]
and TS 29.232 [11] shall be applied.
## 7.3 Call clearing
...
the Release Termination procedure as specified in TS 29.232 [11] to request
the CS-MGW to remove the access side bearer termination.
...
the MSC Server shall act in accordance with clause 7.2.6 in TS 23.231 [40] and
TS 29.232 [11].
...
clause 7.2.6 in TS 23.231 [40] and TS 29.232 [11].
...
described in TS 24.008 [3]. If the call is already established towards the
...
10.1 - 10.10 of TS 23.205 [39] and subclauses 10.12 - 10.16 of TS 23.205 [39].
## 7.5 Explicit Congestion Notification
An MSC Server and a CS-MGW may support Multimedia Telephony using Explicit
Congestion Notification (see IETF RFC 3168 [48], IETF RFC 6679 [49], and TS
26.114 [28]), and may then act as an ECN endpoint to enable ECN with a local
ECN-capable terminal within a local network that properly handles ECN-marked
packets.
The ECN procedures described in TS 29.163 [46], clause 9.2.11, for MGCF and
IM-MGW are then applicable for MSC Server and the CS-MGW.
## 7.6 Multimedia Priority Service (MPS) Support
The Multimedia Priority Service (MPS) is specified in TS 22.153 [54]. An MSC
Server and a CS-MGW may support the priority treatment of a call/session
identified as an MPS call/session.
The MPS procedures described in TS 29.163 [46], clause 9.4, for MGCF and IM-
MGW are then applicable for MSC Server and the CS-MGW.
## 7.7 Interactive Connectivity Establishment
An MSC Server and a CS-MGW may support ICE functionality as specified in IETF
RFC 5245 [60] and TS 24.229 [2].
...
RFC 5245 [60]_._
The requirements as described in clause 9.2.12 of TS 29.163 [46] for MGCF and
IM-MGW, apply to the MSC Server and the CS-MGW when the ICE procedures are
supported.
## 7.8 Codec Parameters Handling
### 7.8.1 Handling of common codec parameters
The requirements as described in clause 9.2.13.1 of TS 29.163 [46] for the
MGCF and the IM-MGW, apply to the MSC Server and the CS-MGW.
### 7.8.2 EVS speech codec
The Enhanced Voice Services (EVS) speech codec is defined in TS 26.441 [61].
Its RTP payload type is defined in TS 26.445 [62], and procedures for its
usage as IMS Multimedia Telephony speech codec are defined in TS 26.114 [28].
...
of TS 29.163 [46] for the MGCF and the IM-MGW, apply to the MSC Server and the
CS-MGW.
#