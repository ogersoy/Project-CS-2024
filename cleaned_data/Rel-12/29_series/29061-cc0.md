...
and RFCs. The networks topologies may be based on LANs (e.g. 15orrecti), Point
...
intercepted by the HA and 37orrectio to the MS\'s care-of address, i.e. the
FA. The FA de‑tunnels the packets and forwards the packets to the MS. Mobile
IP related signalling between the MS and the FA is done in the user plane. MIP
...
for GERAN and UTRAN 54orrecti; in the Attach, UE-requested PDN connectivity
...
allocated by the AAA 94orrec but Ipv6 prefix is allocated by some other means,
...
Intra-SGSN change). See 3GPP TS 23.246 [65] and 3GPP TS29.060 [24]. GGSN shall
...
Identity.
The MBMS Service Area Identity and its semantics are defined in clause 15.3 of
3GPP TS 23.003 [40].
...
plane data with unicast mode (IP multicast packets over UDP encapsulated by IP
unicast header).
...
and sends the AAR to it for the roaming UE to receive data from a 143orrection
service.
...
to receive authorization from a 144orrection service.
3\. The authorization decision is provided from BM-SC in home PLMN in the AAA
...
as specified in clause 20.3.2.1 of 3GPP TS 23.007 [104], the MBMS GW shall set
the Termination-Cause to \"DIAMETER_LINK_BROKEN\" (see IETF RFC 3588 [66]) and
shall include the Diagnostic-Info AVP set to \"User Plane Failure\" if it
tears down the MBMS session as a result of detecting an SGi-mb path failure.
2\. The BM-SC removes the Diameter session and confirms the operation by
...
3588 [66]. As a consequence, the server shall not maintain any state
...
this is not permitted by the Diameter base protocol. See RFC 3588 [66].
## 20.4 SGmb Messages
...
Time-To-Data-Transfer with AVP the value set to 0 shall be included. The MBMS-
...
MBMS-Flag 931 20.5a.9 Unsigned32 V P M Y Restart-Counter 932 20.5a.10
Unsigned32 V P M Y Diagnostic-Info 933 20.5a.11 Unsigned32 V P M Y
* * *
...
\'M\' bit cleared. Otherwise, the RAR shall include the supported features
within the Supported-Features AVP(s) with the M-bit set.
...
+-------------------+----------------+-----+-------------------+ | Feature bit | Feature | M/O | Description | +-------------------+----------------+-----+-------------------+ | 0 | MBMS Heartbeat | O | This feature | | | | | indicates the | | | | | support of the | | | | | MBMS Heartbeat | | | | | functionality, | | | | | including the | | | | | AVPs and | | | | | corresponding | | | | | procedures. | +-------------------+----------------+-----+-------------------+ | Feature bit:The | | | | | order number of | | | | | the bit within | | | | | the Feature-List | | | | | AVP where the | | | | | least significant | | | | | bit is assigned | | | | | number \"0\". | | | | | | | | | | Feature: A short | | | | | name that can be | | | | | used to refer to | | | | | the bit and to | | | | | the feature, e.g. | | | | | \"EPS\". | | | | | | | | | | M/O: Defines if | | | | | the | | | | | implementation of | | | | | the feature is | | | | | mandatory (\"M\") | | | | | or optional | | | | | (\"O\") in this | | | | | 3GPP Release. | | | | | | | | | | Description: A | | | | | clear textual | | | | | description of | | | | | the feature. | | | | +-------------------+----------------+-----+-------------------+
