...
> initiate the Subscriber Profile Retrieval and 3GPP AAA Server registration
...
When IKEv2 [51] is used in the context of this specification the profile
specified in this section shall be supported by the PDG and the WLAN‑UE. IKEv2
shall meet the profiling given in clause 5.4.2 of TS 33.210 [50] with the
following additions:
\- The pseudorandom function AES-XCBC-PRF-128 shall be supported.
\- The integrity algorithm AES-XCBC-MAC-96 shall be supported.
For the certificate based IKEv2 authentication using the PDG certificate the
profiling given in TS 33.310 [49] shall apply.
For NAT traversal, the NAT support of IKEv2 shall be supported as specified in
section 2.23 of IETF RFC 5996 [51].
...
33.210 [50] with the following additions:
\- The authentication algorithm AES-XCBC-MAC-96 shall be supported.
...
The UICC-less WLAN UE\'s identity can be IMEI, if available, or MAC address of
the UE.
4\. The message is routed towards the proper 3GPP AAA Server based on the
realm part of the NAI. The routing path may include one or several AAA proxies
(not shown in the figure).
...
The MAC address of the WLAN AN and the MAC address of the UICC-less WLAN UE
...
specified in IETF RFC 4282 [14]. According to IETF RFC 5996 [51], the shared
secret generated in an EAP exchange (the MSK), when used over IKEv2, shall be
...
establishment. This will help the 3GPP AAA Server to distinguish between
authentications for WLAN access and authentications for tunnel setup.
...
authenticate the IKE_SA_INIT phase messages, as specified in IETF RFC 5996
[51].
...
\- The UE and the PDG use IKEv2, as specified in IETF RFC 5996 [51], in order
to establish IPSec security associations.
\- Public key signature based authentication with certificates, as specified
in IETF RFC 5996 [51], is used in order to authenticate the PDG and the UE.
