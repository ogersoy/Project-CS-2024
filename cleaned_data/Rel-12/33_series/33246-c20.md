...
established. Therefore it shall be possible for the BM-SC to request GCS AS
over MB2-C to establish a security association for the user plane. The
security protocol (DTLS or IKE/IPsec) to be used over MB2-U needs to be
configured in the GCS AS, possibly dependent on BM-SC and/or target network.
It shall also be possible for the BM-SC to receive a request from GCS AS for
the establishment of a security association for the user plane. The target IP
