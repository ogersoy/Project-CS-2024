...
CK~IM~ = CK~IM1~ \|\| CK~IM2~
Where CKIM1 are the 64 most significant bits and CKIM2 are the 64 least
significant bits.
...
with a pool of subscriptions with IMS and can assign individual Public User
Identities within this pool." (quoted from TS 23.228). This assignment is
temporary and the same IMPU (and IMPI) may be re-assigned to a different user
at a later time once they are free and available for re‑use.
