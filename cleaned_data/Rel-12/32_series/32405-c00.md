...
to the measurement cause \'No Reply\' (not specified > in TS 25.331 [4]). Each
RRC connection which is not established > because the cell is going to Energy
Saving mode is added to the > measurement cause 'EnergySaving' (not specified
in TS 25.331 > [4])\ > The sum of all supported per cause measurements shall
equal the > total number of RRC Connection Establishment Failures. In case >
only a subset of per cause measurements is supported, a sum > subcounter will
be provided first.
d) Each measurement is an integer value. The number of measurements is > equal
to the number of causes plus a possible sum value identified > by the _.sum_
suffix.
...
identified by the _.NoReply_ suffix.\ > The cause \'Energy Saving\' is
identified by the _.EnergySaving_ > suffix.
f) UtranCellFDD\ UtranCellTDDLcr\ UtranCellTDDHcr
g) Valid for circuit switched and packet switched traffic
h) UMTS
