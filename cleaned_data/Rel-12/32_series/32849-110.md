...
is for messages exchanged within one Dialog are same. Following the SIP
...
When the S-CSCF‑hA1 receives the initial SIP INVITE request back from the
AS‑hA1 and if no further services apply then the S-CSCF‑hA1 decides according
...
When the IBCF‑hA2 receives the SIP 200 (OK) response the IBCF‑hA2 sends the
SIP 200 (OK) response according to TS 24.229 [106] towards the UE‑A via the
S-CSCF‑hA1, AS‑hA1, IBCF‑hA1, IC‑A1, IBCF‑vA1, P-CSCF‑vA1 path as described
...
>
> When the S-CSCF‑hA1 receives the initial SIP INVITE request and the
...
> this point.
>
> The S-CSCF‑hA1 removes the \"+g.3gpp.home-visited\" Feature-Caps header
> field parameter.
...
> When the TRF‑vA1 receives the initial SIP INVITE request the TRF‑vA1:
>
...
>
> \- possibly adds an additional Route header field pointing to the IBCF‑vA1
> and also to the IC in cases where the IBCF-vA1 has no routeing decision to
> do;
>
> \- removes the \"+g.3gpp.loopback\" header field parameter from the Feature-
> Caps header field of the outgoing request; and
>
> \- appends the \"iotl\" SIP URI parameter with the value \"visitedA-homeB\"
> to the Request-URI.
>
> The TRF‑vA1 selects an IBCF to act as an exit point and sends the SIP INVITE
...
> **31-43. 200 (OK) response (IC-T1 to UE‑A)**
>
> The media connection is established by sending the SIP 200 OK response back
> through all involved network nodes.
>
> When the IBCF‑vV1 receives the SIP 200 (OK) response the IBCF‑vA1 sends the
> 200 (OK) response according to TS 24.229 [106] towards the UE‑A via the TRF,
> IBCF-vA1, IC-A1, IBCF-hA1, S-CSCF‑hA1, AS‑hA1, S-CSCF‑hA1, IBCF‑hA1, IC‑A1,
> IBCF‑vA1, P-CSCF‑vA1 path as described within TS 24.229 [106].
**Table 5.4.11: SIP 200 (OK) response** **SIP Header Field to Information
Element in** **IBCF-vA1**
...
network. The \"orig-ioi\" is set to the same type-3 IOI value that was
included in the initial SIP INVITE request from the S‑CSCF to the AS-hA1; and
...
> the I-CSCF‑hA1 according to TS 24.229 [106].
...
> \"iotl\" SIP URI parameter would be set to \"visitedA‑homeB\".
>
...
> SLF/HSS to identify the correct S**‑** CSCF where the call has to be routed
...
>
> Based on privacy settings within the SIP INVITE request, the P-CSCF‑vA1
> applies privacy according to IETF RFC 3323 [27] and IETF RFC 3325 [28].
>
> The P-CSCF‑vA1 applies trust domain according to TS 24.229 [106] subclause
...
> Network**‑** Info header field are removed from the outgoing SIP INVITE
> request.
>
> The P-CSCF‑vA1 sends the SIP INVITE request according to TS 24.229 [106] to
> the UE-A.
>
...
> by the UE‑A in the initial SIP INVITE request.
>
...
visited network of the terminating user** (i.e. **\"visited‑a\").** The
...
**\"home‑a\")** towards the terminating user.
...
> initial SIP INVITE request from the S‑CSCF‑hA1 to the AS-hA1, the \"transit-
...
not appearing. The S-CSCF removes received \"transit**‑** ioi\" values, the
IOI Type 1 or Type 3 in \"orig-ioi\" and \"term-ioi\" header field parameters.
The \"orig**‑** ioi\" header field parameter is set to the same type 2 IOI
...
> **19\. SIP PRACK request s (IBCF-hA2 to UE‑A via all intermediate nodes) and
...
UE‑** A according to TS 24.229 [4]. The messages are sent via all intermediate
...
**30\. SIP PRACK request s (IBCF‑hA2 to UE‑A via all intermediate nodes) and
SIP 200 (OK) response to the SIP PRACK request (UE-A to IBCF-hA2 via all
intermediate nodes).**
The SIP PRACK request and the SIP 200 (OK) response to the SIP PRACK request
...
network of the terminating user** (i.e. **\"visited‑a\").** The \"orig-ioi\"
header field parameter is set to the same type 1 IOI value that was included
in the initial INVITE request by the home network hA (i.e. **\"home‑a\")**
...
> request from the S‑CSCF‑hA1 to the AS-hA1, the \"transit-ioi\" is deleted by
...
removes received \"transit**‑** ioi\" values, the IOI Type 1 or Type 3 in
\"orig-ioi\" and \"term-ioi\" header field parameters. The \"orig**‑** ioi\"
...
**1.-5. SIP INVITE request (UE‑A to S-CSCF-hA1 via P-CSCF-vA1, IBCF-vA1, IC-A1
and IBCF-hA1)**
...
> party. In such a case the AS‑hA1 acts as a B2BUA. Thus an early dialog for
> the announcement and two dialogs will be created in the final call state one
> from UE-A to the AS‑hA1 and one from the AS‑hA1 to the terminating UE which
...
>
> **10.-15. SIP 183 (Session Progress) response (AS‑hA1 to UE‑A via S-CSCF-
> hA1, IBCF-hA1, IC-A1, IBCF-vA1 and P-CSCF-vA1)**
>
> The AS‑hA1 sends a SIP 183 (Session progress) response to S-CSCF‑hA1. The
> response includes:
...
>
> NOTE 2: If the interconnection network is SIP-unaware, step 13 is skipped.
**16\. SIP PRACK request /SIP 200 (OK) response (UE‑A to AS‑hA1 / AS‑hA1 to
UE‑A)**
...
> AS‑hA1/MRFC in order to inform that the announcement is terminated.
>
...
>
> If the originating UE‑A has indicated support of the SIP 199 (Early Dialog
> Terminated) response code, the AS‑hA1 sends a SIP 199 (Early Dialog
> Terminated) provisional response towards the originating UE‑A to terminate
> the early dialog between itself and the UE‑A.
...
>
> The AS‑hA**1** sends the SIP INVITE request towards the S-CSCF‑hA**1**. The
...
>
> **28-33. SIP INVITE request (S-CSCF‑hA1 to IC-B1 via IBCF-hA1, IC-A1, IBCF-
> vA1, TRF-vA1 and IBCF-vA1)**
>
> The S-CSCF‑hA1 decides to loopback the SIP INVITE request and forwards the
...
>
> NOTE 4: If the interconnection network is SIP-unaware, step 30 is skipped
> and step 33 is executed between IBCF-vA1 and IBCF-hB instead.
>
> **34\. SIP 180 (Ringing) response (IC-B1 to UE‑A via IBCF-vA1, TRF-vA1,
> IBCF-vA1, IC-A1, IBCF-hA1, S-CSCF-hA1, AS-hA1, S-CSCF-hA1, IBCF-hA1, IC-A1,
> IBCF-vA1 and P-CSCF-vA1)**
>
> When the UE‑A of the calling party receives the SIP 200 (OK) response to the
> SIP INVITE request and optional steps 21- 26 were not performed, the UE‑A
> can regard the early dialog created for the announcement between the UE‑A
> and the AS‑hA1 as terminated.
>
...
>
> When the UE‑A of the calling party receives the 200 (OK) response to the SIP
> INVITE request and optional steps 21- 26 were not performed, the UE can
> regard the early dialog created for the announcement between the UE‑A and
> the AS‑hA1 as terminated.
>
...
communication
The steps of the signalling flow are as follows:
> **1-5. SIP INVITE request (UE‑A to S-CSCF-hA1 via P-CSCF-vA1, IBCF-vA1,
> IC-A1 and IBCF-hA1)**
>
...
> involvement.
>
> **7\. SIP INVITE request (S-CSCF-hA1 to AS‑hA1 /MRFC) - see example in table
> 5.3.2-6**
>
...
>
> The AS‑hA1 sends a SIP 183 (Session progress) response to the S-CSCF‑hA1.
...
> c) the Require header field set to \"100rel\".
NOTE 2: If the interconnection network is SIP-unaware, step 13 is skipped.
**16\. SIP PRACK request / 200 (OK) response (UE‑A to AS‑hA1 / AS-hA1 to
UE-A)**
Table 5.10.3.1:
...
> **27\. SIP INVITE request (AS-hA1 to S-CSCF-hA1)**
>
> The AS‑hA1 sends the SIP INVITE request towards the S-CSCF‑hA1. The SIP
...
> The S-CSCF‑hA1 decides not to loopback the SIP INVITE request and forwards
...
>
> **30.-37. SIP 200 (OK) response (IC-B1 to UE-A via S-CSCF-hA1, AS-hA1,
> S-CSCF-hA1, IBCF-hA1, IC-A1, IBCF-vA1 and P-CSCF-vA1)**
>
> When the UE‑A of the calling party receives the 200 (OK) response to the SIP
> INVITE request and optional steps 21- 26 were not performed, the UE can
> regard the early dialog created for the announcement between the UE‑A and
...
### 5.11.1 General
  * This clause describes PS to CS SRVCC access transfer scenarios including the registration and the additional message flow due to PS to CS SRVCC.
...
> vA1 forwards the SIP REGISTER request to an ATCF‑vA1 according to local
...
SRVCC features the visited network supports.
  * **5.‑8. SIP REGISTER request (IBCF-vA1 to S-CSCF-hA1 via IC-A1, IBCF-hA1 and I-CSCF-hA1**
...
> The S-CSCF‑hA1 acknowledges the SIP REGISTER request by means of a SIP 200
...
> party SIP REGISTER request to the SCC AS-hA1. The S‑CSCF‑hA1 includes in the
...
> The SCC AS‑hA1 stores the STN-SR ([tel:+1-237-555-3333]{.underline})
...
> for the ATCF-vA1 during PS to CS SRVCC access transfer, the SCC AS‑hA1 sends
...
> value in a \"term‑ioi\"** header field parameter **identifying the visited
...
> (OK) response was received i.e. the interconnect network A.**
  * 5.11.3 PS to CS SRVCC access transfer for a call in the active phase without loopback
...
**3\. SIP INVITE request due to STN-SR (MSC server-vA1 via P-CSCF-vA1** **to
ATCF-vA1) - see example in table 5.11.3.3.**
...
> response towards the MSC server‑vA1 containing media parameters retrieved
> from the ATGW.
>
...
> the visited network A **(i.e. \"visited‑a\")**.
...
> call the IBCF-hA1 selects an I‑CSCF in the home network and sends the SIP
...
> request due to ATU-STI directly to SCC AS**‑** hA1.
>
...
the SIP INVITE request due to ATU-STI. Additionally the SCC AS**‑** hA1 added
...
IC-A1, IBCF‑vA1, TRF and IBCF‑vA1) - see example in table 5.11.4.1.**
> When the SCC AS-hA1 receives the SIP INVITE request due to ATU-STI the SCC
> AS-hA1 compares the received SDP with the SDP negotiated in the existing
> session. The conclusion is that it differs so the SCC AS-hA1 needs to send a
> re-SIP INVITE request towards the remote UE.
...
hA1, IC-A1, IBCF‑vA1, TRF and IBCF‑vA1)
SIP INVITE request sip:[5555::bbb:ccc:ddd:aaa] SIP/2.0
From: \;tag=171828
To: \;tag=987651
...
vA1, IC-A1, IBCF-hA1 and S‑CSCF‑hA1).**
...
**30-33. SIP 200 (OK) response (SCC AS-hA1 to ATCF-vA1 via S-CSCF‑hA1, IBCF-
hA1, IC‑A1 and IBCF-vA1).**
...
and VPLMN.
