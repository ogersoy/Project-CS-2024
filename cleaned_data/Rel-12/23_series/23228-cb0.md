...
Access Network Information) and a Geographical Identifier. The P-CSCF may
then, based on operator policy, use this mapping table to convert the user
location into a Geographical Identifier, and insert the Geographical
...
access, e.g. a PGW is used as an IP anchor point and IP-Connectivity Access
...
A Geographical Identifier may be generated by the P-CSCF based on the
retrieved Access Network Information, as specified in clause E.8.
...
be set in the path between the PGW and the Functions supporting media
...
The WWSF is provided with a pool of subscriptions, each containing a single
unique IMPU/IMPI pair, to IMS and can assign individual Public and Private
...
Identity IMPU-IMPI pair from its pool can be re-assigned to another user.
