...
be included in event reports (and associated location QoS), and whether only
...
the (H)GMLC contact address and LDR reference number are not included. In all
cases, the service operation includes an LCS Correlation identifier, the AMF
identifier, the serving cell identity, the client type and may include an
indication if UE supports LPP, the required QoS and Supported GAD shapes.
...
the LMF acts a serving LMF. If the multiple QoS class was used in the location
request, the LMF provides the achieved Location QoS Accuracy in step 15. If
the UE cannot support the periodic and triggered location request, the service
operation returned to the AMF shall include a suitable error cause. The
service operation also includes the UE Positioning Capability if the UE
Positioning Capability is received in step 15 including an indication that the
capabilities are non-variable and not received from AMF in step 14.
...
NOTE 8: The scheduled location time included at step 25 equals T + (N-1)*P,
where T is the initial Scheduled Location Time, N is the Report Number (N≥1)
...
NOTE 9: Inclusion of a new deferred routing identifier in the event report
...
and 6.11.3 and as described for step 8 in clause 6.1.1 and step 12 in clause
6.1.2. The LMF then determines the UE location using the location measurements
and/or location estimate(s) obtained at this step and/or received at step 25.
The LMF may also determine the timestamp of the location estimate.
NOTE 10: A precondition for the procedure in clause 6.11.1 is that an LCS
...
NOTE 11: In the case of roaming, the LMF may select the VGMLC for step 28
...
NOTE 12: As an optional optimization for a roaming UE, instead of performing
steps 28 and 29, the LMF may invoke the Nlmf_Location_EventNotify service
operation directly towards the HGMLC.
NOTE 13: In the event of mobility of the UE to an access network for which
...
to the external LCS client or AF (via the NEF), and sends the LDR reference
number to LCS client. The (H)GMLC may also verify UE privacy requirements
before reporting the event and any location to the external LCS client or AF.
If multiple QoS class was used in the initial location request, the LMF
provides the achieved Location QoS Accuracy in step 27.
...
NOTE 14: Service continuity for reporting of periodic or trigger events when a
...
session initiation) or the AMF decides to verify UE location via LCS service
for a UE registering or is registered for NR satellite access.
2\. For verifying UE location via LCS service for NR satellite access this
step is mandatory, for other triggers the step is optional. The AMF selects an
LMF based on NRF query or configuration in AMF and invokes the
...
from a regulatory services client (e.g., emergency services) and may include
an indication if UE supports LPP, the required QoS and Supported GAD shapes,
the UE Positioning Capability if available. When AMF needs to know the
geographical area of the UE to check whether the PLMN is allowed to operate in
the area, an indication of this is included. If any of the procedures in
clause 6.11.1 or 6.11.2 are used the service operation includes the AMF
identity.
...
AMF included an indication of UE country determination at step 2, the LMF maps
the UE location to a geographical area where a PLMN is or is not allowed to
operate.
...
verification, UE Positioning Capability, TNAPId (see TS 29.571 [33]), TWAPId
(see TS 29.571 [33]), Scheduled Location Time.
**Output, Required:** Success/Failure indication
**Output, Optional:** Geodetic Location, Local Location including Coordinate
ID, Civic Location, Position Methods Used (in the case of success indication
...
Accuracy, the timestamp of the Location.
...
**Input, Optional:** Event reporting status, UE location information, UE
Positioning Capabilities, Scheduled Location Time, the timestamp of the
Location.
**Output, Required:** Success/Failure indication
**Output, Optional:** None.
...
**Output, Required:** Success/Failure indication
**Output, Optional:** Geodetic location, Local Location including Coordinate
ID, civic location, age of location, position methods used (in the case of
success indication provided), failure cause (in the case of failure indication
