...
Capability UE GGSN
* * *
DiffServ Edge Function Optional Required RSVP/IntServ Optional Optional IP
Policy Enforcement Point Optional Required (*)
...
{width="6.6875in" height="3.84375in"}
...
NOTE 1: The diagrams in this clause depict one possible signalling sequence,
...
NOTE 2: The diagrams in this clause depict the case when the GGSN is not RSVP
...
NOTE 3: There is no timing relationship between the set of flows for the
...
NOTE 4: There is no timing relationship between the set of flows for the
uplink (above the line) and the downlink (below the line).
...
\- to trigger the Create PDP Context Request message after the RESV message.
\- to trigger only one PDP context after all RSVP exchanges have completed.
NOTE 2: The diagrams in this clause depict the case when the GGSN is RSVP
...
Information.
NOTE 4: If the decision was previously cached locally at the GGSN, it may not
be necessary to query the PDF again. Otherwise the GGSN may have to query the
PDF.
...
NOTE 5: If the decision was previously cached locally at the GGSN, it may not
be necessary to query the PDF again. Otherwise the GGSN may have to query the
PDF.
...
NOTE 6: If the decision was previously cached locally at the GGSN, it may not
be necessary to query the PDF again. Otherwise the GGSN may have to query the
PDF.
...
NOTE 7: Steps 16 to 20 are optional if the existing PDP context already
...
NOTE 8: If the decision was previously cached locally at the GGSN, it may not
be necessary to query the PDF again. Otherwise the GGSN may have to query the
PDF.
...
Local Policy
NOTE 9: There is no timing relationship between the set of flows for the
uplink (above the line) and the downlink (below the line).
#