...
responses to the broadcast message by high numbers of devices are sent at
almost the same time, it is recommended that the SCS/AS provide the UEs with a
...
can activate reception using MBMS device APIs (see TS 26.347 [42]).
Application level interactions between the UE and the SCS/AS are out of scope
of this specification.
