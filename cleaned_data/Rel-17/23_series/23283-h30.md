...
Alert indicator O May be used to indicate that an emergency alert is to be
sent Location O Location, if available Implicit floor request (see NOTE) O
...
party Functional alias O The functional alias of the calling party MCPTT group
ID M The MCPTT group ID with which the alert is associated Organization name O
The alerting MCPTT user\'s mission critical organization name. Location O The
alerting MCPTT client\'s location
* * *
...
MCData transaction Disposition M Disposition which is delivered, read,
delivered and read, or disposition prevented by system NOTE: A reserved value
of the Information Element shall be defined which indicates that the sender
does not support this Information Element.
* * *
...
procedures between the MC service system and the IWF to enable role based
addressing of users homed in the IWF.
