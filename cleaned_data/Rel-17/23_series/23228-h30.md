...
[54] or TS 23.503 [95], where P-CSCF requests the PCRF/PCF to report the PLMN
identifier where the UE is currently located. The received PLMN ID information
is then forwarded in the SIP REGISTER request.
...
2\. If this is initial IMS registration then the P-CSCF subscribes to the
PCRF/PCF to be notified of the PLMN ID where the UE is currently attached.
...
* * *
imsSubscriber Data Get Request/Response S-CSCF, I-CSCF, AS Management
...
Request/Response AS imsUE Context **Registration** Request/Response S-CSCF
Management Deregistration**Notification** Subscribe/Notify S-CSCF (_ImsUECM)
Deregistration Request/Response S-CSCF **Authorize** Request/Response I-CSCF
Update Request/Response S-CSCF RestorationInfoGet Request/Response S-CSCF
RestorationInfoUpdate Request/Response S-CSCF ImsUE Authentication **Get**
Request/Response S-CSCF
...
+----------------------+----------------------------------------------+ | IMS Subscriber data | Description | +======================+==============================================+ | Service Profile Data | This may include e.g. service parameters, | | | the S-CSCF allocated to a public identity or | | | the list of S-CSCFs and their capabilities, | | | Application Server address, triggers, | | | information on subscribed media, profile | | | parameters (e.g. barring indicator, etc.) as | | | defined in TS 29.228 [30]. | | | | | | **Service Profile Data is consumed by | | | CSCF.** | +----------------------+----------------------------------------------+ | Repository Data | Data that is understood syntactically but | | | not semantically by the HSS (unstructured | | | Data). It is data that an AS may store in | | | the HSS to support its service logic. One | | | example is data that an AS stores in the | | | HSS, using it as a repository. | | | | | | Service Indication identifies the set of | | | service related transparent data associated | | | to a Public Identity. | | | | | | **Repository Data is consumed by IMS-AS.** | +----------------------+----------------------------------------------+ | Non-Transparent Data | Data that is understood both syntactically | | | and semantically by the HSS e.g. location | | | information. Non-Transparent Data is | | | structured using data references as defined | | | in TS 29.328 [79]. | | | | | | **Non-Transparent Data is consumed by | | | IMS-AS.** | +----------------------+----------------------------------------------+
...
## AA.3.1 General
An SBI capable IMS entity (e.g. I-CSCF, S-CSCF or IMS AS) performs HSS
