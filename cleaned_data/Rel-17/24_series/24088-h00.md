...
The NotifySS operation containing the SS-Status indicating that a barring
program relating to incoming calls is currently active and operative shall be
sent to the calling subscriber only in the RELEASE COMPLETE message, if the MS
only supports GSM Phase 1.
# 3 Anonymous Call Rejection
Activation, deactivation and interrogation of the status of the ACR
supplementary service are supported by USSD procedure (see 3GPP TS 23.088
[9]).
