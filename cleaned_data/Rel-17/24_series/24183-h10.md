...
Upon receiving the first reliable SIP 18x response to the initial INVITE
request, and this 18x response contains a Contact header field parameter
...
appear in the SDP answer in the 200 (OK) response, set the port number of the
...
###### ## Annex F (normative): Media feature tags defined within the current
document
## F.1 General
This subclause describes the media feature tag definitions that are applicable
for the CRS service.
## F.2 Definition of media feature tag g.3gpp.crs
Editor\'s note [CR#0075, WI TEI17]: The media feature tag g.3gpp.crs should be
registered immediately on approval of this specification. The ASN.1 Identifier
will need to be updated once the IANA registration is completed.
Media feature-tag name: g.3gpp.crs
ASN.1 Identifier: 1.3.6.1.8.2.x
Summary of the media feature indicated by this tag: This media feature tag is
for indicating terminating User Equipment (UE) support of the capabilities for
the 3GPP Customized Ringing Signal (CRS) service.
Values appropriate for use with this feature tag: Token with an equality
relationship. Values specified:
> rs: The value is for indicating terminating UE supports the capability of
> playing CRS media (i.e., the received early media before the terminating
> user answers the call) as a ringing signal in regular session.
...
Related standards or documents: 3GPP TS 24.183: IP Multimedia Subsystem (IMS)
Customized Ringing Signal (CRS); Protocol specification.
Security Considerations: Security considerations for this media feature tag
are discussed in subclause 11.1 of IETF RFC 3840.
#