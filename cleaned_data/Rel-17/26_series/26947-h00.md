...
of RS+LDPC as MBMS application layer FEC as provided by the proponent.
# 1 Scope
...
\- Channel model with Markov model loss rate of 1%, 5%, 10% and 20% target
BLER as introduced in TDoc R1-120831, Annex B, Table 2 section 3.2 with speed
...
Search for \'SSHDroid\'. Once installed, make sure to configure port 2222 in
its settings. For some reason when SSHDroid defaults to port 22 when running
in root mode, it is not possible to ssh in. Port 2222 has no such restriction.
