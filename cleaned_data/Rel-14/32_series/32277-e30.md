...
message to the ProSe Function in the HPLMN to get the Target RPAUID.
...
for open discovery and ProSe Restricted Code in serving PLMN for restricted
discoveryusing the radio resources authorized and configured by E-UTRAN to be
...
Responsewhen step 7 is successful.
\- (ProSe Application Code, validity timer) for open discovery.
\- (ProSe Restricted Code/ProSe Restricted Code Prefix[ProSe Restricted Code
Suffix pool], validity timer, Discovery Entry ID, ProSe Enabled Indicator) for
districted discovery.
...
by E-UTRAN to be used for ProSe as defined in RAN specifications.
...
validity timer) message. The Prose Function in the other PLMN also stores, in
the context of the announcing UE, the PDUID of the monitoring UE and the
allocated validity timer.
...
the ProSe Function in the HPLMN and send a Discovery Request (RPAUID, UE
Identity, command, Discovery Type, Discovery Model, Application ID, Discovery
Entry ID) message. ProSe Function in the HPLMN check the authorization with
the HSS.
3\. ProSe Function sends Debit Units Request [Event] indicating \"Restricted
Announce\" to the OCS.
...
7\. The UE may then obtain the radio resources to monitor using the Discovery
...
3\. ProSe Function sends Reserve Units Request [Initial, RPUID, cmd =
Restricted Announce] to the OCS for requesting units.
...
(Discovery Model, ProSe Response Code, Discovery Query Filter(s), validity
timer, Discovery Entry ID) message. After the ProSe Function in the HPLMN
...
the ProSe Function in the HPLMN and send a Discovery Request (RPAUID, UE
Identity, command, Discovery Type, Discovery Model, Application ID,
Application Transparent Container, Discovery Entry ID) message. ProSe Function
in the HPLMN check the authorization with the HSS. Based on the Target ProSe
Discovery UE ID, Application ID, and Target Restricted ProSe App User ID, the
ProSe Function locates the Discoveree UE(s) context.
3\. ProSe Function sends Debit Units Request [Event] indicating \"Restricted
Monitor\" to the OCS.
...
(ProSe Query Code(s), ProSe Response Code, validity timer).
14\. The UE may then obtain the radio resources to monitor using the Discovery
Query Filter, as authorised and configured by E-UTRAN for ProSe as defined in
RAN specifications.
...
ProSe Function locates the Discoveree UE(s) context.
3\. ProSe Function sends Reserve Units Request [Initial, RPUID, cmd =
Restricted Monitor] to the OCS for requesting units.
...
procedures for Model B restricted discovery (non-roaming) are defined in TS
23.303 [238] clause 5.3.4A.1.
1-2. These Steps are defined in TS 23.203 [238].3. ProSe Function sends Debit
Units Request[Event] indicating \"Match\" to the OCS.
4\. The OCS performs the Debit Units operation for \"Match \".
5\. The OCS returns the Debit Units Response to ProSe Function.
