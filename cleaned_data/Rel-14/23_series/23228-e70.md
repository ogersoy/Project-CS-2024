...
In this scenario, the breakout point for both the IMS signalling and media
traffic is in the home network for a roaming UE, i.e. for 3GPP systems, the
PGW/GGSN for a roaming UE is in the HPLMN of the UE.
...
IMS services with home routed traffic are depicted in Annex W.
...
sharing indicator and an optional pre-emption control information in addition
to the application identifier and the service priority. For MCPTT, the service
priority and the priority sharing indicator are defined in TS 23.179 [92]. The
pre-emption control information is used to indicate to the PCRF how to perform
pre-emption in accordance to TS 23.203 [54].
...
information, the P-CSCF shall convey to the PCRF the priority sharing
...
is applicable to UTRAN and E-UTRAN using EPC access. In this roaming model the
PGW is located in the home PLMN and therefore UE IMS signalling and user plane
are routed to home PLMN.
# W.2 Architecture
...
The UE shall be provisoned by HPLMN with an enumerated list of SIP-based
services that are 3GPP PS Data Off exempted either via Device Management or in
the UICC.
In this release of the specification there shall be a single list of 3GPP PS
Data Off Exempt Services for all VPLMNs.
...
the UICC the list of 3GPP PS Data Off Exempted Services.
The information shall be provisioned in the UE prior to enabling the 3GPP PS
Data Off Service. Any change to the list shall immediately be sent to the UE
for enforcement.
...
In this release of the specification, each Application Server shall be
configured with the applicable list of 3GPP PS Data Off Exempted Service.
Furthermore, in this release of the specification, there shall be a single
list for all PLMNs.
...
only SIP-based services which are part of the the SIP-based 3GPP PS Data Off
Exempted Services are permitted.
...
were not Data Off exempted.
NOTE: The AS could be implemented in an existing AS e.g. SCC AS or distributed
across other AS\'s.
#