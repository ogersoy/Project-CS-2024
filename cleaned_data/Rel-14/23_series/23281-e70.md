...
1\".
[3] 3GPP TS 22.281: \"Mission Critical Video services over LTE\".
...
MCVideo server)
The MCVideo-3 reference point, which exists between the MCVideo server and
another MCVideo server from an MC service provider for MCVideo application
signalling.
...
resources for MCPTT application usage. The MCVideo-6 reference point uses the
MB2-C interface as defined in 3GPP TS 29.468 [13].
...
Table 7.1.2.2.10-1 describes the information flow group call join request from
the MCVideo client to the MCVideo server.
Table 7.1.2.2.10-1: Group call join request (MCVideo client -- MCVideo server)
* * *
Information Element Status Description MCVideo ID M The MCVideo ID of the
...
is requested SDP offer M Media parameters of MCVideo client
* * *
##### 7.1.2.2.11 Group call join response (MCVideo server -- MCVideo client)
Table 7.1.2.2.11-1 describes the information flow group call join response
from the MCVideo server to the MCVideo client.
...
associated
* * *
##### 7.1.2.2.16 MCVideo emergency state cancel request
Table 7.1.2.2.16-1 describes the information flow MCVideo emergency state
...
23.280 [6].
Table 7.1.2.2.16-1: MCVideo emergency state cancel request
+-----------------------------+--------+-----------------------------+ | Information Element | Status | Description | +-----------------------------+--------+-----------------------------+ | MCVideo ID | M | The identity of the | | | | cancelling party | +-----------------------------+--------+-----------------------------+ | MCVideo group ID | M | The MCVideo group ID with | | | | which the alert is | | | | associated | +-----------------------------+--------+-----------------------------+ | Client emergency state | O | Requests cancellation of | | cancel inform | | the emergency state | | | | associated with this | | (NOTE) | | MCVideo client | +-----------------------------+--------+-----------------------------+ | Group emergency condition | O | Requests cancellation of | | cancel request | | the emergency condition of | | | | the group | | (NOTE) | | | +-----------------------------+--------+-----------------------------+ | NOTE: At least one of these | | | | information elements shall | | | | be present | | | +-----------------------------+--------+-----------------------------+
##### 7.1.2.2.17 MCVideo emergency state cancel response
Table 7.1.2.2.17-1 describes the information flow MCVideo emergency state
cancel response from the MCVideo client to the MCVideo server and from the
MCVideo server to the MCVideo client, for the procedures defined in 3GPP TS
23.280 [6].
Table 7.1.2.2.17-1: MCVideo emergency state cancel response
* * *
Information Element Status Description MCVideo ID M The identity of the
...
(success or failure)
* * *
##### 7.1.2.2.20 MCVideo emergency group call cancel request
Table 7.1.2.2.20-1 describes the information flow emergency group call cancel
request from the MCVideo client to the MCVideo server.
Table 7.1.2.2.20-1: MCVideo emergency group call cancel request
* * *
Information Element Status Description MCVideo ID M The identity of the
...
* * *
##### 7.1.2.2.21 MCVideo emergency group call cancel response
Table 7.1.2.2.21-1 describes the information flow emergency group call cancel
response from the MCVideo server to the MCVideo client.
Table 7.1.2.2.21-1: MCVideo emergency group call cancel response
* * *
Information Element Status Description MCVideo ID M The identity of the
cancelling party MCVideo group ID M The MCVideo group ID on which the MCVideo
...
Pre-conditions:
1\. The MCVideo server may use the group call join request to generate an
implicit affiliation (using the affiliation procedure for MCVideo) on behalf
of the client that sends the MCVideo group call join request.
2\. MCVideo user 2 and MCVideo user 3 have previously joined (affiliated) to
...
authorized to use the MCVideo service. No call is currently in progress for
the group.
...
of the desired group. It contains the MCVideo user\'s MCVideo ID and the
MCVideo client media parameters. If there is a request for media transmission,
then the group call join request contains an indication of the transmit media
request.
1b. The MCVideo server receives the group call join request. MCVideo server
...
acceptance of the group call join request and also returns the MCVideo server
selected media parameters for the group call in the group call join response.
...
###### 7.1.2.5.1.3 MCVideo emergency group call cancel
...
for the MCVideo client cancelling an in-progress emergency.
NOTE 1: For simplicity, a single MCVideo server is shown in place of a user
home MCVideo server and a group hosting MCVideo server.
NOTE 2: The end of the MCVideo emergency group call does not cancel the
...
5\. MCVideo client 1 previously initiated the in-progress emergency.
Figure 7.1.2.5.1.3-1: MCVideo emergency group cancel
1\. The user at the MCVideo client 1 initiates an MCVideo emergency group
cancel.
NOTE 3: An MCVideo user authorized to cancel in-progress emergencies on the
...
2\. MCVideo client 1 sends an MCVideo emergency group call cancel request to
the MCVideo server.
NOTE 4: When MCVideo emergency alerts are in effect together with an MCVideo
emergency group condition on the same MCVideo group, the MCVideo emergency
alert can, under some circumstances be cancelled at the same time. In that
case, the MCVideo emergency group cancel request carries an indication that
the alert is also being cancelled.
...
5\. The MCVideo server sends an MCVideo emergency group call cancel request to
the MCVideo group members.
6\. MCVideo group members are notified of the MCVideo emergency cancel.
7\. The receiving MCVideo clients send the MCVideo emergency group call cancel
response to the MCVideo server to acknowledge the MCVideo emergency cancel.
For a multicast call scenario, these acknowledgements are not sent.
8\. The MCVideo server sends the MCVideo emergency group call cancel response
to the MCVideo user 1 to confirm the MCVideo emergency group cancel. MCVideo
client 1 resets its local emergency status since it is the one requesting the
MCVideo emergency group cancel.
...
emergency state cancel procedures defined in 3GPP TS 23.280 [6] using
...
MCVideo emergency communication. Only the MCVideo user of the MCVideo client
can clear the client\'s local MCVideo emergency state.
...
##### 7.1.3.9.1 General
The MCVideo client use emergency alert initiation and emergency state cancel
...
* * *
Information Element Status Description MCVideo ID M The MCVideo ID of the call
originating party MCVideo ID M The MCVideo ID of the source party who should
transmit media MCVideo ID O (see NOTE) The MCVideo ID of the destination party
who should receive media MCVideo group ID O (see NOTE) The MCVideo group ID of
the destination group whose affiliated group members should receive media SDP
offer O Media parameters of MCVideo client. Requested commencement mode O An
indication that is included if the user is requesting a particular
commencement mode NOTE: MCVideo ID information element is present if remote
video push to destination user is requested. MCVideo group ID information
element is present if remote video push to group is requested.
* * *
...
transmit media MCVideo ID O (see NOTE) The MCVideo ID of the destination party
who should receive media MCVideo group ID O (see NOTE) The MCVideo group ID of
the destination group whose affiliated group members should receive media
Acceptance confirmation O An indication whether the user or group members have
positively accepted the call. SDP answer M Media parameters selected NOTE:
MCVideo ID information element is present if remote video push to destination
user is requested. MCVideo group ID information element is present if remote
video push to group is requested.
* * *
...
who should receive media MCVideo group ID O (see NOTE) The MCVideo group ID of
the destination group whose affiliated group members should receive media
NOTE: MCVideo ID information element is present if remote video push to
destination user is requested. MCVideo group ID information element is present
if remote video push to group is requested.
* * *
...
* * *
Information element Status Description Result M Indicates success or failure
MCVideo capabilities M List of capabilities available at the requested MCVideo
user\'s UE
* * *
...
multiplex, present only if media multiplexing
* * *
##### 7.7.1.2.20 Remote transmit media end response
...
Pre-conditions:
1\. An off-network group communication has been established.
2\. MCPTT client 1 has higher transmission priority than MCPTT client 2.
Figure 7.7.2.10-1: Simultaneous transmission requests
1a. The MCPTT client 1 sends the transmission request message to the MCPTT
group.
1b. The MCPTT client 2 sends the transmission request message to the MCPTT
group.
...
are used for video and audio. TMGI (NOTE) O The MBMS bearer identifier if the
media of the MCVideo group call is not sent on the same MBMS bearer as this
MapGroupToBearer message. Call acknowledgement indicator O Indication that the
MCVideo group call requires acknowledgement from receiving MCVideo clients.
NOTE: TMGI shall be present if this message is sent over a different MBMS
bearer than the media of the group call; TMGI may be present if this message
is sent over the same MBMS bearer as the media of the group call.
* * *
