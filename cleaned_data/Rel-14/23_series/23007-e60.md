...
the SCS/AS for the deleted bearer, the SCEF behaves as specified in subclause
...
The SGW-U will receive the SGW-C recovery time stamps in Sx association setup
Request/Response and PFCP heartbeat requests/responses.
When an SGW-U detects that a peer SGW-C has restarted, the SGW-U shall delete
all its Sx session contexts and the Sx Association affected by the SGW-C
restart. When the SGW-U receives a GTP‑U PDU not matching any PDRs, it shall
discard the GTP‑U PDU and return a GTP error indication to the originating
node (the PGW, the eNodeB, the S4-SGSN, or if Direct Tunnel is established,
the RNC); as an exception, if the restoration of PDN connections after an SGW
failure is supported, the SGW-U shall not send Error indication message for a
configurable period after the SGW-C restart.
...
procedure.
When a SGW-U detects that a peer SGW-C is not reachable for a preconfigured
time, the SGW-U shall delete all session contexts and the PFCP association
contexts affected by the unreachability of the SGW-C that it may have stored.
### 16.1A.4 SGW-U Failure
The SGW-C will receive the SGW-U recovery time stamps in Sx association setup
Request/Response and PFCP heartbeat requests/responses.
After and SGW-U restart, the Sx association between SGW-C(s) and the SGW-U has
to be re-astablished.
Restoration of Sx sessions may start immediately after the Sx association
setup procedure:
...
the CP function. See subclause 5.3.2 of TS 29.244 [43]
The SGW-U shall not send Error indication message for a configurable period
after an SGW-U restart when the SGW-U receives G-PDU not matching any PDRs.
...
an SGW-U restart in subclause 16.1A.4) is not supported in this release. In
some cases, a restarted PGW-U cannot determine from the UE IP address the CP
...
The PGW-U will receive the PGW-C recovery time stamps in Sx Association Setup
Request/Response and PFCP heartbeat requests/responses.
When a PGW-U detects that a peer PGW-C has restarted, the PGW-U shall delete
all session contexts and the Sx association contexts affected by the restart
that it may have stored.
When a PGW-U detects that a peer PGW-C is not reachable for a preconfigured
time, the PGW-U shall delete all session contexts and the PFCP association
contexts affected by the unreachability of the PGW-C that it may have stored.
### 17.1A.4 PGW-U Failure
The PGW-C will receive the PGW-U recovery time stamps in Sx Association Setup
Request/Response and PFCP heartbeat requests/responses.
Immediately after the re-establishment of an Sx association between the PGW-C
and the PGW-U, the PGW-C may start restoring Sx sessions in the PGW-U.
...
handle a peer PFCP entity failure or restart.
...
Recovery Time Stamps and shall clear all remote Recovery Time Stamps.
...
Failure without Restart, see subclause 17.1A.4.
If the path to the PGW-C is down, the PGW-U should handle this as PGW-C
Failure without Restart, see subclause 17.1A.3.
...
detection and handling shall be supported as specified in subclause 20.3.1
...
the same way as specified in the subclause 16.1A.3 for the SGW-U failure and
...
The procedures specified for an SGW in subclauses 27.2 and 27.3 apply as well
to an SGW-C. An SGW-C shall always apply in addition the related PFCP
procedures towards the SGW-U.
