...
Comments:
  * It should be highlighted that the PULL and PUSH modes are only determined by the corresponding PSAP configuration to either wait for a SEND messages from IVS before sending START messages (PUSH mode), or sending START messages immediatedly after call connect without waiting for SEND messages from IVS (PULL mode). If the IVS does not know whether PSAP uses PUSH or PULL mode, the IVS shall always start with SEND messages until it detects START messages from PSAP.
  * Naturally, the PULL-RTT will be always shorter than the PUSH-RTT since PUSH mode comprises the SEND procedure followed by the entire PULL mode procedure.
  * Comparison of PULL-RTT and PUSH-RTT may be used to quantify the time advantage of PULL over PUSH mode.
  * The defined RTTs can be easily measured in practical field or lab tests on PSAP and IVS side. Hence these performance indicators may be used to test and certify IVS and PSAP products end-to-end using a fast RF channel simulator with controllable SNR, to which the IVS is connected.
