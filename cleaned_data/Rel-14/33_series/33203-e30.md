...
be done in the same way as specified in RFC 2617 [12] and the present
document.
It is the policy of the HN that decides if an authentication shall take place
...
\- CipherSuites with NULL encryption may be used.
\- The UE shall always include at least one CipherSuite that supports (non-
NULL) encryption during the handshake phase.
NOTE 0: TLS 1.0, TLS 1.1, and TLS 1.2 already fulfill the preceding
requirement as the mandatory cipher suite has non-NULL encryption and the
client includes all supported cipher suites in the Client Hello message.
...
be at least as strong as IPsec(RFC 4301 53 [)] using triple-DES and HMAC-MD5.
...
clause specifies the protection using IPsec as specified in RFC 4301 [53] with
...
based on the IPsec IKE (RFC 4301 [53], RFC 2407 [47], RFC 2408 [48] and RFC
2409 [49]) protocols.
...
the HTTP CONNECT method (see RFC 2817 [62]) to request the proxy to establish
...
protocol together with an enhanced version of TURN. ICE is defined in RFC
5245[58] and is a protocol for performing NAT traversal of UDP based media
streams. ICE in turn makes use of TURN, defined in RFC 5766 [60], which is a
protocol for relaying media through a relay server. An IMS client that
supports ICE will allocate relayed candidates at the TURN server and include
the candidate information in the SDP offer/answer sent to the peer. The
relayed candidates will be used as a last resort when the client and peer fail
to establish a direct communication path. The communication between the client
and the TURN server (this includes both the relayed media and the control
information needed to setup the relayed candidates) can occur over UDP, TCP or
TCP/TLS. By using TCP/TLS on port 443 (HTTPS) or TCP on port 80 (HTTP) the
communication will appear as HTTP(S) to firewalls and will (typically) be
allowed through. Using TCP instead of TLS/TCP reduces the overhead but will
fail when the firewall performs DPI or if an HTTP proxy is present. An IMS
client may be configured to use both TURN over TCP/80 and TURN over TLS/443,
in such case, the client should prefer to use TURN over TCP/80 to avoid TLS
overhead.
ICE and TURN have later on been extended to also support TCP based media. ICE
TCP is defined in RFC 6544 [59] and TURN TCP is defined in RFC 6062 [61]. One
of the changes introduced in TURN TCP is that the multiple TCP connections are
established between the client and TURN server: one for exchange of control
information and one for each relayed TCP based media stream. All UDP based
media streams are relayed over the same TCP connection that is used for the
control information, just as in the original TURN protocol. The TURN server
will use TCP/TLS on port 443 (HTTPS) or TCP over port 80 (HTTP) for all the
connections. In order to reduce the TLS setup time when several TCP
connections are established, the IMS client and TURN server may use the TLS
session resumption feature.
An IMS client that is configured to use an HTTP proxy uses the HTTP CONNECT
method (see RFC 2817 [62]) to request the proxy to establish a TCP connection
...
\- support the HTTP CONNECT method in RFC 2817 [X5] for establishing the TLS
...
\- support the HTTP CONNECT method in RFC 2817 [62] for establishing TLS
...
without changes.
NOTE 1: The IMS-ALG is invoked by the P-CSCF, IBCF, or ISC to handle the case
when the remote endpoint lacks support of ICE. The IMS-ALG in turn inserts the
IMS-AGW on the media path.
NOTE 2: IMS-AGW may be inserted in other cases as well, e.g. for hosted NAT.
...
(see RFC 2617 [12]) is stored in the WIC, the password needs to be re-obtained
...
supports the Authorization header defined in RFC 2617 [13], for example SIP
and HTTP.
...
## X.5.1 Introduction
TURN RFC 5766 [61] specifies that TURN servers and clients MUST implement
\"Long-Term Credential Mechanism\" as specified in clause 10.2 of RFC 5389
[69]. In this mechanism, the client and server share a pre-provisioned
...
CSCF
### X.5.2.1 Overview
This solution reuses the TURN long-term credential method defined in RFC 5766
[61] , but the credential is dynamically provisioned by eP-CSCF via the
...
minor change to existing functions. It addresses the security issues in [69]
by dynamically generating TURN user name and password. This solution is
optional to support. When to use this solution depends on WebRTC deployment
...
INTEGRITY value of TURN Allocate request as defined in IETF RFC 5766 [61].
...
\- WAF: WAF has to support extensions as defined by the IETF RFC 7635 [70].
WAF generates a self-contained access token and other required parameters
(TURN session key and key id) in compliance with the IETF RFC 7635 [70]. The
WAF and the TURN server share a long-term secret K. This key is used by the
WAF to generate additional keys for encrypting the access token and ensuring
message integrity of the message. Additional details on this can be found in
the IETF RFC 7635 [70]. The provisioning of the long-term secret key in WAF
and TURN server is out of scope of this solution.
...
compliant with the extensions defined in the IETF RFC 7635 [70].
### X.5.3.2 Procedures
...
GET request is defined in [IETF RFC 7635]{.underline} [70].
**5\. WAF validates WWSF and issues access token**
WAF authenticates WWSF and generates access token, session key and key id
(kid) according to [IETF RFC 7635]{.underline} [70]. Access token is
structured according to the format in [IETF RFC 7635]{.underline} [70].
...
third party authorization as defined in [IETF RFC 7635]{.underline} [70]. A
relayed transport address is allocated in the TURN server.
