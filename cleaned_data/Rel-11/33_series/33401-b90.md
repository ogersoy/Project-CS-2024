...
derive the K~eNB~* as normal (see clause A.5). The eNB shall take the K~eNB~
it received from the MME, which is equal to the temporary K~eNB,~ as basis for
...
a 128-bit integritykey K~UPint~ as KEY, a 5-bit bearer identity BEARER which
value is assigned as specified by TS 36.323 [12], the 1-bit direction of
...
This subclause covers ths case that the UE include a mapped GUTI into the
...
SRVCC.
2For UMTS and GSM subscribers, the source MSC server enhanced for SRVCC shall
generate a NONCE~MSC~.
...
> using the key derivation function as specified in annex B.6 of TS 33.102 [2]
> . The source MSC server enhanced for SRVCC shall further set the KSI\'~PS~
> equal to the KSI~CS~ associated with the latest key set as specified for
...
> enhanced for SRVCC shall furth er set the CKSN\'~PS~ equal to the CKSN~CS~
...
> request.
NOTE 1a: The MSC server enhanced for SRVCC does not include any authentication
...
NOTE 1b: The MSC server enhanced for SRVCC does not include any UE security
...
NOTE 1c: The naming of the KSI~SGSN~ hints at that this identifier is somehow
...
to the NAS key derivation functions (see Annex A.7) . MME generates
NONCE~MME~. MME shall include KSI~SGSN~, NONCE~MME~, the selected NAS security
...
6.The target eNB shall select the AS algorithms (including ciphering for both
RRC and UP, and integrity protection for RRC) which have the highest priority
from its configured list and is also present in the UE EPS security
...
NOTE XXXX: The handover command in the target to source transparent container
is not security protected by the target eNB.
...
NOTE 4: The CS to PS HO command is integrity protected and optionally ciphered
...
NOTE 1d: Since the MME denies access to E-UTRAN for GSM subscribers, the UE
...
(cf. subclause 3.1) EPS security context at AS The MME and ME shall NAS level
and overwrite any existing current mapped EPS security context with the newly
...
security context cannot be used in the future. The MME and ME shall in this
case delete the new mapped EPS security context.
