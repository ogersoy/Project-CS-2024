...
the selected codec is the same as received from the 3GPP node to avoid
...
Subsystem. Where an \"IWU\" is mentioned in those procedures, this shall be
understood as MGCF.
...
as specified in 3GPP TS 29.163 [13] subclause 7.4.24.
...
in 3GPP TS 29.163 [13] subclause 7.5.13.2.
...
specified in 3GPP TS 29.163 [13] subclause 7.5.13x.3.
...
specified in 3GPP TS 29.163 [13] subclause 7.5.13.4.
## 7.8 MGCF -- IM-MGW Interaction
### 7.8.1 General
The present subclause describes signalling procedures that are applicable at
the Mn interface when the MGCF and attached IM-MGW interwork between an IM CN
...
extensions as detailed in 3GPP TS 29.332 [54].
### 7.8.2 Call related Procedures for Terminations towards the IM CN Subsystem
Subclause 9.3.1 in 3GPP TS 29.163 [13] shall apply.
### 7.8.3 Call related Procedures for Terminations towards SIP-I based
Circuit-switched Core Network
Those procedures defined in 3GPP TS 23.205 [9] that are listed in table
7.8.3.1 shall apply.
Table 7.8.3.1: Call related Procedures defined in 3GPP TS 23.205 [9] that are
applicable at terminations towards SIP-I based circuit-switched core network
+----------------------------------+----------------------------------+ | Procedure defined in 3GPP TS | Remarks | | 23.205 [9] | | +----------------------------------+----------------------------------+ | Change Flow Direction | NOTE 1 in that subclause is | | | replaced by: | | | | | | NOTE 1: This procedure may be | | | combined with the \"Reserve RTP | | | Connection Point\" procedure, | | | the \"Configure RTP Connection | | | Point\" procedure, or the | | | \"Reserve and Configure RTP | | | Connection Point\" procedure. | +----------------------------------+----------------------------------+ | Change Through-Connection | The NOTE in that subclause is | | | replaced by: | | | | | | NOTE 2: This procedure may be | | | combined with the \"Reserve RTP | | | Connection Point\" procedure, | | | the \"Configure RTP Connection | | | Point\" procedure, or the | | | \"Reserve and Configure RTP | | | Connection Point\" procedure. | +----------------------------------+----------------------------------+ | Bearer Released | | +----------------------------------+----------------------------------+ | Release Termination | | +----------------------------------+----------------------------------+ | Send Tone | | +----------------------------------+----------------------------------+ | Stop Tone | | +----------------------------------+----------------------------------+ | Play Announcement | Optional | +----------------------------------+----------------------------------+ | Stop Announcement | Optional | +----------------------------------+----------------------------------+ | Announcement Completed | Optional | +----------------------------------+----------------------------------+ | Tone Completed | | +----------------------------------+----------------------------------+ | Activate Voice Processing | Optional | | Function | | +----------------------------------+----------------------------------+ | Termination heartbeat indication | | +----------------------------------+----------------------------------+ | Reserve RTP Connection Point | | +----------------------------------+----------------------------------+ | Configure RTP Connection Point | | +----------------------------------+----------------------------------+ | Reserve and Configure RTP | | | Connection Point | | +----------------------------------+----------------------------------+
### 7.8.4 Non-call related procedures
Subclause 9.3.4 in 3GPP TS 29.163 [13] shall apply.
...
profile as defined by 3GPP TS 29.232 [55] shall provide the required
functionality (described in subclause A.8). If CS-IBCF is physically separated
then the CS-IBCF control function is connected to the CS-TrGW via the CS-Ix
reference point (described in subclause A.7), which is realised by the Ix
profile as defined by 3GPP TS 29.238 [29].
# A.4 Void
...
described in subclause A.5.3.6; and
...
interface type associated with the RTP bearer termination as required by 3GPP
TS 32.407 [44]. The provision of these statistics is outside of the scope of
this specification.
...
This subclause considers the message flow for the CS-IBCF and CS-TrGW, when IP
...
The call is established as described in subclause A.7.1.2.1.1. With reference
to the steps in figure A.7.1.2.1.1, the following additions are required,
...
This subclause considers the message flow for the CS-IBCF and CS-TrGW, when
the CS-IBCF is acting as an Entry point and remote source address filtering is
...
The call is established as described in subclause A.7.1.2.1.1.With reference
to the steps in figure A.7.1.2.1.1.1, the following additions are required,
specific to this use case:
...
This subclause considers the message flow for the CS-IBCF and CS-TrGW, when
the CS-IBCF requires the use of differentiated services (see IETF RFC 2474
[41]).
...
This subclause considers the message flow for the CS-IBCF and CS-TrGW, when
the two networks are in different IP realms. IP realms are described in 3GPP
TS 23.205 [9] clause 20.
...
Realm Availability Procedure (see subclause A.7.2.3), if this procedure is
...
parameters as detailed in subclause A.7.1.2.2.8.2.
If such policing is requested, the CS-TrGW shall police the corresponding
media streams as detailed in subclause A.7.1.2.2.8.2, by measuring the data
rate for the received packets within that media stream. If the permissible
data rate provided by the CS-IBCF is exceeded, the CS-TrGW shall discard
packets to reduce their data rate to the permissible data rate.
...
This procedure is identical to that of subclause A.7.1.2.1.1apart from the CS-
...
This subclause describes the logical signalling procedures (i.e. message
identifiers are not part of the protocol) between the CS-IBCF and CS-TrGW. The
procedures within this subclause are intended to be implemented using the
...
handling component of the Ix interface. Where a corresponding procedure in
3GPP TS 23.205 [9] is specified, that procedure shall apply. Where no
corresponding procedure in 3GPP TS 23.205 [9] is specified, the procedure is
specified in a subclause of this specification.
Table A.7.2.3.1.1: Non-call related procedures
+----------------------+----------------------+----------------------+ | Procedure defined in | Corresponding | Remarks | | | Procedure defined in | | | 3GPP TS 29.238 | 3GPP TS 23.205 [9] | | | [29] | | | +----------------------+----------------------+----------------------+ | TrGW Out of service | MGW Out of Service | | +----------------------+----------------------+----------------------+ | TrGW Communication | MGW Communication Up | | | Up | | | +----------------------+----------------------+----------------------+ | TrGW Restoration | MGW Restoration | | +----------------------+----------------------+----------------------+ | TrGW Register | MGW Register | | +----------------------+----------------------+----------------------+ | TrGW Re-register | MGW Re-register | | +----------------------+----------------------+----------------------+ | CS-IBCF Ordered | (G)MSC Server | | | Re-register | Ordered Re-register | | +----------------------+----------------------+----------------------+ | CS-IBCF Restoration | (G)MSC Server | | | | Restoration | | +----------------------+----------------------+----------------------+ | CS-IBCF Out of | (G)MSC Server Out of | | | Service | Service | | +----------------------+----------------------+----------------------+ | Termination | Termination | The \"Termination | | Out-of-Service | Out-of-Service | Out-of-Service | | | | procedure\" is also | | | | used as a | | | | call-related H.248 | | | | command | +----------------------+----------------------+----------------------+ | Audit Value | Audit Value | | +----------------------+----------------------+----------------------+ | Command Rejected | Command Rejected | The \"Command | | | | Rejected\" procedure | | | | may be used in | | | | response both to | | | | call-related and | | | | non-call-related | | | | H.248 Commands. | +----------------------+----------------------+----------------------+ | TrGW Capability | Capability Update | | | Change | | | +----------------------+----------------------+----------------------+ | TrGW Resource | MGW Resource | | | Congestion Handling | Congestion Handling | | | -- Activate | -- Activate | | +----------------------+----------------------+----------------------+ | TrGW Resource | MGW Resource | | | Congestion Handling | Congestion Handling | | | -- Indication | -- Indication | | +----------------------+----------------------+----------------------+ | Inactivity timeout | Inactivity timeout | | | activation | activation | | +----------------------+----------------------+----------------------+ | Inactivity timeout | Inactivity timeout | | | indication | indication | | +----------------------+----------------------+----------------------+ | Realm Availability | | See subclause | | Change Activate | | A.7.2.3.2. | +----------------------+----------------------+----------------------+ | Realm Availability | | See subclause | | Change Indication | | A.7.2.3.3. | +----------------------+----------------------+----------------------+
...
Reserve RTP Connection Point as defined by 3GPP TS 23.205 [9] is used.
\- Instead of the procedure Reserve and Configure TrGW Connection Point the
procedure Reserve and Configure RTP Connection Point as defined by 3GPP TS
23.205 [9] is used.
\- Instead of the procedure Configure TrGW Connection Point the procedure
Configure RTP Connection Point as defined by 3GPP TS 23.205 [9] is used.
...
corresponding subclause of this Annex shall be applied.
...
See 3GPP TS 23.226 [45], 3GPP TS 26.114 [52], 3GPP TS 26.235 [50] and 3GPP TS
26.236 [51].
