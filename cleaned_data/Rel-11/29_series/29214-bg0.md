...
requests an AF session termination in a ST-Request command, the PCRF shall
...
INVITE/UPDATE requests that it forwards.
...
with the response confirmation before forwarding it.
...
reply to the SIP session termination request (at the terminating side).
