...
{width="6.690277777777778in" height="2.4381944444444446in"}
Figure 4.6.1.1: TDF Session Establishment in case of solicited reporting
  1. PCRF initiates a session towards the TDF. The PCRF provisions the applicable ADC Rules for the corresponding TDF session by sending a Diameter TS-Request to the TDF, including user identity information, the UE Ipv4 address and/or UE Ipv6 prefix and, if available, PDN identifier, IP-CAN type, RAT type and additional parameters as defined in subclause 4b.5.1.1 of 3GPP TS 29.212 [9]. PCRF may also subscribe to the Event Triggers (e.g. APPLICATION_START and APPLICATION_STOP).
...
on 73vailabi simultaneous traffic flows for the same Ipv6 prefix (i.e. two or
...
TDF reports the failure to the PCRF as defined in sub clause 4b.5.5 of 3GPP TS
29.212 [9].
...
6.2.2.2 and the IP-CAN specific annexes of 3GPP TS 23.203 [4]).
...
interface in either the Frame-IP-Address AVP or the Framed-Ipv6-Prefix AVP
with the Ipv4 address or Ipv6 prefix received via the Gx interface. The UE
Identity if present in the Subscription-Id AVP and the PDN information if
available in the Called-Station-Id AVP may also assist on this association.
The Domain identity if available in the IP-Domain-Id AVP may also assist on
this association, e.g. if other user identity information than the UE IP
address is unavailable and the UE IP addresses is not unique for a certain
APN.
...
the MPS session according to clauses 4.5.x.1.3 and 4a.5.x.1.3 in 3GPP TS
29.212 [9] for Gx/Gxx respectively.
...
NOTE: The PCRF provides the appropriate ARP/QCI for both PCC Rules and default
bearer QoS so that the PCEF can perform a valid bearer binding.
...
the same change to the ARP/QCI for all PCC/QoS Rules bound to the same bearer,
modify the bearer ARP/QCI as requested.
...
NOTE: For GPRS and EPS, the state of TFT packet filters, as defined in 3GPP TS
...
is used as SDI, the AF should apply the mapping described in Clause 6.2. If
MPD (3GPP TS 26.247 [30]) is used, the AF may apply the mapping described in
Annex X in 3GPP TS 26.247 [30]. For IMS, the mapping rules in Clause 6.2 shall
be used at the P-CSCF. The AF passes service information to the PCRF over the
Rx interface. Clause 6.2 specifies the QoS parameter mapping functions at the
...
and the selected PCRF address for a certain IP-CAN Session.
NOTE 1 : The DRA derives the PCEF identity from the Origin-Host AVP of the CCR
command received from the PCEF. The DRA finds the correct PCRF by matching the
user identity (if available), IPv4 address or IPv6 address/prefix (if
available) and APN (if available) received in the message from the
BBERF/PCEF/AF/TDF/V-PCRF with the corresponding information stored the DRA.
...
 If the PCRF routing information is per IP-CAN session, the DRA shall set the
Redirect-Host-Usage AVP to ALL_SESSION. The DRA client shall contact the DRA
on IP-CAN session termination.
...
sent over Rx as defined in 29.214 [10].
...
The IMS calls out procedures for NAT traversal for media and 152vailabili
...
procedures in subclause C.2.1), the P-CSCF shall, within the service
information sent to the PCRF:
...
Evolved ARP to Rel-99 ARP as specified in subclause B.3.3.3 of 3GPP TS 29.212
[9].
...
in subclause 4.1.5 of 3GPP TS 23.402 [21].
...
# E.4 PCC Procedures
## E.4.1 Introduction
From the network scenarios listed in subclause 4.0, in order to support
...
traffic. This procedure is same as described in subclause 4.1 with the
exceptions described in this clause.
...
The PCRF provides the data as described in 3GPP TS 29.215[22], subclause
A.5.1.1
...
described in 3GPP TS 29.215[22], subclause A.6.1.1.1 or A.6.3.1.0.
...
The V-PCRF provides the data as described in 3GPP TS 29.215[22], subclause
A.5.1.1.
...
> 3c. The BPCF acknowledges to the V-PCRF by sending the TEA/RAA command.
  1. For the non-roaming case, triggered by step 2, the BPCF initiates an S9a session establishment with the PCRF by sending a CCR to the PCRF with the CC-Request-Type AVP set to the value INITIAL_REQUEST. The BPCF provides the data as described in 3GPP TS 29.215[22], subclause A.5.1.2.
...
Figure E.4.2.2.1: IP-CAN Session Establishment for NSWO traffic
  ...
  2. For the non-roaming case, the BPCF initiates an S9a* session establishment procedure with the H-PCRF by sending a CCR using the CC-Request-Type AVP set to the value INITIAL_REQUEST. The BPCF shall provide the data as described in 3GPP TS 29.215 [22], subclause A.5.1.2.1.
...
termination is initiated as defined in subclause 4.6.2. For this case, the
PCRF described in subclause 4.6.2 acts as a V-PCRF.
...
theBPCF.
  1. If there is an active TDF session between TDF and H-PCRF, the TDF session termination is initiated as defined in subclause 4.6.2.
...
traffic
  1. Step 1 to 3 as specified in figure 4.3.1.1.1 are executed. The H-PCRF receives an internal or external trigger to update PCC/QoS Rules. External trigger in Step 1 in figure 4.3.1.1.1 only applies to AF session interactions (as described in subclause 4.3.1.2) and TDF session interactions (as described in subclause 4.3.1.2). For V-PCRF initiated IP-CAN session modification, the V-PCRF always asks the H-PCRF for policy decision.
...
PCRF-Initiated IP-CAN Session Modification for EPC routed traffic.
  1. The H-PCRF sends a Diameter RAR to update the QoS information to the BPCF.
If the UE is roaming, then steps 2a \~ 2c are executed instead of step 2:
...
Diameter CCR to the H-PCRF to report QoS Rule failure.
  1. The H-PCRF sends a Diameter CCA to the BPCF to acknowledge the CCR command.
...
information to the PCRF.
  1. The H-PCRF sends a Diameter RAR to request that the BPCF installs, modifies or removes QoS Rules (e.g. SDF template, QCI, ARP, MBR and GBR).
When the UE is in roaming case, steps 2a \~ 2c are executed instead of step 2:
2a. For Visited Access scenario, the H-PCRF may provision the PCC rule(s) to
the V-PCRF. For Home Routed scenario the H-PCRF may provision the QoS rule(s)
...
traffic
  1. Step 1 to 3 as specified in figure 4.3.1.1.1 are executed. The H-PCRF receives an internal or external trigger to update PCC Rules for NSWO traffic (e.g. upon a request from the AF, a request from the TDF in the non roaming case or due to operator policies). External trigger in Step 1 in figure 4.3.1.1.1 only applies to AF session interactions (as described in subclause 4.3.1.2), TDF session interactions (as described in subclause 4.3.1.1) and SPR subscription data modification (as described in subclause 4.3.1.1).
...
necessary.
2c. The V-PCRF sends a Diameter RAR to the BPCF to update the QoS information.
  1. The BPCF shall perform QoS validation and translates the PCC rule as received (i.e. SDF template, QCI, MBR, GBR and ARP) into access specific QoS parameters applicable in the Fixed Broadband Access.
NOTE: The detail of the mapping from 3GPP QoS parameters on S9a to QoS
parameters applicable in the Fixed Broadband Access is out of 3GPP scope.
4\. The BPCF sends RAA to the H-PCRF to acknowledge the RAR and informs the
...
> RAR command to the HNB GW and includes the information as defined in
> subclause E.5.3.2.2 of 3GPP TS 29.212.
>
...
# E.6 PCRF Addressing
## E.6.1 General
PCRF discovery at the DRA shall be done according to subclause 7.1 with the
...
For the EPC-routed scenario, DRA is defined as specified in subclause 7.2 with
...
subclause 7.3.1 with the additions described in this subclause. The V-PCRF
...
according to subclause 7.3.3.
### E.6.3.3 Redirect DRA
Redirect DRA is specified in subclause 7.3.4 with the additions described in
this subclause.
...
information.
### E.6.3.4 Proxy DRA
Proxy DRA is specified in subclause 7.3.5 with the additions described in this
subclause.
...
The BPCF may also use the DRA procedures as described in subclause 7.3. In
order to do so, the BPCF shall provide the DRA of the PCRF realm with identity
...
described in subclause 7.3. In order to do so, the BPCF shall provide the DRA
of the PCRF realm with identity parameters during the S9a* Session
Establishment procedure. The identity parameters from the BPCF shall comprise
the UE local Ipv4 adress or UE local Ipv6 prefix/address in the UE-Local-IP-
Address AVP or UE-Local-IP-Prefix AVP, the APN in the Called-Station-Id AVP
and user identity in the Subscription-Id AVP.
...
If proxy agent is used for DRA, the DRA should use the proxy procedure as
specified in IETF RFC 3588 [14]. For PA2 solution (described in subclause 7.1)
...
the HPLMN according to subclause 7.3.8.
...
PCRF selection by the AF shall be done according to subclause 7.3.7.
PCRF selection by the TDF shall be done according to subclause 7.3.9.
...
procedures as described in subclause 7.3.8 to address the H-PCRF. In order to
do so, the V-PCRF shall provide the DRA of the H-PCRF realm with identity
...
the non-roaming case in the subclauses 7.4.1 and 7.4.2 are applied with the
following exception:
...
roaming case in subclauses 7.4.1 and 7.4.2 are applied with the following
exception:
...
case.
The flows in subclauses E.6.4.2 and E.6.4.3 are applicable to the EPC-routed
...
roaming and roaming case in the subclauses 7.4.1 and 7.4.2 are applied with
following exception:
...
procedure.
2\. If the V-DRA is deployed in the VPLMN as indicated in step 6 of subclause
...
6\. The V-PCRF-1 returns a TEA 197vailab.
...
using DRA (Proxy), which is described in subclause E.6.4.2.2.
