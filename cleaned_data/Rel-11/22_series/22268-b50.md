...
behaviour in response to some or all Warning Notifications. Depending on the
...
NOTE 2: Once the HPLMN operator is aware that the PWS-UE is in a VPLMN where
the integrity of Warning Notifications is known to be compromised, over the
air device management mechanisms may be used to change the VPLMN-related
setting on the USIM.
