...
A-UE B session establishment is sent to the S‑CSCF.
17\. The S‑CSCF routes the request onwards to UE C.
18.-21. UE C sends back session progress messages to the ICS UE B via S‑CSCF
and SCC AS.
...
25\. The S‑CSCF sends the NOTIFY to the SCC AS as it was inserted at session
establishment.
...
INVITE to S‑CSCF.
2\. The S‑CSCF sends the INVITE to the SCC AS as it was inserted at session
establishment.
...
ICS UE A via S‑CSCF and SCC AS.
...
24.173 [8].
12\. The S‑CSCF sends the REFER to the SCC AS as it was inserted at session
establishment.
...
10\. The ICS UE A sends a Hold message to the S‑CSCF as specified in TS 23.228
[2].
...
in TS 24.173 [8], and received at the S‑CSCF of the ICS UE C.
3\. The S‑CSCF executes terminating initial filter criteria and forwards the
INVITE message to the SCC AS.
...
3\. The MSC Server sends the Hold message to the S‑CSCF indicating the session
...
4\. The S‑CSCF sends the Hold message to the SCC AS as it was inserted at
session establishment.
...
9\. The MSC Server sends a Resume message to the S‑CSCF indicating the session
...
3\. The SCC AS sends the INVITE to the S‑CSCF.
4\. The S‑CSCF forwards the INVITE to the MSC Server based on the contact
...
2\. The MSC Server sends a REFER to the S‑CSCF. The REFER indicates that UE B
is to be transferred to UE C and that this new session is to replace the
current session between UE A and UE B.
3\. The S‑CSCF sends the REFER to the SCC AS as it was inserted at session
establishment.
...
13\. The S‑CSCF sends the NOTIFY to the SCC AS as it was inserted at session
establishment.
...
4\. The S‑CSCF sends the REFER to the SCC AS as it was inserted at session
establishment.
5\. The SCC AS sends the REFER to the S‑CSCF within the dialog created for the
...
12\. The S‑CSCF continues with originated session processing as specified in
...
16 The S‑CSCF sends the NOTIFY to the SCC AS as it was inserted at session
establishment.
...
13\. The S‑CSCF continues with originated session processing as specified in
TS 23.228 [2] and routes the request onwards to UE C.
...
defined for MMTel.
The MSC Server enhanced for ICS shall not subscribe to the conference related
events in IMS.
NOTE 2: The MSC Server enhanced for ICS and hence also the UE are not notified
if a remote party leaves the conference.
...
When using procedures as defined in TS 24.010 [30], and not using MSC Server
enhanced for ICS, the following apply:
...
Figure 7.6.2.11.2-1 describes a call flow via an MSC Server not enhanced for
ICS. CCNR and CCNL follow the same principles.
Figure 7.6.2.11.2-1: IMS Communication Completion via MSC Server enhanced for
ICS
There is an ongoing session between UE A and UE B.
...
1\. The S‑CSCF decides the session should be terminated due to administrative
...
initiated using I1
1\. UE A terminates the session by sending a SIP BYE (via its own S‑CSCF)
towards the I/S‑CSCF of ICS UE B.
2\. The S‑CSCF sends a SIP BYE message to the SCC AS.
3\. The SCC AS sends a release message over I1 to the ICS UE.
4\. The ICS UE sends a release-ack to the SCC AS over I1.
5\. The SCC AS sends a SIP OK to the I/S‑CSCF.
...
S‑CSCF. Step 4 does not have to wait for step 3 to complete.
...
video media to the ongoing speech session to the S‑CSCF.
4\. The S‑CSCF sends the re-INVITE to the SCC AS which has been inserted at
session establishment.
5\. The SCC AS sends the re-INVITE to the S‑CSCF.
...
the ongoing speech session to the S‑CSCF.
...
media to the ongoing speech session to the S‑CSCF.
2\. The S‑CSCF sends the re-INVITE message to the SCC AS.
...
1\. The ICS UE A sends a re-INVITE message that offers the adding of the video
media to the ongoing speech session to the S‑CSCF.
2\. The S‑CSCF sends the re-INVITE message to the SCC AS which has been
inserted at session establishment.
3-4. The SCC AS sends 200 OK to the ICS UE A via the S‑CSCF.
5\. The ICS UE A sends a CS call disconnect message to the MSC server to
release the existing CS access leg.
6\. The MSC server sends the BYE message to the S‑CSCF.
7\. The S‑CSCF forwards the BYE message to the SCC AS.
...
10\. The MSC server sends an INVITE message to the S‑CSCF.
11\. The S‑CSCF sends the INVITE to the SCC AS.
12\. The SCC AS sends a re-INVITE message to the S‑CSCF that offers the adding
of the video media to the ongoing speech session to update the remote leg.
13\. The S‑CSCF sends the re-INVITE to UE B.
...
7\. The MGCF sends a BYE message to the S‑CSCF.
...
12\. The MGCF sends an INVITE message to the S‑CSCF.
...
release the existing CS access leg.
4\. The MSC server sends the BYE message to the S‑CSCF.
5\. The S‑CSCF forwards the BYE message to the SCC AS.
...
a new CS access leg supporting video and speech media.
8\. The MSC server sends the INVITE to the S‑CSCF.
9\. The S‑CSCF sends the INVITE to the SCC AS.
10\. The SCC AS sends a re-INVITE to the S‑CSCF that offers the adding of the
video media to the ongoing speech session to update the remote leg.
11\. The S‑CSCF sends the re-INVITE to UE B.
...
generated by S‑CSCF or other functional entities in the IMS network, for the
...
**As driven by operator policy and VPLMN support.** **Yes, only if I1 is
supported** **Yes** **Yes**
* * *
...
**As driven by operator policy and VPLMN support.** **Yes, if I1 is
supported** **Yes** **Yes**
* * *
