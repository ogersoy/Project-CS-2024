...
[30] Broadband Forum WT-203: \"Interworking between Next Generation Fixed and
...
[33] Broadband Forum WT-291: \"Nodal Requirements for Interworking between
...
control and, for a PCEF enhanced with ADC, application detection and control.
...
for a PCEF enhanced with ADC, for application detection and control.
...
usage monitoring thresholds shall be based on volume. The PCEF or TDF shall
notify the PCRF when a threshold is reached and report the accumulated usage
since the last report for usage monitoring.
...
ofservice data flow(s), or for all traffic of an IP-CAN session in the PCEF.
...
application traffic and to apply the specified enforcement actions.
...
or stop event to the PCRF by activating the appropriate ADC rules in the TDF
or PCC rules in the PCEF enhanced with ADC. Reporting notifications of start
and stop of application detection to the PCRF may be muted, in addition, per
specific ADC or PCC rule. The PCRF may, in a dynamic ADC or PCC rule, instruct
the TDF or PCEF enhanced with ADC, what enforcement actions to apply to the
detected application traffic. The PCRF may activate application detection only
...
NOTE: Redirection may not be possible for all types of detected application
...
bandwidth limitation as described above.
For the solicited application reporting, it is the PCRF\'s responsibility to
...
(e.g. SDFs, pplicationidentifier, etc.).
...
traffic detection and enforcement at the TDF;
...
NOTE 1: The enforced PCC rule request and the enforced ADC rule request
...
provided by the PCRF, are reached, the PCEF or TDF (whichever received the
event trigger) shall report this event to the PCRF.
...
Report to the PCRF.
NOTE 2: At PCC rule deactivation the User Location Report includes information
...
Report is set and the user location (i.e. cell) is not available to the ERF,
...
subscription information.
The PCRF shall decide how certain service data flow/detected application
traffic shall be treated in the PCEF and in the TDF, if applicable, and ensure
that the PCEF user plane traffic mapping and treatment is in accordance with
...
RAT type change, SGSN change, Serving GW change.
...
requested node (PCEF or TDF) shall report to the PCRF. The PCRF shall decide
if and when to activate usage monitoring to the PCEF and TDF.
...
node (PCEF or TDF) containing the accumulated volume usage since the time of
...
roaming agreement.
NOTE 1: Proxying the Gateway Control Session Establishment makes the H‑PCRF
...
6.2.1.3.3 (i.e. no S9 is used). In such case, the PCRF in the VPLMN has no
access to subscriber policy information from the HPLMN, only static policies
will apply based on roaming agreements. The VPCRF may also interact with the
AF in the VPLMN in order to generate PCC Rules for services delivered via the
VPLMN. V-PCRF uses locally configured policies according to the roaming
agreement with the HPLMN operator as input for PCC Rule generation.
...
The PCEF shall discard a packet in case there is no service data flow template
...
verification. For the case that a BBERF is present, uplink bearer binding
verification is done by the BBERF.
NOTE 3: If a PCC Rule containing an Application Identifier inspects traffic on
...
type is applicable to the PCC rule.
...
usage report for the affected Monitoring keys, including the resources
consumed before and after the Monitoring time was reached, if provided
according to clause 6.2.1.0.
...
Monitoring key.
When the Monitoring time occurs, the accumulated volume usage shall be
recorded by the PCEF and:
...
the volume usage up to the Monitoring time and volume usage after the
Monitoring time.
#### 6.2.2.4 QoS control
...
For session termination, the communication between the AF and the PCRF shall
...
perform enforcement actions and usage monitoring.
...
rule.
The _Service Identifier Level_ _Reporting_ indicates whether the PCEF shall
...
may include service data flow informationfilters (if possible to provide) and
the application instance identifier associated with the service data flow
filters.
...
* * *
Attribute Description PCRF permitted to modify the attribute Scope Event
trigger Defines the event(s) that shall cause a re-request of ADC rules for
the TDF session. Yes TDF session ADC Revalidation time limit Defines the time
period within which the TDF shall perform an ADC rules request. Yes TDF
session
* * *
...
+----------------+----------------+----------------+----------------+ | Information | Description | Category | Scope | | name | | | | +----------------+----------------+----------------+----------------+ | Monitoring key | The PCRF uses | Mandatory | IP-CAN | | | the monitoring | | session, | | | key to group | | | | | services that | | TDF session | | | share a common | | | | | allowed usage. | | | +----------------+----------------+----------------+----------------+ | Volume | Defines the | Optional | Monitoring key | | threshold | traffic volume | | | | | value after | | | | (NOTE 1) | which the PCEF | | | | | or the TDF | | | | | shall report | | | | | usage to the | | | | | PCRF for this | | | | | monitoring | | | | | key. | | | +----------------+----------------+----------------+----------------+ | Monitoring | Defines the | Optional | Monitoring Key | | time | time at which | | | | | the PCEF or | | | | | the TDF shall | | | | | reapply the | | | | | Volume | | | | | Threshold. | | | +----------------+----------------+----------------+----------------+ | Subsequent | Defines the | Optional, | Monitoring Key | | Volume | traffic volume | Conditional | | | threshold | value after | | | | | which the PCEF | (NOTE 2) | | | | or the TDF | | | | | shall report | | | | | usage to the | | | | | PCRF for this | | | | | Monitoring key | | | | | for the period | | | | | after the | | | | | Monitoring | | | | | time. | | | +----------------+----------------+----------------+----------------+ | NOTE 1: This | | | | | attribute is | | | | | also used by | | | | | the PCEF, e.g. | | | | | during IP-CAN | | | | | session | | | | | termination, | | | | | to inform the | | | | | PCRF about the | | | | | resources that | | | | | have been | | | | | consumed by | | | | | the UE. | | | | | | | | | | NOTE 2: This | | | | | attribute is | | | | | applicable in | | | | | presence of | | | | | Monitoring | | | | | Time only. | | | | +----------------+----------------+----------------+----------------+
The _Monitoring Key_ is the reference to a resource threshold. Any number of
...
active and a volume threshold value for the IP‑CAN session has been provided.
\- For TDF session level monitoring at the TDF, a TDF session is active and a
volume threshold value has been provided.
For usage monitoring on Monitoring key level at the PCEF or the TDF the
following conditions are applicable:
\- A volume threshold has been provided for a Monitoring key to the PCEF and
there is at least one PCC rule activated for the IP-CAN session that is
associated with that Monitoring key.
\- A volume threshold has been provided for a Monitoring key to the TDF and
there is at least one ADC rule at the TDF activated for the TDF session that
is associated with that Monitoring key.
...
\- apply enforcement actions for the application traffic detected by the rule.
...
NOTE 3: The same application identifier value could be used for a dynamic ADC
...
defined in clause 7.7.1 (applicable to case 2a during initial attach and to
case 2b, as defined in clause 7.1).
...
request and assigns an IP address and (if requested) network prefix for the
user.
...
NOTE: In case of TDF and solicited application reporting, either PCEF informs
PCRF with TDF IP address, or PCRF has it preconfigured per each one of PCEFs.
...
location/access network information, if available. The PCRF may also subscribe
to the Event Triggers, relevant for the TDF, according to table 6.2.
9\. The TDF sends an Ack (accept or reject of the ADC rule operation(s)) to
inform the PCRF about the outcome of the actions related to the decision(s)
received in step 8. The Ack may also include the list of Event Triggers to
report. The Event Triggers indicate to the PCRF what events to be forwarded
from the PCRF to the TDF, once PCRF gets the corresponding Event Report from
the PCEF/BBERF.
10\. The PCRF sends the decision(s) including the chosen IP‑CAN bearer
...
11\. If online charging is applicable, and at least one PCC rule was
activated, the PCEF shall activate the online charging session, and provide
relevant input information for the OCS decision. Depending on operator
configuration PCEF may request credit from OCS for each charging key of the
activated PCC rules.
12\. If online charging is applicable, the OCS provides the possible credit
...
13\. If at least one PCC rule was successfully activated and if online
...
14\. If network control applies the GW may initiate the establishment of
additional IP-‑CAN bearers. See Annex A and Annex D for details.
15\. If the PCRF in step 10 has requested an acknowledgement based on PCC rule
...
NOTE 2: The GW(PCEF) may proceed to step 11 in parallel with the indication of
...
9\. If there is an active TDF session between TDF and PCRF, the PCRF
terminates the TDF session (s).
...
11\. The PCRF removes the information related to the terminated IP‑CAN Session
...
NOTE 3: Step 11 may be initiated any time after step 7.
12\. The GW (PCEF) continues the IP‑CAN Session removal procedure.
13\. If case 2a applies, the GW Control and QoS Rules Provision procedure as
...
14\. If online charging is applicable, the PCEF issues final reports and
returns the remaining credit to the OCS.
NOTE 4: Step 14 may be initiated any time after step 11.
15\. If online charging is applicable the OCS acknowledges that credit report
and terminates the online charging session.
16\. The PCRF sends a cancellation notification request to the SPR if it has
...
NOTE 5: Step 16 may be initiated any time after step 11.
17\. The SPR sends a response to the PCRF.
...
session.
11\. If there is an active TDF session between TDF and PCRF, the PCRF informs
...
termination.
NOTE 2: Step 13 may be initiated any time after step 6.
14\. If case 2a applies, the GW Control and QoS Rules Provision procedure as
defined in clause 7.7.4 may be initiated to remove the QoS rules associated
with the IP‑CAN session being terminated. This applies e.g. in case the
Gateway Control Session shall remain to serve other IP‑CAN sessions.
...
15\. If online charging is applicable, the GW issues final reports and returns
the remaining credit to the OCS.
NOTE 3: Step 15 may be initiated any time after step 13.
16\. If online charging is applicable the OCS acknowledges the credit report
and terminates the online charging session.
17\. The PCRF sends a cancellation notification request to the SPR if it has
subscribed such notification. If all IP-CAN sessions of the user to the same
APN are terminated, the PCRF stores the remaining usage allowance in the SPR.
NOTE 4: Step 17 may be initiated any time after step 8.
18\. The SPR sends a response to the PCRF.
...
termination of the transmission resources for a PCC Rule, the GW (PCEF)
reports this to the PCRF. If flow mobility applies, the GW (PCEF) may include
updated IP flow mobility routing information for any IP flows; the GW (PCEF)
also provides an indication if default route for the IP‑CAN session is
changed.
...
of an IPv4 address in step 5, the PCRF terminates the related TDF session.
12\. The TDF sends an Ack (accept or reject of the ADC rule operation(s)) to
inform the PCRF about the outcome of the actions related to the decision(s)
received in step 11. The Ack may also include the list of Event Triggers to
report. The Event Triggers indicate to the PCRF what events to be forwarded
from the PCRF to the TDF, once PCRF gets the corresponding Event Report from
the PCEF/BBERF.
13\. The PCRF sends an Acknowledge of IP‑CAN Session modification (PCC Rules,
...
14\. If online charging is applicable, the GW (PCEF) may request credit for
new charging keys from and/or shall issue final reports and return remaining
credit for charging keys no longer active to the OCS.
15\. If OCS was contacted, the OCS provides the credit information to the GW
(PCEF), and/or acknowledges the credit report.
16 The GW (PCEF) acknowledges or rejects any IP‑CAN Session signalling
received in step 3.
...
to case 1).
17\. The GW (PCEF) receives the response for the IP‑CAN Session signalling
request (applicable to case 1).
18\. The GW (PCEF) sends a Provision Ack (accept or reject of the PCC rule
operation(s)) to inform the PCRF about the outcome of the GW (PCEF) actions
related to the decision(s) received in step 13.
NOTE 3: For Cases 2a and 2b, the rejection of PCC rule operation can only
occur as a result of online charging interaction.
19\. Based on the result of PCC rule operations, the PCRF decides whether to
...
20\. If the AF requested it, the PCRF notifies the AF of related bearer level
...
For unsolicited application reporting, the TDF reports the same application
information to the PCRF unconditionally. The TDF establishes a new TDF session
if it detects an application for an IPv4 address or IPv6 address for which no
corresponding TDF session exists.
...
a TDF session modification (for solicited application reporting). For the TDF
...
terminate the TDF session towards the PCRF. If there is no active TDF session
yet between the TDF and the PCRF, the PCRF requests the TDF to establish the
TDF session towards PCRF and provides an ADC decision to the TDF. In case of
...
S9.
5a. For the TDF solicited application reporting, in the case of an existing
...
related to the received ADC decision(s). The Provision Ack / Ack may also
include the list of Event Triggers to report. The Event Triggers indicate to
the PCRF what events to be forwarded from the PCRF to the TDF, once PCRF gets
the corresponding Event Report from the PCEF/BBERF.
6\. If there is no Gateway Control and QoS Rules Reply pending and there is a
...
If there are multiple BBERFs associated with the IP‑CAN session, Step 5 is
...
7\. The PCRF sends the Policy and Charging Rules Provision (PCC Rules, Event
...
9\. If online charging is applicable, the PCEF may request credit for new
charging keys from and/or shall return the remaining credit for charging keys
no longer active to the OCS.
10\. If OCS was involved, the OCS provides the credit information to the PCEF,
and/or acknowledges the credit report
11\. The GW (PCEF) may send an IP‑CAN Bearer establishment, modification or
...
12\. The GW (PCEF) receives the response for the IP‑CAN Bearer modification or
termination request (applicable to case 1).
13\. The PCEF sends Acknowledge Policy and Charging Rules Provisioning (accept
or reject of the PCC rule operation(s)) to the PCRF.
14\. If the AF requested it, the PCRF notifies the AF related bearer level
events (e.g. transmission resources are established/released/lost).
...
+----------------+----------------+---------------+----------------+ | Event trigger | Description | Reported from | Condition for | | | | | reporting | +----------------+----------------+---------------+----------------+ | SGSN change | The UE has | PCEF | PCRF | | | moved to a new | | | | | SGSN. (Note 2) | | | +----------------+----------------+---------------+----------------+ | Serving GW | The UE has | PCEF | PCRF | | change | moved to a new | | | | | Serving GW. | | | | | (Note 1) | | | +----------------+----------------+---------------+----------------+ | RAT type | The | PCEF | PCRF | | change. | c | | | | | haracteristics | | | | | of the air | | | | | interface, | | | | | communicated | | | | | as the radio | | | | | access type, | | | | | has changed. | | | +----------------+----------------+---------------+----------------+ | Location | The routeing | PCEF | PCRF | | change | area of the UE | | | | (routeing | has changed. | | | | area) | | | | +----------------+----------------+---------------+----------------+ | Location | The tracking | PCEF | PCRF | | change | area of the UE | | | | (tracking | has changed. | | | | area) | (Note 1) | | | +----------------+----------------+---------------+----------------+ | Location | The ECGI of | PCEF | PCRF | | change (ECGI) | the UE has | | | | | ch | | | | | anged.(Note 1) | | | +----------------+----------------+---------------+----------------+ | Location | The CGI/SAI of | PCEF | PCRF | | change | the UE has | | | | (CGI/SAI) | ch | | | | | anged.(Note 2) | | | +----------------+----------------+---------------+----------------+ | Subscribed | The subscribed | PCEF | Always set | | APN-AMBR | APN-AMBR has | | | | change | changed | | | +----------------+----------------+---------------+----------------+ | EPS Subscribed | The QoS of the | PCEF | Always set | | QoS change | default EPS | | | | | bearer has | | | | | changed. | | | +----------------+----------------+---------------+----------------+ | NOTE 1: These | | | | | triggers are | | | | | used for | | | | | E‑UTRAN | | | | | access. | | | | | | | | | | NOTE 2: These | | | | | triggers are | | | | | used for | | | | | GERAN/UTRAN | | | | | accesses. | | | | +----------------+----------------+---------------+----------------+
...
From the network scenarios listed in clause 7.1, the Case 1 (no Gateway
Control Session) applies.
For an IP-CAN session set-up over a Trusted WLAN Access with S2a, at IP‑CAN
Session Establishment, if TWAN location information (e.g. BSSID and the SSID
of the access point to which the UE is attached, as described in TS 23.402
[18]) has been received by the PCEF, the PCEF provides this information to the
PCRF (over Gx). This occurs at step 3 of Figure 7.2-1 (IP‑CAN Session
Establishment). This information is also provided by the PCEF to the charging
entities as described in TS 32.251 [9].
...
The complete specification of the BPCF is defined in BBF WT-134 [31] and BBF
WT-203 [30] and BBF WT-291 [33] and it is out of the scope of 3GPP.
...
The complete specification of the BPCF is defined in BBF WT-134 [31] and BBF
WT-203 [30] and it is out of the scope of the 3GPP.
...
HNB local IP address. When the S15 Session terminates, the DRA shall remove
the information about the S15 Session.
