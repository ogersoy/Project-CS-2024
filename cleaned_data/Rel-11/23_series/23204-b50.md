...
with the UE, etc.
6 Mechanisms to handle MT-SMS without MSISDN.
...
## 4a.4 MT-SMS without MSISDN interworking
For MT-SMS without MSISDN interworking, the architecture allows for the
...
SM message.
### 5.2.4 E/Gd/SGd interface
The E/Gd/SGd interface allows the IP-SM-GW to connect to the SMS‑GMSC,
...
protocols, appearing to the SMS‑GMSC as an MSC, SGSN or MME using the E, Gd or
SGd interfaces;
...
protocols, appearing to the SMS-IWMSC as an MSC, SGSN or MME using the E, Gd
or SGd interfaces;
...
## 6.0a MT-SMS without MSISDN in IMS
The selection of IMS to deliver MT-SMS without MSISDN for device triggering is
defined in TS 23.682 [20]. This clause describes the additional functionality
required for the delivery of MT-SMS toward a UE in IMS without MSISDN.
...
UE which does not have MSISDN will have at least one SIP-URI as IMPU. IP‑SM‑GW
shall try to deliver the MT-SMS via IMS without retrying to other domain.
...
without MSISDN (see clause 6.0a), uses it as an indication that the UE is
...
MAP or Diameter based (SGd) signalling (as described in TS 23.040 [2]).
...
(i.e. when a Correlation ID is received in step 4), the IP-SM-GW performs
