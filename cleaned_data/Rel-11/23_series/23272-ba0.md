...
UE needs to be paged, the MME sets the priority level in the S1 AP paging
...
received from the eNodeB and/or MPS CS Priority stored in UE\'s EPS
...
\- Indicating the last used LTE PLMN ID to GERAN/UTRAN to help GERAN/UTRAN
(immediately following a successful RR connection setup or during the release
of the UTRAN RR connection) re-route the UE back to the last used LTE PLMN by
selecting the suitable dedicated target frequency list (for idle mode
...
Connection Release procedure. Furthermore BSS shall during the handover
preparation phase send the last used LTE PLMN ID to the target BSS/RNS.
...
mobility to E-UTRAN in RR Connection Release procedure. Furthermore RNS shall
during the handover preparation phase send the last used LTE PLMN ID to the
target RNS/BSS.
## 4.4 Control plane
...
a UE that is subscribed to a CS domain priority scheme may contain an
...
MME sends an MO CSFB indication via SGs to the MSC to indicate that the CM
service request message, as described in step 4b in this clause , is due to
the CSFB via CS MO call.
...
1d. If configured to support the return to the last used PLMN after CSFB, the
MME sends an MO CSFB indication via SGs to the MSC to indicate that the CM
service request message, as described in step 9 in this clause, is due to the
CSFB via CS MO call.
2\. The eNodeB may optionally solicit a measurement report from the UE to
...
a Service User according to TR 22.952 [47], sets the RRC establishment cause
...
a successful setup an RR connection or during the release of the UTRAN RR
...
value(s) to the value configured in the MME for CSFB priority call handling.
...
1\. The UE initiates combined attach or combined TA/LA Update to an MME.
...
SMS in HSS, (e.g. because the UE initiate a EPS attach only, TAU), the
