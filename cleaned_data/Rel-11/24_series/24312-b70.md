...
node subtree (see subclause 5.7A) encoded as XML formatted as specified by
...
TimeStart to 24:00 and from 00:00 to the time indicated by TimeStop, from
...
### _5.7.17A \ _/ISRP/_\ _/ForFlowBased/_\ _/IPFlow/_\
_/DomainName
...
\/ISRP/\/ForFlowBased/\/RoutingCriteria/\ node indicates matching
validity condition, the UE shall consider the corresponding flow distribution
rule as valid.
If the \/ISRP/\/ForFlowBased/\/RoutingCriteria/\ node is present
and empty (i.e. none of the nodes ValidityArea, TimeOfDay or APN exist), the
\/ISRP/\/ForFlowBased/\/RoutingCriteria/\ node is not considered
...
if all the present interior nodes (ValidityArea, TimeOfDay and APN) within a
single instance of \/ISRP/\/ForFlowBased/\/RoutingCriteria/\ node
...
\/Policy/\/PrioritizedAccess unless explicitly specified.
...
\- Values: N/A
If at least one instance of
\/ISRP/\/ForServiceBased/\/RoutingCriteria/\ node indicates
matching validity condition, the UE shall consider the corresponding flow
distribution rule as valid.
If the \/ISRP/\/ForServiceBased/\/RoutingCriteria/\ node is
present and empty (i.e. none of the nodes ValidityArea or TimeOfDay exist),
the \/ISRP/\/ForServiceBased/\/RoutingCriteria/\ node is not
considered when evaluating the validity of the corresponding flow distribution
rule.
...
\- Values: N/A
The UE shall consider the corresponding flow distribution rule as valid only
if all the present interior nodes (ValidityArea and TimeOfDay) within a single
instance of \/ISRP/\/ForServiceBased/\/RoutingCriteria/\ node
indicate matching validity condition.
...
An empty IPFlow node indicates the match-all flow description.
If the flow distribution rule contains more than one instance of the
\/ISRP/\/ForNonSeamlessOffload/\/IPFlow/\ node (i.e. flow
...
If the flow description is defined with more than one interior node or leaf
(i.e. App-ID, AddressType, etc.) within a single instance of the
\/ISRP/\/ForNonSeamlessOffload/\/IPFlow/\ node, then UE shall
...
### _5.7.37A_ \/ISRP/_\ _/ForNonSeamlessOffload/\/ IPFlow/_\
/_App-ID
This interior node acts as a placeholder for an IPFlow identification based on
an applicationID.
An IP flow is evaluated as matching if the IP flow matches at least one child
...
examined when matching packets against the IP flow description of the rule.
### 5.7.37B \/ISRP/_\ _/ForNonSeamlessOffload/\/ IPFlow/_\ /_App-
ID/_\ /_
This interior node acts as a placeholder for one or more combinations of OSId
and OSAppId values.
An IP flow is evaluated as matching if the IP flow is sent by an application:
...
\- Values: N/A
### _5.7.37C_ Void
### _5.7.37D_ \/ISRP/_\ _/ForNonSeamlessOffload/\/ IPFlow/_\
/_App-ID/_\ /_OSId
The OSId leaf indicates an operating system identifier.
\- Occurrence: One
\- Format: chr
...
The format of the operating system identifier is a Universally Unique
IDentifier (UUID) as specified in IETF RFC 4122 [22].
### _5.7.37E \ _/ISRP/_\ _/ForNonSeamlessOffload/\/IPFlow/_\
_/App-ID/ \/OSApps/
This interior node acts as a placeholder for the list of OS specific
application identifiers.
\- Occurrence: One
...
\- Access Types: Get, Replace
\- Values: \
### _5.7.37F \ _/ISRP/_\ _/ForNonSeamlessOffload/\/IPFlow/_\
_/App-ID/ \/OSApps/\
This interior node acts as a placeholder for an OS specific application
identifiers.
\- Occurrence: OneOrMore
...
\- Access Types: Get, Replace
\- Values: \
### _5.7.37G \ /ISRP/\/ForNonSeamlessOffload/\/IPFlow/\/ App-ID/_
...
form.
The DS and ToS values are defined in IETF RFC 3260 [10A].
### _5.7.48A \ _/ISRP/_\ _/ForNonSeamlessOffload/\/IPFlow/_\ _/
DomainName
The DomainName leaf indicates the destination domain name of the IP Flow
description.
\- Occurrence: ZeroOrOne
...
\- Access Types: Get, Replace
\- Values: N/A
If at least one instance of \/ISRP/\/
ForNonSeamlessOffload/\/RoutingCriteria/\ node indicates matching
validity condition, the UE shall consider the corresponding flow distribution
rule as valid.
If the \/ISRP/\/ ForNonSeamlessOffload/\/RoutingCriteria/\ node is
present and empty (i.e. none of the nodes ValidityArea, TimeOfDay or APN
exist), the \/ISRP/\/ ForNonSeamlessOffload /\/RoutingCriteria/\
...
The UE shall consider the corresponding flow distribution rule as valid only
if all the present interior nodes (ValidityArea, TimeOfDay and APN) within a
single instance of \/ISRP/\/ ForNonSeamlessOffload
/\/RoutingCriteria/\ node indicate matching validity condition.
...
If the UE supports ISRP flow distribution rules (see subclause 5.7A.4)
containing an OS specific application identifier, one \ interior node with
OSId leaf is included for each supported OS in the \/UE_Profile node.
### 5.7A.3 _\ _/UE_Profile/_\ /OSId_
...
The format of the operating system identifier is a Universally Unique
IDentifier (UUID) as specified in IETF RFC 4122 [22].
### 5.7A.4 _\ _/UE_Profile/DevCapability
