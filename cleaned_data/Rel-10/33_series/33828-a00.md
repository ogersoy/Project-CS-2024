...
plain ID-A，plain ID-B and Ea (Ra, ID-A, ID-B) and Eb(Rb, ID-A,ID-B) to the KMS
...
> parameters: plain ID-A，plain ID-C and Ea (Ra, ID-A, ID-C) and Ec(Rc,
...
without informing the originator. A common scenario is one in which the S‑CSCF
...
presented. In the scenario (Figure XX) presented, user A and B's mailbox
perform mutual authentication before they agree on the key to be used for
decrypting the content of the massage intended for deferred delivery.
...
did not receive the massage and that the actual recipient was not able to
...
and aP encrypted using B's public key. Upon reception of this massage, the B's
...
mailbox server. Note that, B may have to authenticate with the mailbox to
obtain the key (this is not shown in the figure below) -- this could be based
on existing authentication mechanisms already in place.
