...
The present document defines the user plane of GTP used on:
  ...
  * the S1-U, S2b-U, X2, S4, S5, S8, S12, M1 and Sn interfaces of the > Evolved Packet System (EPS);
...
plane based protocols. RNC, SGSN, GGSN, eNodeB, SGW, ePDG or PGW.
## 3.2 Abbreviations
...
endpoints using control plane messaging.
...
to user plane entities in the RNC, SGSN, GGSN, eNodeB, SGW, ePDG and PGW. The
...
header.
For PGW, SGW, ePDG and eNodeB the usage of sequence numbers in G-PDUs is
...
cases, the PGW, SGW, ePDG and eNodeB should set the \"S\" flag to 0 in the
GTPv1 header which then indicates that the sequence number is not used in the
T-PDU.
...
is optional for G-PDUs, the PGW, SGW, ePDG and eNodeB should set the flag to
\'0\'. However, when a G-PDU (T-PDU+header) is being relayed by the Indirect
Data Forwarding for Inter RAT HO procedure, then if the received G-PDU has the
S flag set to \'1\', then the relaying entity shall set S flag to \'1\' and
forward the G-PDU (T-PDU+header). In an End marker message the S flag shall be
set to \'0\'.
...
interfaces Iu-U, Gn, Gp, S1-U, S2b, S4, S5, S8, S12, X2-U, M1 and Sn.
...
GGSN, SGSN, RNC, PGW, SGW, ePDG or eNodeB address. The TEID and GTP-U peer
Address together uniquely identify the related PDP context, RAB or EPS bearer
