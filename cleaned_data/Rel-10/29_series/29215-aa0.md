...
> Info AVP.
###### ## Annex A (normative): Access specific aspects, EPC-based eHRPD Access
## A.1 General
In case of EPC-based eHRPD access the BBERF is located in the HRPD Serving
Gateway (HSGW) as defined in 3GPP2 X.S0057 [13].
## A.2 IPv6 prefix provisioning
...
\"EHRPD\" and the the Multiple-BBERF-Action AVP set to the value
\"ESTABLISHMENT\" from the V-PCRF, and if the UE has acquired an IPv6 prefix
via the 3GPP access, the H-PCRF shall provide the IPv6 prefix of the UE to the
V-PCRF by including the Framed-Ipv6-Prefix AVP in Subsession-Decision-Info AVP
within the CCA command.
