...
A mechanism to restrict access by low priority MSs is provided via EAB. A
network operator can restrict network access of those MSs that are configured
for EAB in addition to common access control and domain specific access
control.
...
mechanism (see 3GPP TS 24.008 [23], 3GPP TS 44.018 [34]) when configured for
EAB.
NOTE: Access network procedures for EAB in UTRAN and E-UTRAN are not specified
...
11 to 15) and that special access class is not barred, then the MS shall
ignore EAB.
If an MS configured for EAB is initiating an emergency call and emergency
calls are allowed in the cell, then the MS shall ignore EAB.
