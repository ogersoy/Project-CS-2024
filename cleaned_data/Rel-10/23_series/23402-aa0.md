...
\- S5 and S2b can be GTP-based or PMIP-based.
...
Breakout
NOTE 4: The two Rx instances in Figure 4.2.3-4 apply to different application
...
NOTE 5: The two Rx instances in Figure 4.2.3-5 apply to different application
functions in the HPLMN and VPLMN.
...
or S8, or if S2a or PMIP based S2b is used. The LMA function shall be able to
accept UL packets from any trusted MAG without enforcing that the source IP
address must match the CoA in the MN BCE.
...
S5/S8, or S2a or PMIP based S2b interface.
...
Proxy Mobile IP version 6. To enable access via Trusted Non 3GPP IP accesses
that do not support PMIPv6, S2a also supports Client Mobile IPv4 FA mode.
\- S2b interface is based on GTP or Proxy Mobile IP version 6.
\- S2c is based on DSMIPv6 [10].
\- The PMIP-based S5, PMIP-based S8, S2a and PMIP-based S2b interfaces are
based on the same protocols and differences shall be minimized. The S5
interface is based on the PMIPv6 specification, RFC 5213 [8].
\- The GTP based S5/S8 and GTP based S2b interfaces are based on the GTP
protocol (TS 29.274 [57]). The GTP variant of S5 interface is described in TS
23.401 [4].
...
receiving entity to determine an appropriate APN-OI. The ePDG may also use
this information to determine the S2b protocol type (PMIP or GTP).
...
Evolved Packet Core.
### 4.6.2 EPS bearer identity with GTP based s2b
...
NOTE 1: For LIPA case, the L-GW with PMIP-based S5 should be statically
configured with either same or larger Link-MTU value than the value configured
in the Serving GW.
...
NOTE 2: The DHCPv4 client may skip DHCPv4 Discovery phase, and send DHCPv4
...
DHCPv4 server.
NOTE 3: After releasing the IPv4 address using DHCPv4 Release procedure, UE
can request an IPv4 address for the same PDN connection subsequently.
NOTE 4: Allocation of IP address from an external PDN using Radius or Diameter
...
NOTE 5: In order to renew the allocated IPv6 prefix, the Serving GW sends an
...
NOTE 6: The L-GW identifies appropriate UE BCE (Binding Cache Entry) when
receiving Router Solicitation or DHCPv4 Discovery request messages via the
direct user data path. How the L-GW identifies the UE BCE is implementation
dependent (e.g. using Correlation ID or UE\'s link-local address).
...
NOTE 1: ANDSF push interactions might not always be possible via non-seamless
WLAN offload.
...
or on any APN. A filter rule also identifies which radio accesses are
restricted for traffic that matches specific IP filters on aspecific APN (e.g.
WLAN is not allowed for RTP/RTCP traffic flows on APN-x) or on any APN;
...
preferences. In addition, the UE shall route traffic that matches specific
criteria according to the filter rules in the received / provisioned inter-
system routing policies and according to the user preferences.
...
to the 3GPP AAA Server. If the detach procedures was initiated from the 3GPP
...
message to the 3GPP AAA Server. If the detach procedures was initiated from
the 3GPP AAA Server and if the UE no longer has any context in the 3GPP AAA
...
AAA. If the detach procedures was initiated from the 3GPP AAA Server and if
the UE no longer has any context in the 3GPP AAA Server, the 3GPP AAA Server
...
procedures was initiated from the 3GPP AAA Server and if the UE no longer has
...
TS 23.203 [19].
5\. The PDN GW shall acknowledge the termination of the S2c session to the
3GPP AAA Server. If the detach procedures was initiated from the 3GPP AAA
...
3GPP AAA Server should wait to receive acknowledgement(s) from PDN GW(s)
before sending the detach indication message to the ePDG. The ePDG releases
the IPSEC tunnels to the UE and acknowledges the receipt of the detach
indication message to the 3GPP AAA Server.
...
NOTE 2: All steps outside of (C) are common for architecture variants with
GTP-based S2b and PMIP-based S2b. Procedure for steps outside of (C) for GTP-
based S2b are described in clause 8.6.1.1.
...
provided, and the UE context in the MME contains a PDN GW identity
...
NOTE 2: All steps in (D) are common for architecture variants with GTP based
