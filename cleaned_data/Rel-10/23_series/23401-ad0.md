...
defined in bullet n in clause 4.3.17.2.
#### 4.3.17.5 Void
...
A TFT may be also assigned to the default EPS bearer. The UE uses the UL TFT
for mapping traffic to an EPS bearer in the uplink direction. The PCEF (for
GTP-based S5/S8) or the BBERF (for PMIP-based S5/S8) uses the DL TFT for
mapping traffic to an EPS bearer in the downlink direction. The UE may use the
UL TFT and DL TFT to associate EPS Bearer Activation or Modification
procedures to an application and to traffic flow aggregates of the
application. Therefore the PDN GW shall, in the Create Dedicated Bearer
Request and the Update Bearer Request messages, provide all available traffic
flow description information (e.g. source and destination IP address and port
numbers and the protocol information).
...
A TFT of a unidirectional EPS bearer is either associated with UL packet
filter(s) or DL packet filter(s) that matches the unidirectional traffic
flow(s) and a DL packet filter or a UL packet filter that effectively
disallows any useful packet flows (see clause 15.3.3.4 in TS 23.060 [7] for an
example of such packet filter).
...
NOTE 9: The above algorithm implies that there is at most one EPS bearer
...
the PCEF (for GTP-based S5/S8) and the BBERF (for PMIP-based S5/S8) applies
the Session Management restrictions as described in clause 9.2.0 of TS 23.060
[7].
...
NOTE: At intersystem changes between GERAN/UTRAN and E-UTRAN there is a 1-to-1
mapping between PDP type IPv4v6 and PDN type IPv4v6 without re-negotiation of
the PDP/PDN type used for a PDN connection.
...
NOTE 1: If the MT and TE are separated, the UE might not be able to use reason
...
NOTE 2: It is transparent to the UE whether the PLMN or the external PDN
...
is an EPS attach or a combined EPS/IMSI attach or an Emergency Attach.
...
NOTE 7: A SGSN always responds with the UMTS security parameters and the MME
...
Over PS Sessions, UE SRVCC capability) message to the HSS. The MME
capabilities indicate the MME\'s support for regional access restrictions
functionality. ULR-Flags indicates \"Initial-Attach-Indicator\" as this is an
Attach procedure. Homogenous Support of IMS Over PS Sessions indicates whether
...
The Subscription Data may contain CSG subscription information. If the UE
provided APN is authorized for LIPA according to the user subscription, the
MME shall use the CSG Subscription Data to authorize the connection.
...
Serving Network, RAT type, APN-AMBR, Default EPS Bearer QoS are provided to
the PCRF by the PDN GW if received by the previous message. The User Location
Information and UE Time Zone are used for location based charging. For
emergency attached UEs which are unauthenticated the PDN GW provides the IMEI
as the UE Identity instead of IMSI, to the PCRF.
...
NOTE 8: While the PDN GW/PCEF may be configured to activate predefined PCC
...
NOTE 9: If the IP address is not available when the PDN GW performs the IP-CAN
...
HeNB and the L-GW.
NOTE 10: In this release of the 3GPP specification the Correlation ID is set
...
NOTE 11: If the UE receives an Attach Accept message via a hybrid cell, the UE
...
if necessary.
NOTE 12: The IP address allocation details are described in clause 5.3.1 on
...
Response to the MME.
NOTE 13: For handover from non-3GPP access, the PDN GW initiates resource
...
Context Response (IMSI, ME Identity (IMEISV), MSISDN, MM Context, EPS Bearer
...
the UE. The MSISDN is included if the old MME/old S4 SGSN has it stored for
that UE.
...
new MME for this UE, or for some network sharing scenario (e.g. GWCN) if the
PLMN-ID of the TAI supplied by the eNodeB is different from that of the GUTI
in the UE\'s context, then the new MME sends an Update Location Request (MME
Identity, IMSI, ULR-Flags, MME Capabilities, Homogeneous Support of IMS Over
PS Sessions, UE SRVCC capability, ME Identity) message to the HSS. ULR-Flags
...
functionality. Homogenous Support of IMS Over PS Sessions indicates whether or
not \"IMS Voice over PS Sessions\" is supported homogeneously in all TAs the
serving MME. The ME Identity is included if step 5 caused the MME to retrieve
...
CSG subscription data for the PLMN.
...
and is not expired. If the CSG ID is not present or expired, the MME shall
send a Tracking Area Update reject message to the UE with an appropriate cause
value. The UE shall remove the CSG ID and associated PLMN from its Allowed CSG
list if present. If the UE has ongoing emergency bearer services no CSG access
...
Voice over PS session supported as described in clause 4.3.5.8. The Emergency
Service Support indicator informs the UE that Emergency bearer services are
supported. LCS Support Indication indicates whether the network supports the
EPC-MO-LR and/or CS-MO-LR as described in TS 23.271 [57].
...
NOTE 6: If the UE receives a TAU Accept message via a hybrid cell, the UE does
...
NOTE 7: The new MME may initiate E‑RAB establishment (see TS 36.413 [36])
...
EPS bearers with ARP values reserved for emergency services.
NOTE 8: If MS (UE) was in PMM-CONNECTED state the bearer contexts are sent
...
also when it cannot validate the Context Request.
5\. If the Context Request is sent to an old MME the old MME responds with a
Context Response (IMSI, ME Identity (IMEISV), MSISDN, unused EPS
...
(if available), CSG Information Reporting Action (if available), UE Time Zone,
UE Core Network Capability, UE Specific DRX Parameters, Change to Report (if
...
new MME for this UE, or for some network sharing scenario (e.g. GWCN) if the
PLMN-ID of the TAI supplied by the eNodeB is different from that of the GUTI
in the UE\'s context, then the new MME informs the HSS of the change of MME by
sending an Update Location Request (MME Id, IMSI, ULR-Flags, MME Capabilities,
Homogenous Support of IMS Over PS Sessions, UE SRVCC capability, ME Identity
...
for regional access restrictions functionality. Homogenous Support of IMS Over
PS Sessions indicates whether or not \"IMS Voice over PS Sessions\" is
supported homogeneously in all TAs in the serving MME. The ME Identity is
included if step 5 caused the MME to retrieve the IMEISV from the UE.
If the MME determines that only the UE SRVCC capability has changed, the MME
sends a Notify Request to the HSS to inform about the changed UE SRVCC
capability.
If all the EPS bearers of the UE have emergency ARP value, the new MME may
...
NOTE 7: ISR Activated is never indicated from new to old MME.
...
may contain the CSG subscription data for the PLMN.
If the UE initiates the TAU procedure at a CSG cell, the new MME shall check
whether the CSG ID and associated PLMN is contained in the CSG subscription
and is not expired. If the CSG ID is not present or expired, the MME shall
send a Tracking Area Update reject message to the UE with an appropriate cause
...
the IMS Voice over PS session supported as described in clause 4.3.5.8. The
...
Contexts, Serving GW signalling Address and TEID(s), ISR Supported, MS Info
Change Reporting Action (if available), CSG Information Reporting Action (if
available), UE Time Zone, UE Core Network Capability, UE Specific DRX
...
If there are no subscription data in the new SGSN for this UE, or for some
network sharing scenario (e.g. GWCN) if the PLMN-ID of the RAI supplied by the
RNC is different from that of the RAI in the UE\'s context, then the new SGSN
informs the HSS of the change of the SGSN by sending an Update Location (SGSN
Number, SGSN Address, IMSI, Homogenous Support of IMS Over PS Sessions, UE
SRVCC capability) message to the HSS. Homogenous Support of IMS Over PS
...
subscription and is not expired. If the CSG ID is not present or expired, the
S4 SGSN shall send a RAU reject message to the UE with an appropriate cause
value. The UE shall remove the CSG ID and associated PLMN from its Allowed CSG
list if present.
16\. Void.
17\. Void.
...
> security related information as well as other parameters (including IMSI and
> MSISDN) as described in clause 5.7.2 (Information Storage for MME). The PDN
> GW Address and TEID(s) (for GTP-based S5/S8) or GRE Keys (PMIP-based S5/S8)
> for uplink traffic and control plane, and the TI(s) is part of the EPS
> Bearer context(s). The unused Authentication Quintets in the MM Context may
> be sent if stored by the MME and if the MME received the unused
...
network sharing scenario (e.g. GWCN) if the PLMN-ID of the RAI supplied by the
RNC is different from that of the RAI in the UE\'s context, then the new SGSN
informs the HSS of the change of SGSN by sending an Update Location (SGSN
Number, SGSN Address, IMSI, Homogenous Support of IMS Over PS Session, UE
SRVCC capability) message to the HSS. Homogenous Support of IMS Over PS
Sessions indicates whether or not \"IMS Voice over PS Sessions\" is supported
homogeneously in all RAs in the serving SGSN.
13\. The HSS sends a Cancel Location (IMSI, Cancellation Type) message to the
...
subscription data for this CSG ID or the CSG subscription is expired, the MME
rejects the Service Request with an appropriate cause. The UE shall remove the
CSG ID and associated PLMN of the cell where the UE has initiated the service
request procedure from the Allowed CSG list, if present.
For UEs with emergency EPS bearers, i.e. at least one EPS bearer has an ARP
values reserved for emergency services, if CSG access restrictions do not
allow the UE to get normal services the MME shall deactivate all non-emergency
...
was sent for or ifthe Serving GW has sent the second Downlink Data
...
With UE Lost, the MME shall trigger the MME Initiated Dedicated Bearer
Deactivation procedure (clause 5.4.4.2) for the GBR bearer(s) of the UE after
...
subscription for this CSG ID is absent or expired, the MME shall trigger a
MME-initiated Detach procedure as specified in clause 5.3.8.3.
...
deactivated by the MME sending Delete Session Request (LBI) per PDN connection
to the Serving GW. If ISR is activated, then the Serving GW shall not release
the Control Plane TEID allocated for MME/SGSN until it receives the Delete
Session Request message in step 5. If the PDN GW requested UE\'s location
info, the MME also includes the User Location Information IE in this message.
If the UE Time Zone has changed, the MME includes the UE Time Zone IE in this
message.
...
deactivated by the SGSN sending Delete Session Request (LBI) per PDN
connection to the Serving GW. If the PDN GW requested UE\'s location info, the
SGSN also includes the User Location Information IE in this message. If the UE
Time Zone has changed, the SGSN includes the UE Time Zone IE in this message.
6\. If ISR is activated, Serving GW deactivates ISR. If ISR is not activated
in the Serving GW, the Serving GW sends Delete Session Request (LBI) per PDN
connection to the PDN GW. If ISR is not activated, this step shall be
triggered by step 2. This message indicates that all bearers belonging to that
PDN connection shall be released. If the MME and/or SGSN sends UE\'s Location
Information and/or UE Time Zone in step 2 and/or step 5, it is an
...
SGSN-initiated Detach procedure as specified in clause 5.3.8.3A.
2\. The active EPS Bearers in the Serving GW regarding this particular UE are
deactivated by the SGSN sending Delete Session Request (LBI) per PDN
...
in this message. If the UE Time Zone has changed, the SGSN includes the UE
Time Zone IE in this message.
...
5\. The active PDP contexts in the Serving GW regarding this particular UE are
deactivated by the MME sending Delete Session Request (LBI) per PDN connection
to the Serving GW. If the PDN GW requested UE\'s location info, the MME also
includes the User Location Information IE in this message. If the UE Time Zone
has changed, the MME includes the UE Time Zone IE in this message.
6\. Serving GW deactivates ISR and sends Delete Session Request (LBI) per PDN
connection to the PDN GW. If ISR is not activated, this step shall be
triggered by step 2. This message indicates that all bearers belonging to that
PDN connection shall be released. If the MME and/or SGSN sends UE\'s Location
Information and/or UE Time Zone in step 2 and/or step 5, it is an
...
ID is absent or expired, the MME shall send a Detach Request to UE with an
appropriate cause indicating the UE is not allowed to access this CSG.
...
Session Request (LBI) message per PDN connection to the Serving GW. If the PDN
GW requested UE\'s location info, the MME also includes the User Location
Information IE in this message. If the UE Time Zone has changed, the MME
...
Request (LBI) message per PDN connection to the Serving GW. If Cause indicates
local detach then SGSN deactivates ISR and steps 5 to 9 shall be skipped. If
the PDN GW requested UE\'s location info, the SGSN also includes the User
Location Information IE in this message. If the UE Time Zone has changed, the
...
Session Request (LBI) message for each associated PDN connection to the PDN
GW. This message indicates that all bearers belonging to that PDN connection
shall be released.
If the MME and/or SGSN send(s) UE\'s Location Information and/or UE Time Zone
in step 2 and/or Step 5, it is an implementation specific behaviour on S-GW to
decide which of the received User Location Information to forward to the PDN
GW.
...
If this Detach procedure is due to the UE\'s Detach Request via a CSG cell
which the UE is not allowed to access, i.e. the CSG subscription for this CSG
ID is absent or expired, the SGSN shall send a Detach Request to UE with an
appropriate cause indicating the UE is not allowed to access this CSG.
2\. Any EPS Bearer Context information in the Serving GW regarding this
particular UE and related to the SGSN is deactivated by the SGSN sending
Delete Session Request (LBI) message per PDN connection to the Serving GW. If
the PDN GW requested UE\'s location info, the SGSN also includes the User
Location Information IE in this message. If the UE Time Zone has changed, the
SGSN includes the UE Time Zone IE in this message.
3\. Because the Serving GW receives this message in ISR activated state, the
...
Request (LBI) message per PDN connection to the Serving GW. If Cause indicates
local detach then MME deactivates ISR and steps 5 to 9 shall be skipped. If
the PDN GW requested UE\'s location info, the MME also includes the User
Location Information IE in this message. If the UE Time Zone has changed, the
MME includes the UE Time Zone IE in this message.
6\. The Serving GW sends a Delete Session Request (LBI) message per PDN
connection to the PDN GW. This message indicates that all bearers belonging to
that PDN connection shall be released. If the MME and/or SGSN sends UE\'s
Location Information and/or UE Time Zone in step 2 and/or step 5, it is an
...
Request (LBI) message per PDN connection to the Serving GW to deactivate the
EPS Bearer Context information in the Serving GW.
3b. If the SGSN has an active UE context, the SGSN sends a Delete Session
Request (LBI) per PDN connection to the Serving GW to deactivate the EPS
...
EPS Bearer context information and sends a Delete Session Request (LBI)
...
up to the point that the PDN GW requests IP‑CAN Bearer Signalling. If dynamic
PCC is not deployed, the PDN GW may apply local QoS policy. The PDN GW
initiated Bearer deactivation is also performed when handovers without
optimization occurs from 3GPP to non-3GPP, in which case, the default bearer
and all the dedicated bearers associated with the PDN address are released,
...
If the UE wants to link the new packet filter to an existing packet filter to
enable the usage of existing bearer resources for the new packet filter, the
UE shall provide an existing packet filter identifier together with the new
packet filter. If the UE wants to change the GBR in addition, the UE includes
the GBR requirement of the EPS Bearer. The TAD is released when the procedure
is completed.
...
eNodeB when the handover is complete or to the source eNodeB when the handover
...
message. The MME determines the CSG membership based on the CSG ID and CSG
Access Mode received from the target eNodeB.The MME updates the User CSG
information based on the CSG ID and CSG Access Mode received from the target
eNodeB and CSG membership if one of the parameters has changed.
NOTE 2: X2 handover between HeNBs is possible when the handover is between
closed/hybrid access HeNBs having the same CSG ID or when the target HeNB is
an open access HeNB.
...
NOTE 3: The MME knows the S‑GW Service Area with a TA granularity.
...
NOTE 4: It is only a subset of the TA update procedure that is performed by
...
source MME may allow the handover by validating the CSG membership of the UE
in the target CSG cell using the CSG-ID list of the registered PLMN-ID.
Otherwise, the source MME shall reject the handover due to no CSG membership
...
RAT handovers.
For inter-PLMN handover to a CSG cell, based on operator\'s configuration the
source MME/S4-SGSN may allow the handover by validating the CSG membership of
the UE in the target CSG cell using the CSG-ID list of the registered PLMN-ID.
Otherwise, the source MME/S4-SGSN shall reject the handover due to no CSG
membership information of the target PLMN-ID
NOTE 1: Inter-PLMN handover to a CSG cell in a PLMN which is not an equivalent
PLMN for the UE is not supported.
NOTE 2: Inter-PLMN handover to a CSG cell of an equivalent PLMN is only
supported if the CSG-ID of the cell is in the CSG-ID list of both equivalent
...
a handover from non-3GPP access and the UE has already established
...
change of subscription or lack of resources or due to SIPTO in case the PDN
connection serves a SIPTO-allowed APN. If the UE is in ECM-IDLE state and the
reason for releasing PDN connection is \"reactivation requested\" e.g. due to
SIPTO, the MME initiates paging via Network Triggered Service Request
procedure in clause 5.3.4.3 from step 3a onwards in order to inform UE of the
request.
2\. The EPS Bearers in the Serving GW for the particular PDN connection are
deactivated by the MME by sending Delete Session Request (Cause, LBI) to the
Serving GW. This message indicates that all bearers belonging to that PDN
connection shall be released. If the PDN GW requested UE\'s location info
(determined from the UE context), the MME also includes the User Location
Information IE in this message. If the UE Time Zone has changed, the MME
includes the UE Time Zone IE in this message.
NOTE 3: In case PDN connection release was initiated by the MME the User
...
was acquired last.
3\. The Serving GW sends Delete Session Request (Cause, LBI) to the PDN GW.
The S‑GW also includes User Location Information IE and/or UE Time Zone IE if
it is present in step 2.
...
certain PDN connections e.g. for SIPTO. The MME initiates the deactivation of
all Bearers associated with the PDN connection to the eNodeB by sending the
Deactivate Bearer Request message to the eNodeB. The MME shall re-calculate
the UE-AMBR (see clause 4.7.3). This S1-AP message carries the list of EPS
bearers to be released, the new UE-AMBR, and a NAS Deactivate EPS Bearer
Context Request (LBI) message. The MME builds a NAS message Deactivate EPS
Bearer Context Request including the EPS Bearer Identity, and includes it in
the S1-AP Deactivate Bearer Request message.
...
NOTE 4: If the UE is in ECM‑IDLE state and the PDN disconnection is decided by
...
Support of IMS Over PS Sessions) to the HLR. IMEISV is sent if the ADD
function is supported._ Update Type indicates \"normal update\". Homogenous
...
Capabilities, Homogenous Support of IMS Over PS Sessions) message to the HSS.
The ME Identity is included if the SGSN Context Response did not contain the
IMEISV. Because of interoperation with an Gn/Gp SGSN, which the new MME
...
support for regional access restrictions functionality. Homogenous Support of
IMS Over PS Sessions indicates whether or not \"IMS Voice over PS Sessions\"
is supported homogeneously in all TAs in the serving MME.
...
value from the HSS, it calculates the UE-AMBR (UE-AMBR=MIN(subscribed UE-AMBR,
sum APN-AMBR of all active APNs)). Then it compares this value with the local
UE-AMBR and of the local UE-AMBRs is different from the corresponding derived
UE-AMBR, the MME initiates HSS Initiated Subscribed QoS Modification procedure
to notify the derived UE-AMBR to the eNodeB.
...
7), the Residual BER is considered \ GBR 9.5.0 10.0.0 2010-06 SP-48 SP-100337
1589 2 C ECN grace period 9.5.0 10.0.0 2010-06 SP-48 SP-100341 1590 2 B
Introduction of SIPTO in EPS 9.5.0 10.0.0 2010-06 SP-48 SP-100341 1593 1 B
Definition of the SIPTO flag in the HSS and MME subscriber info 9.5.0 10.0.0
2010-06 SP-48 SP-100351 1599 2 B Introduction of IPv6 Prefix Delegation into
23.401 9.5.0 10.0.0 2010-06 SP-48 SP-100341 1618 3 B PDN GW selection for
SIPTO 9.5.0 10.0.0 2010-06 SP-48 SP-100351 1629 1 F Clarification on S-TMSI
and GUMMEI usage 9.5.0 10.0.0 2010-09 SP-49 SP-100530 1669 1 A Correction of
TAU procedure with ISR activation 10.0.0 10.1.0 2010-09 SP-49 SP-100530 1733 1
A Correction to ISR indication during Context Req Procedure 10.0.0 10.1.0
2010-09 SP-49 SP-100530 1751 2 A Modifying and rejecting bearer level QoS
parameter for default bearer 10.0.0 10.1.0 2010-09 SP-49 SP-100534 1719 1 A
Removal of unnecessary signaling from detach and PDN disconnection procedures
10.0.0 10.1.0 2010-09 SP-49 SP-100534 1748 2 A Setting of IMS voice over PS
Session Supported Indication based on IMS roaming agreements 10.0.0 10.1.0
2010-09 SP-49 SP-100542 1651 2 A The ISR capability of S-GW for ISR activation
10.0.0 10.1.0 2010-09 SP-49 SP-100542 1679 - A Clarification of bearer
deactivation procedure in Inter-RAT handover 10.0.0 10.1.0 2010-09 SP-49
SP-100542 1708 2 A Correction of cause code handling in appendix D 10.0.0
10.1.0 2010-09 SP-49 SP-100542 1736 2 A Reporting UE Time Zone Change
Correction 10.0.0 10.1.0 2010-09 SP-49 SP-100550 1638 4 B Introduction of LIPA
in EPS 10.0.0 10.1.0 2010-09 SP-49 SP-100550 1643 1 B Addition of correlation
identifier for optimised routing in LIPA 10.0.0 10.1.0 2010-09 SP-49 SP-100550
1644 2 B LIPA permissions and CSG information for LIPA-able APNs in the HSS
10.0.0 10.1.0 2010-09 SP-49 SP-100550 1645 3 B LIPA Support for Service
Discovery 10.0.0 10.1.0 2010-09 SP-49 SP-100550 1647 3 B LIPA deactivation
10.0.0 10.1.0 2010-09 SP-49 SP-100550 1653 5 B Gateway selection for LIPA
10.0.0 10.1.0 2010-09 SP-49 SP-100550 1654 2 B Paging Handling for LIPA 10.0.0
10.1.0 2010-09 SP-49 SP-100551 1655 1 C Serving GW Selection for SIPTO 10.0.0
10.1.0 2010-09 SP-49 SP-100551 1691 3 B LIPA PDN Connection Setup Procedures
10.0.0 10.1.0 2010-09 SP-49 SP-100551 1694 1 F Correction and Clarification of
SIPTO procedure 10.0.0 10.1.0 2010-09 SP-49 SP-100551 1727 1 F SIPTO and
handover from other type of access network 10.0.0 10.1.0 2010-09 SP-49
SP-100636 1675 6 B High level function for machine type communication 10.0.0
10.1.0 2010-09 SP-49 SP-100554 1680 1 B Signalling IMSI at Attach/PLMN change
(TR 23.888, 7.1a, reference 6.33.2d) 10.0.0 10.1.0 2010-09 SP-49 SP-100554
1686 2 B CN Rejection Solution for MTC devices for PGW 10.0.0 10.1.0 2010-09
SP-49 SP-100554 1695 3 B Normative NIMTC text for low priority access 10.0.0
10.1.0 2010-09 SP-49 SP-100634 1697 4 B Introduction of normative NIMTC ACB
text in \'Functions for Machine Type Communications\' 10.0.0 10.1.0 2010-09
SP-49 SP-100554 1724 3 B Optimization for Periodic RAU/TAU Signalling 10.0.0
10.1.0 2010-09 SP-49 SP-100554 1726 3 B NAS level reject per APN with back-off
timer function 10.0.0 10.1.0 2010-09 SP-49 SP-100544 1713 1 B eMPS
subscription data 10.0.0 10.1.0 2010-09 SP-49 SP-100544 1714 2 B Introduction
of MPS for E-UTRAN 10.0.0 10.1.0 2010-09 SP-49 SP-100545 1723 4 B MPS Mobile
Termination impacts to Network Triggered Service Request and Bearer
Modification Procedures 10.0.0 10.1.0 2010-09 SP-49 SP-100558 1663 2 F
Correction of the PDN Disconnection procedure 10.0.0 10.1.0 2010-09 SP-49
SP-100558 1706 2 B Solution for terminating call barring based on service type
10.0.0 10.1.0 2010-09 SP-49 SP-100614 1750 1 A Remove SRB-Only PS handover
Support 10.0.0 10.1.0 2010-12 SP-50 SP-100688 1642 4 F MME overload control by
throttling of DL low priority traffic 10.1.0 10.2.0 2010-12 SP-50 SP-100690
1729 3 F Clarification of Delete Session Request information elements 10.1.0
10.2.0 2010-12 SP-50 SP-100685 1753 1 F LIPA L-GW clarification on static GW
and VPLMN LIPA roaming 10.1.0 10.2.0 2010-12 SP-50 SP-100687 1760 12 F
Correction and clarification of usage of low priority and MTC indicators
10.1.0 10.2.0 2010-12 SP-50 SP-100687 1761 5 F OTA configuration for MTC
10.1.0 10.2.0 2010-12 SP-50 SP-100687 1767 3 F Include session management
back-off timer for priority users/emergency services during APN based
congestion control 10.1.0 10.2.0 2010-12 SP-50 SP-100683 1770 1 F Alignment of
Network Triggered Service Request procedure 10.1.0 10.2.0 2010-12 SP-50
SP-100690 1778 - F Correction of HSS Initiated Subscribed QoS Modification
10.1.0 10.2.0 2010-12 SP-50 SP-100689 1779 1 D Correction of Reference 10.1.0
10.2.0 2010-12 SP-50 SP-100679 1783 - A Alignment of selective camping
procedures with Stage 3 10.1.0 10.2.0 2010-12 SP-50 SP-100832 1794 2 F
Restricting IPv6 interface identifier generation to protect usage patterns
10.1.0 10.2.0 2010-12 SP-50 SP-100864 1795 8 F Corrections and additions to
the MTC specific ACB and RRC connection reject functions 10.1.0 10.2.0 2010-12
SP-50 SP-100683 1806 - F Correction of network triggered service request.
10.1.0 10.2.0 2010-12 SP-50 SP-100685 1808 3 F Trigger for providing LGW
address 10.1.0 10.2.0 2010-12 SP-50 SP-100680 1813 6 A Handling of dedicated
bearer related procedures during handover 10.1.0 10.2.0 2010-12 SP-50
SP-100680 1823 2 A Missed corrections for time zone reporting 10.1.0 10.2.0
2010-12 SP-50 SP-100686 1824 1 F Deactivating direct path during S1 Release
for LIPA 10.1.0 10.2.0 2010-12 SP-50 SP-100687 1831 4 F Clarifying the
condition for a UE configured for MTC to perform Attach procedure 10.1.0
10.2.0 2010-12 SP-50 SP-100686 1838 2 F Addition of correlation identifier for
LIPA in service request procedure 10.1.0 10.2.0 2010-12 SP-50 SP-100685 1851 3
F Efficient SIPTO GW re-selection 10.1.0 10.2.0 2010-12 SP-50 SP-100685 1853 3
F LIPA corrections in Service Request procedures 10.1.0 10.2.0 2010-12 SP-50
SP-100686 1862 3 F Optimised paging for Local IP Access 10.1.0 10.2.0 2010-12
SP-50 SP-100685 1863 2 F Clarification on Correlation ID 10.1.0 10.2.0 2010-12
SP-50 SP-100690 1865 1 F TAU Triggers correction 10.1.0 10.2.0 2010-12 SP-50
SP-100680 1867 1 A Correction on MME overload condition 10.1.0 10.2.0 2010-12
SP-50 SP-100683 1868 1 A Clarification on the use of ARP and EPS bearer ID in
Downlink Data Notification 10.1.0 10.2.0 2010-12 SP-50 SP-100687 1871 1 F
Clarifications of MTC overload protection overview 10.1.0 10.2.0 2010-12 SP-50
SP-100686 1886 2 F No S-GW impact for LIPA solution 10.1.0 10.2.0 2010-12
SP-50 SP-100686 1887 - F Correction of NOTE placement. 10.1.0 10.2.0 2010-12
SP-50 SP-100685 1897 1 F LIPA PDN management 10.1.0 10.2.0 2010-12 SP-50
SP-100855 1907 2 F eMPS subscription data clarification 10.1.0 10.2.0 2010-12
SP-50 SP-100685 1911 5 F Correction to LIPA deactivation 10.1.0 10.2.0 2010-12
SP-50 SP-100710 1914 5 F APN based congestion control: Update Stage 2 text
based on CT1\'s feedback 10.1.0 10.2.0 2010-12 SP-50 SP-100680 1921 1 A Update
to TAU regarding selective camping 10.1.0 10.2.0 2010-12 SP-50 SP-100689 1928
1 C Prefix Exclusion within the Prefix Delegation Procedure 10.1.0 10.2.0
2010-12 SP-50 SP-100689 1936 - F Clarification on MTU size 10.1.0 10.2.0
2010-12 SP-50 SP-100690 1937 1 F Core Network selection function 10.1.0 10.2.0
2010-12 SP-50 SP-100680 1939 - A Restriction that an eNB serves a Single Time
Zone 10.1.0 10.2.0 2010-12 SP-50 SP-100790 1940 1 F Clarification of Iu
Release in UTRAN to E-UTRAN Inter RAT handover 10.1.0 10.2.0 2011-01 - - - - -
Update of LTE logo to LTE-Advanced logo 10.2.0 10.2.1 2011-03 SP-51 SP-110079
1789 2 F Correction of eNB F-TEID storage in MME 10.2.1 10.3.0 2011-03 SP-51
SP-110079 1792 3 F New Modify Access Bearers procedure 10.2.1 10.3.0 2011-03
SP-51 SP-110079 1793 2 F MABR procedure at inter-MME intra-SGW TAU procedure
10.2.1 10.3.0 2011-03 SP-51 SP-110065 1827 3 A UE Context Modification(CSFB)
retry when X2 HO fails 10.2.1 10.3.0 2011-03 SP-51 SP-110079 1885 1 F
Correction to the PDN GW initiated QoS update 10.2.1 10.3.0 2011-03 SP-51
SP-110070 1910 1 F Correction of wrong S1 message in Attach procedure 10.2.1
10.3.0 2011-03 SP-51 SP-110079 1919 1 F Correction of UE or MME requested PDN
disconnection 10.2.1 10.3.0 2011-03 SP-51 SP-110079 1941 - F Update the ME
identity to the HSS 10.2.1 10.3.0 2011-03 SP-51 SP-110079 1947 1 F Corrections
to TAU procedure with Serving GW change 10.2.1 10.3.0 2011-03 SP-51 SP-110079
1948 - F Corrections to the figure in Annex F 10.2.1 10.3.0 2011-03 SP-51
SP-110060 1951 1 A Correction to authentication vectors forwarding between MME
and SGSN 10.2.1 10.3.0 2011-03 SP-51 SP-110077 1954 1 F UE SRVCC capability
notification to HSS 10.2.1 10.3.0 2011-03 SP-51 SP-110079 1955 1 F NAS
explicit signalling clarification 10.2.1 10.3.0 2011-03 SP-51 SP-110079 1956 1
F Introduction of NAS explicit indication 10.2.1 10.3.0 2011-03 SP-51
SP-110079 1957 - F Clarification in RAU without SGW change procedure 10.2.1
10.3.0 2011-03 SP-51 SP-110079 1958 1 F Update X2 HO for HeNB mobility
enhancement 10.2.1 10.3.0 2011-03 SP-51 SP-110065 1960 3 A Correction of
S1-release procedure 10.2.1 10.3.0 2011-03 SP-51 SP-110065 1962 - A Alignment
of UE Time Zone for Create Session Request 10.2.1 10.3.0 2011-03 SP-51
SP-110065 1964 1 A UE Time Zone in Service Request procedure 10.2.1 10.3.0
2011-03 SP-51 SP-110079 1965 1 F Correction on MME overload condition 10.2.1
10.3.0 2011-03 SP-51 SP-110077 1971 1 F Correction of delete session request
procedure 10.2.1 10.3.0 2011-03 SP-51 SP-110079 1972 - F Updating RFSP index
10.2.1 10.3.0 2011-03 SP-51 SP-110079 1973 1 F ISR Correction for PSHO 10.2.1
10.3.0 2011-03 SP-51 SP-110061 1977 - A Deletion of Notify Messages in PGW-
initiated Bearer Deactivation Procedure 10.2.1 10.3.0 2011-03 SP-51 SP-110070
1978 2 F Correction of S1 Release impact for LIPA 10.2.1 10.3.0 2011-03 SP-51
SP-110077 1980 1 D The correction for reference of TS 29.274 10.2.1 10.3.0
2011-03 SP-51 SP-110061 1983 1 A Correction of Gn/Gp SGSN to MME Tracking Area
Update procedure 10.2.1 10.3.0 2011-03 SP-51 SP-110079 1989 1 F Correction of
TAU procedure with Serving GW change 10.2.1 10.3.0 2011-03 SP-51 SP-110079
1994 2 C Support for inter PLMN mobility 10.2.1 10.3.0 2011-03 SP-51 SP-110079
1995 2 B Introducing support for relaying for E-UTRAN 10.2.1 10.3.0 2011-03
SP-51 SP-110067 1996 1 F ECN support for UTRAN 10.2.1 10.3.0 2011-03 SP-51
SP-110070 1997 2 F Clarify SIPTO reconnection 10.2.1 10.3.0 2011-03 SP-51
SP-110072 2000 4 F Applying EMM backoff timer at PLMN change 10.2.1 10.3.0
2011-03 SP-51 SP-110072 2006 2 F Correction of Low Access Priority Indictor
Handling 10.2.1 10.3.0 2011-03 SP-51 SP-110070 2008 1 F Correction of L-GW
functions for 23.401 10.2.1 10.3.0 2011-03 SP-51 SP-110072 2011 - F DL low
priority traffic throttling 10.2.1 10.3.0 2011-03 SP-51 SP-110072 2023 1 F
Clarifying the scope of APN based SM congestion control 10.2.1 10.3.0 2011-03
SP-51 SP-110072 2025 2 F Cleanup of NIMTC related aspects 10.2.1 10.3.0
2011-03 SP-51 SP-110072 2027 5 F Low access priority indication handling
10.2.1 10.3.0 2011-03 SP-51 SP-110070 2030 2 F LIPA PDN disconnection for X2
HO 10.2.1 10.3.0 2011-03 SP-51 SP-110072 2033 1 F UE configuration
clarifications 10.2.1 10.3.0 2011-03 SP-51 SP-110065 2039 2 A CSFB retry in S1
HO procedure 10.2.1 10.3.0 2011-03 SP-51 SP-110070 2041 - F PDN GW initiated
bearer deactivation for LIPA 10.2.1 10.3.0 2011-03 SP-51 SP-110070 2047 - F
Correction of LIPA PDN connection release in TAU procedure 10.2.1 10.3.0
2011-03 SP-51 SP-110070 2048 2 F LIPA related corrections 10.2.1 10.3.0
2011-03 SP-51 SP-110072 2053 1 F Clarification of low priority traffic for DL
Data Notification reject 10.2.1 10.3.0 2011-03 SP-51 SP-110060 2059 1 A
Modifying and rejecting bearer level QoS parameter QCI for default bearer
10.2.1 10.3.0 2011-03 SP-51 SP-110072 2060 4 F Congestion control in MM
procedures 10.2.1 10.3.0 2011-06 SP-52 SP-110336 2021 1 F Clarifying handling
of DDN in MME for connected state UE 10.3.0 10.4.0 2011-06 SP-52 SP-110324
2022 4 A Updating APN-AMBR in case of multiple PDNs to the same APN 10.3.0
10.4.0 2011-06 SP-52 SP-110336 2049 4 C MME overload procedure enhancement
10.3.0 10.4.0 2011-06 SP-52 SP-110331 2062 2 F Correct the condition of re-
selecting PDN GW in case of APN congestion 10.3.0 10.4.0 2011-06 SP-52
SP-110336 2063 2 D Correction of mobility procedures 10.3.0 10.4.0 2011-06
SP-52 SP-110336 2064 3 F Clarify how to off-load Relay from MME 10.3.0 10.4.0
2011-06 SP-52 SP-110330 2069 2 F Clarification on the association of PDN
connections with SIPTO permissions 10.3.0 10.4.0 2011-06 SP-52 SP-110336 2072
3 F Multiple radio UE PDN type selection 10.3.0 10.4.0 2011-06 SP-52 SP-110336
2073 1 F Inter PLMN mobility functionality corrections 10.3.0 10.4.0 2011-06
SP-52 SP-110331 2074 2 F Applicability of MTC functionality 10.3.0 10.4.0
2011-06 SP-52 SP-110336 2076 1 F S10 and S3 use across PLMN\'s 10.3.0 10.4.0
2011-06 SP-52 SP-110324 2078 1 A Intra CN node multi PLMN support 10.3.0
10.4.0 2011-06 SP-52 SP-110331 2080 1 F MME overload control for M2M 10.3.0
10.4.0 2011-06 SP-52 SP-110331 2081 1 F MTC overload control handling for
EUTRAN 10.3.0 10.4.0 2011-06 SP-52 SP-110331 2093 3 F Clarification of
reachability management for a back-off UE 10.3.0 10.4.0 2011-06 SP-52
SP-110331 2095 1 F Clarification to the APN based session management
congestion control procedure 10.3.0 10.4.0 2011-06 SP-52 SP-110336 2098 - F
Corrections to charging 10.3.0 10.4.0 2011-06 SP-52 SP-110336 2100 3 F
Clarification of GW functions in relay architecture 10.3.0 10.4.0 2011-06
SP-52 SP-110392 2105 7 F Network initiated requests during NAS level
congestion control 10.3.0 10.4.0 2011-06 SP-52 SP-110336 2107 - F
Clarification on Dedicated Bearer Activation with ISR 10.3.0 10.4.0 2011-06
SP-52 SP-110336 2110 1 F Corrections in Annex D 10.3.0 10.4.0 2011-06 SP-52
SP-110330 2113 1 F LIPA PDN disconnection during X2-based handover procedures
10.3.0 10.4.0 2011-06 SP-52 SP-110336 2116 1 F Inter-PLMN Handover to a CSG
cell 10.3.0 10.4.0 2011-06 SP-52 SP-110330 2121 2 F Definitions of Correlation
ID and LIPA PDN connection 10.3.0 10.4.0 2011-06 SP-52 SP-110321 2124 1 A MME
Behaviour when MS Network Capability is not included in Attach/TAU 10.3.0
10.4.0 2011-06 SP-52 SP-110331 2125 1 F Clarification of TAU Operation for
Backed-off UE 10.3.0 10.4.0 2011-06 SP-52 SP-110324 2127 1 A S1AP and 24.301
message reception order at MME 10.3.0 10.4.0 2011-06 SP-52 SP-110338 2128 2 F
Completion of Relay procedure overview 10.3.0 10.4.0 2011-06 SP-52 SP-110331
2129 2 F UE configuration for specific handling of the forbidden PLMN lists.
10.3.0 10.4.0 2011-09 SP-53 SP-110456 2148 1 A Adding Associated PLMN to CSG
entries 10.4.0 10.5.0 2011-09 SP-53 SP-110458 2133 3 F Emergency Attach when
MM back-off timer is running 10.4.0 10.5.0 2011-09 SP-53 SP-110458 2145 2 F
Correction and Clean Up for MTC overload issue 10.4.0 10.5.0 2011-09 SP-53
SP-110458 2156 - F Alignment of text of UE behaviour when EMM back-off timer
applies 10.4.0 10.5.0 2011-09 SP-53 SP-110458 2160 - F Alignment of EWT timer
handling with RAN 10.4.0 10.5.0 2011-09 SP-53 SP-110460 2026 6 F
Unidirectional bearer 10.4.0 10.5.0 2011-09 SP-53 SP-110459 2138 - F APN
resolution in the VPLMN if using LBO 10.4.0 10.5.0 2011-09 SP-53 SP-110460
2146 1 F Correct AS security information for MME 10.4.0 10.5.0 2011-09 SP-53
SP-110460 2163 1 F Further clarifications on Relay Node architecture 10.4.0
10.5.0 2011-09 SP-53 SP-110459 2168 2 F Dual-stack and Single-stack PDN Types
in Subscription Data 10.4.0 10.5.0 2011-09 SP-53 SP-110459 2169 1 F InterPLMN
Handover to a CSG cell 10.4.0 10.5.0 2011-12 SP-54 SP-110734 2170 2 F
Relationship between LIPA permissions and SIPTO permissions 10.5.0 10.6.0
2011-12 SP-54 SP-110736 2177 1 F Throttling of Downlink Data Notification
Request when ISR is activated 10.5.0 10.6.0 2011-12 SP-54 SP-110732 2181 1 A
Restriction on inter-PLMN handover to a CSG cell 10.5.0 10.6.0 2011-12 SP-54
SP-110736 2190 1 F UE initiates detach procedure when MM backoff timer is
running 10.5.0 10.6.0 2011-12 SP-54 SP-110736 2191 1 F APN based Session
Management congestion control clarification 10.5.0 10.6.0 2011-12 SP-54
SP-110729 2197 3 A QoS mapping 10.5.0 10.6.0 2011-12 SP-54 SP-110736 2205 2 F
Back-off timer handling in connected mode mobility 10.5.0 10.6.0 2011-12 SP-54
SP-110736 2212 1 F Emergency attach when MM back-off timer is running 10.5.0
10.6.0 2011-12 SP-54 SP-110732 2246 2 A Handover to CSG cell with emergency
bearer 10.5.0 10.6.0 2011-12 SP-54 SP-110764 2204 2 A Correction to UE warning
message indication with regards to \"digital signature\" and \"timestamp\"
10.5.0 10.6.0 2012-03 SP-55 SP-120074 2189 2 F Service user correction for MTC
congestion control 10.6.0 10.7.0 2012-03 SP-55 SP-120073 2261 1 F LIPA PDN
connection handling when the UE\'s CSG membership changes 10.6.0 10.7.0
2012-03 SP-55 SP-120073 2262 - F Correction of the PDN GW selection for the
APN with SIPTO permission 10.6.0 10.7.0 2012-03 SP-55 SP-120064 2268 1 A
Warning System details move to 23.041 10.6.0 10.7.0 2012-03 SP-55 SP-120076
2278 1 F Clarify reference point used inter-PLMN and intra-PLMN 10.6.0 10.7.0
2012-03 SP-55 SP-120076 2291 1 F Permit RRC Release in parallel with S1
Release 10.6.0 10.7.0 2012-06 SP-56 SP-120236 2378 2 F Clarification for
E-UTRAN Tracking Area Update without S-GW and MME Change 10.7.0 10.8.0 2012-06
SP-56 SP-120235 2388 1 F Missing Paging Priority for Bearer
activation/modification 10.7.0 10.8.0 2012-06 SP-56 SP-120233 2397 1 A Re-
attach for emergency bearer service 10.7.0 10.8.0 2012-06 SP-56 SP-120236 2412
1 F ISR deactivation for PGW-initiated Bearer Deactivation Procedure 10.7.0
10.8.0 2012-06 SP-56 SP-120236 2426 - F Fix wrong statement for the Reporting
Action related storage 10.7.0 10.8.0 2012-12 SP-58 SP-120715 2446 2 F
Correction of packet filters in TFT 10.8.0 10.9.0 2012-12 SP-58 SP-120713 2465
1 A Clarification on SDU Error Ratio value when mapping EPS QCI=4 to Release
99 QoS parameters 10.8.0 10.9.0 2012-12 SP-58 SP-120710 2469 3 A Restriction
of Inter RAT handover/release with redirection between E-UTRAN and UTRAN/GERAN
10.8.0 10.9.0 2012-12 SP-58 SP-120715 2477 4 F Notifying UE on specific PDN
disconnection while the UE is in the idle state 10.8.0 10.9.0 2012-12 SP-58
SP-120713 2486 1 A Regular deterministic IMEI fetch 10.8.0 10.9.0 2013-03
SP-59 SP-130081 2497 2 F Delayed reporting of Serving Network, and UE Time
Zone 10.9.0 10.10.0 2013-03 SP-59 SP-130100 2512 - A ULI age removal 10.9.0
10.10.0 2014-03 SP-63 SP-140098 2642 - F Correction of IETF reference on
prefix delegation 10.10.0 10.11.0 2014-06 SP-64 SP-140251 2661 1 F Correction
on priority paging 10.11.0 10.12.0