...
###### ## Annex B Deployment Architecture (Informative):
...
transported via the Security Gateway is shown in Figure B.1-2. Note: If the
Security Gateway and the HNB-GW are co-located then the co-located node should
support IP routing.
...
Figure B.1-3. HNB access network deploying Iurh connectivty via the HNB-GW.
#