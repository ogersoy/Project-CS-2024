...
configured with Four Carrier HSDPA operation with or without MIMO in CELL_DCH
state. Four Carrier HSDPA operation may be activated and deactivated using HS-
SCCH orders.
...
\- The maximum number of MAC-ehs PDUs per TTI is 6, when three serving HS-DSCH
cells are active. The maximum number of MAC-ehs PDUs per TTI is 8, when four
serving HS-DSCH cells are active.
