...
g) Valid for packet switched traffic.
h) IMS.
### 4.8.5 Gateway Control session establishment related measurements
The measurements defined in subclauses 4.8.5.1, 4.8.5.2 and 4.8.5.3 are
subject to the \"2 out of 3 approach\".
#### 4.8.5.1 Attempted Gateway Control session establishment
a) This measurement provides the number of attempted session establishments
...
g) Valid for packet switched traffic.
h) IMS.
#### 4.8.5.2 Successful Gateway Control session establishment
a) This measurement provides the number of successful session establishments
initiated by BBERF to PCRF.
b) CC.
...
BBERF. (see 3GPP TS 29.213[33]).
d) A single integer value.
e) QoS.EstabfGCSuccCCA.
f) PCRFFunction.
g) Valid for packet switched traffic.
h) IMS.
#### 4.8.5.3 Failed Gateway Control session establishment
...
(3GPP TS 29.213[33]). Each failed Gateway Control session establishments will
be added to the relevant measurement according to the failure cause. Possible
failure causes are included in RFC 3588 [15]. The sum of all supported per
...
##### 4.10.7.1.1 Attempted Communication Diversion (CDIV) service procedure
a) This measurement provides the number of attempted Communication Diversion
(CDIV) service procedure.
b) CC.
c) On receipt by the AS of a SIP_INVITE message to create the Communication
Diversion (CDIV) service (see 3GPP TS 24.604 [36]).
d) Each measurement is an integer value. The number of measurements is equal
to the number of services plus a possible sum value identified by the ._sum_
suffix.
e) The measurement name has the form SC.AttCdiv._Subservice\_ where
_Subservice_ identifies the specific service. The following subcounters with
specific services(see 3GPP TS 24.604 [36]) are required:
\- SC.AttCdiv.Cfu Communication Forwarding Unconditional (CFU) service;
\- SC.AttCdiv.Cfb Communication Forwarding on Busy user (CFB) service;
\- SC.AttCdiv.Cfnr Communication Forwarding on no Reply (CFNR) service;
\- SC.AttCdiv.Cfnrc Communication Forwarding on Subscriber Not Reachable
(CFNRc) service;
f) ASFunction.
g) Valid for packet switched traffic.
h) IMS.
##### 4.10.7.1.2 Successful Communication Diversion (CDIV) service procedure
a) This measurement provides the number of successful Communication Diversion
(CDIV) service procedure.
b) CC.
c) On transmission by the AS of a SIP_200_OK message indicating the
Communication Diversion (CDIV) service is successfully created (see 3GPP TS
24.604 [36]).
d) Each measurement is an integer value. The number of measurements is equal
to the number of services plus a possible sum value identified by the ._sum_
suffix.
e) The measurement name has the form SC.SuccCdiv._Subservice\_ where
_Subservice_ identifies the specific service. The following subcounters with
specific services(see 3GPP TS 24.604 [36]) are required:
\- SC.SuccCdiv.Cfu Communication Forwarding Unconditional (CFU) service;
\- SC.SuccCdiv.Cfb Communication Forwarding on Busy user (CFB) service;
\- SC.SuccCdiv.Cfnr Communication Forwarding on no Reply (CFNR) service;
\- SC.SuccCdiv.Cfnrc Communication Forwarding on Subscriber Not Reachable
(CFNRc) service;
f) ASFunction.
g) Valid for packet switched traffic.
h) IMS.
#### 4.10.7.2 Originating Identification Presentation (OIP) service
establishments
##### 4.10.7.2.1 Attempted Originating Identification Presentation (OIP)
procedure
a) This measurement provides the number of attempted Originating
Identification Presentation (OIP) service procedure.
b) CC.
c) On receipt by the AS of a SIP_INVITE message to create the Originating
Identification Presentation (OIP) service (see 3GPP TS 24.607 [37]).
d) A single integer value.
e) SC.AttOip.
f) ASFunction.
g) Valid for packet switched traffic.
h) IMS.
##### 4.10.7.2.2 Successful Originating Identification Presentation (OIP)
procedure
a) This measurement provides the number of successful Originating
Identification Presentation (OIP) service procedure.
b) CC.
c) On transmission by the AS of a SIP_200_OK message indicating the
Originating Identification Presentation (OIP) service is successfully created
(see 3GPP TS 24.607 [37]).
d) A single integer value.
e) SC.SuccOip.
f) ASFunction.
g) Valid for packet switched traffic.
...
Emergency session function is an important function at P-CSCF. The emergency
session related measurements can help operator to evaluate the emergency
session processing capacity of IMS network entity, analyze the establishement
failure reasons of emergency session and solve the problems. The emergency
session establishment procedure may not comply with the eme emergency
configuration policy, and these measurements can be used to check the
...
Reply (CFNR) and Communication Forwarding on Subscriber Not Reachable (CFNRc).
The **Communications Diversion (CDIV)** service related measurements at AS can
reflect the performance of the **CDIV** service processing and the status of
user using the CDIV service.
...
OIP service related measurements at AS can reflect the performance of the OIP
service processing and the status of user using the OIP service.
