...
(segmentation) and packetization as specified by each codec [22], [24] and RTP
payload format [25], [29] specification. Further guidelines on how the video
...
\- For H.264 / MPEG-4 (Part 10) AVC, a slice corresponds to such a segment.
...
## 10.2 Speech
### 10.2.0 General
To reduce the risk for confusion in the sender, it is beneficial if the
signaling for the media adaptation is the same regardless of which triggers
are used in the adaptation. The adaptation for AMR and AMR-WB includes
adapting the media bit-rate, the frame aggregation and the redundancy level.
...
RTCP reports and TMMBR messages. **This adaptation should be performed while
maintaining a balance between spatial quality and temporal resolution, which
matches the bitrate and image size.** Some examples are given in Annex B. For
the handling of packet loss signaled through AVPF NACK and PLI, or for rate
adaptation with RTCP reports and TMMBR messages, it is recommended that video
sender is able to dynamically adapt to the reported conditions, in particular
to facilitate the operation of quality-recovery techniques pertinent to the
...
payload. When receiving inband CMR in the payload from the remote network, the
MTSI MGW does not need to move the adaptation signalling to RTCP APP packets
...
If an MSRP client supports video, H.264 (AVC) Constrained Baseline Profile
Level 1.2 decoder without requirements on output timing conformance [24] shall
be supported. In addition, an MSRP client should support:
\- H.263 Profile 0 Level 45 decoder [22].
...
value "MP4V-ES" refers to the MPEG4 video codec as defined in MPEG. The value
"H264" refers to the H.264 codec as defined by MPEG and ITU. The usage of
H.263 and H.264 codecs (profiles, levels etc) is described in the document TS
...
This leaf gives the profile-level-id of H.264 video codec, which indicates the
profile that the codec supports and the highest level supported for the
signaled profile [24], [25].
\- Occurrence: ZeroOrOne
\- Format: chr
\- Minimum Access Types: Get
**/_\ _/Video/\/ImageAttr**
...
ConRef Undefined
* * *
An example of SDP offer for text session is shown in Table A.5.1, which
includes two RTP payload types, with and without redundancy, for T.140.
Parameter set with a higher priority applies to the preferred case, i.e.,
sending text with 200 % of redundancy while parameter set with a lower
priority corresponds to the case of no redundancy. Note that the value of the
TextFormat node is used in the "a=rtpmap" line for no redundancy while "red"
is used in the "a=rtpmap" line for redundancy.
...
ConRef Undefined
* * *
An example of SDP offer for video session is shown in Table A.4.9, which
includes two RTP payload types for H.264 and H.263 respectively. Although the
"b=AS" value can also be computed with the Source and PayloadSize nodes, a
different value with appropriate implementation margin can be directly
assigned to the AS node. If the AS, Source, and PayloadSize nodes are defined
together, the AS node value should be used for setting "b=AS". In Table 15.3,
the "b=AS" values of 315 and 57 kbps, for H.264 and H.263 respectively, are
computed assuming IPv4 addressing. Note that the Priority nodes of H.264 and
H.263 are assigned values of 5 and 3 respectively, which shows that depending
on service policy, parameters sets of lower priority may be preferred in the
construction of SDP offer. If the ImageAttr node is to be defined, as for
H.264 in Table A.4.9, the maximum image size in either the Send or Recv node
shall not exceed the maximum size limited by the offered codec level, which is
352x288 for Baseline profile at level 1.1.
...
level-id parameter indicates Constrained Baseline profile at level 1.2, which
supports bitrates up to 384 kbps. It also indicates, by using so‑called
constraint-set flags, that the bit stream can be decoded by any Baseline, Main
or Extended profile decoder. The third parameter, sprop-parameter-sets,
includes base-64 encoded sequence and picture parameter set NAL units that are
referred by the video bit stream. The sequence parameter set used here
includes syntax that specifies the number of re‑ordered frames to be zero so
that latency can be minimized. The second offer in the SDP is H.263 Profile 0
(Baseline) at level 10. It is used here as a fallback in case the other MTSI
client does not support H.264/AVC. The bandwidth (including IP, UDP and RTP
overhead) for video is restricted to 315 kbps. A fallback to H.263 should
modify that bandwidth in the SDP answer to one more suited to the H.263 level.
...
+-------------------------------------------------------------+ | SDP answer | +-------------------------------------------------------------+ | m=video 49154 RTP/AVPF 99 | | | | a=acfg:1 t=1 | | | | b=AS:315 | | | | b=RS:0 | | | | b=RR:2500 | | | | a=rtpmap:99 H264/90000 | | | | a=fmtp:99 packetization-mode=0; profile-level-id=42e00c; \ | | | | sprop-parameter-sets=J0LgDJWgUH6Af1A=,KM46gA== | | | | a=rtcp-fb:* trr-int 5000 | | | | a=rtcp-fb:* nack | | | | a=rtcp-fb:* nack pli | | | | a=rtcp-fb:* ccm fir | | | | a=rtcp-fb:* ccm tmmbr | +-------------------------------------------------------------+
...
level (level 1.2) of H.264/AVC as well as the minimum constraint set, there is
...
Table A.4.4a: Example SDP answer with H.263 fallback
+-------------------------------+ | SDP answer | +-------------------------------+ | m=video 49154 RTP/AVPF 99 | | | | a=acfg:1 t=1 | | | | b=AS:48 | | | | b=RS:0 | | | | b=RR:2500 | | | | a=rtpmap:100 H263-2000/90000 | | | | a=fmtp:100 profile=0;level=10 | +-------------------------------+
In this example the responding pre-Rel-10 MTSI client doesn't support H.264
and wishes to fallback to H.263. The bandwidth is constrained further by
reducing the bandwidth in b=AS.
If the SDP offer or answer is sent by a MTSI MGW that inter-works between MTSI
and 3G-324M, b=AS for video can be set such that the video bandwidth of MTSI
matches that of 3G-324M. For example, video bit-rate of 3G-324M on a 64 kbps
UMTS bearer is about 48 kbps, assuming multiplex level 2 (H.223 Annex B), and
the corresponding b=AS, assuming 75 bytes of H.263 payload and 2 bytes of
payload header per packet, will be 57 kbps [4], [29].
## A.4.3 Void
...
indicates single NAL unit mode. This is the default mode and it is therefore
not necessary to include this parameter (see RFC 6184 [25]). The profile-
level-id parameter indicates Baseline profile at level 1.2, which supports
...
agreed with the offered bandwidth of 315 kbps and Baseline profile at level
1.2. From the four image sizes offered, the responding MTSI client included
...
attribute at media level. In this example, the H.264 video stream should not
...
This SDP example is based on the SDP example found in Tables A.4.3 and A.4.4,
except that the offerend bandwidth is 128 kbps instead of 57 kbps. The
negotiation for ECN has been added.
Table A.12.2.1: Example SDP offer for H.263 and H.264 video with ECN
+------------------------------------------------------------+ | SDP offer | +------------------------------------------------------------+ | m=video 49154 RTP/AVP 99 100 | | | | a=tcap:1 RTP/AVPF | | | | a=pcfg:1 t=1 | | | | b=AS:128 | | | | b=RS:0 | | | | b=RR:2500 | | | | a=rtpmap:99 H264/90000 | | | | a=fmtp:99 packetization-mode=0;profile-level-id=42e00a; \ | | | | sprop-parameter-sets=J0LgCpWgsToB/UA=,KM4Gag== | | | | a=rtpmap:100 H263-2000/90000 | | | | a=fmtp:100 profile=0;level=10 | | | | a=rtcp-fb:* trr-int 5000 | | | | a=rtcp-fb:* nack | | | | a=rtcp-fb:* nack pli | | | | a=rtcp-fb:* ccm tmmbr | | | | a=rtcp-fb:* ccm fir | | | | a=ecn-capable-rtp: leap ect=0 | +------------------------------------------------------------+ | SDP answer | +------------------------------------------------------------+ | m=video 49154 RTP/AVPF 99 | | | | a=acfg:1 t=1 | | | | b=AS:128 | | | | b=RS:0 | | | | b=RR:2500 | | | | a=rtpmap:99 H264/90000 | | | | a=fmtp:99 packetization-mode=0;profile-level-id=42e00a; \ | | | | sprop-parameter-sets=J0LgCpWgsToB/UA=,KM4Gag== | | | | a=rtcp-fb:* trr-int 5000 | | | | a=rtcp-fb:* nack | | | | a=rtcp-fb:* nack pli | | | | a=rtcp-fb:* ccm tmmbr | | | | a=rtcp-fb:* ccm fir | | | | a=ecn-capable-rtp: leap ect=0 | +------------------------------------------------------------+
**Comments:**
The SDP offer includes the SDP attribute 'ecn-capable-rtp' to indicate that
ECN is supported. The SDP offer further includes the parameters: 'leap' to
indicate that the leap-of-faith initiation method is to be used; and 'ect=0'
...
negotiation for ECN has been added.
Table A.12.2.2: Example SDP offer for H.263 and H.264 video with ECN
+------------------------------------------------------------+ | SDP offer | +------------------------------------------------------------+ | m=video 49154 RTP/AVP 99 100 | | | | a=tcap:1 RTP/AVPF | | | | a=pcfg:1 t=1 | | | | b=AS:128 | | | | b=RS:0 | | | | b=RR:2500 | | | | a=rtpmap:99 H264/90000 | | | | a=fmtp:99 packetization-mode=0;profile-level-id=42e00a; \ | | | | sprop-parameter-sets=J0LgCpWgsToB/UA=,KM4Gag== | | | | a=rtpmap:100 H263-2000/90000 | | | | a=fmtp:100 profile=0;level=10 | | | | a=ecn-capable-rtp: leap ect=0 | | | | a=rtcp-fb:* trr-int 5000 | | | | a=rtcp-fb:* nack | | | | a=rtcp-fb:* nack pli | | | | a=rtcp-fb:* ccm tmmbr | | | | a=rtcp-fb:* ccm fir | | | | a=rtcp-fb:* nack ecn | | | | a=rtcp-xr:ecn-sum | +------------------------------------------------------------+ | SDP answer (non-MTSI client) | +------------------------------------------------------------+ | m=video 49154 RTP/AVPF 99 | | | | a=acfg:1 t=1 | | | | b=AS:128 | | | | b=RS:0 | | | | b=RR:2500 | | | | a=rtpmap:99 H264/90000 | | | | a=fmtp:99 packetization-mode=0;profile-level-id=42e00a; \ | | | | sprop-parameter-sets=J0LgCpWgsToB/UA=,KM4Gag== | | | | a=ecn-capable-rtp: leap ect=0 | | | | a=rtcp-fb:* trr-int 5000 | | | | a=rtcp-fb:* nack | | | | a=rtcp-fb:* nack pli | | | | a=rtcp-fb:* nack ecn | | | | a=rtcp-xr:ecn-sum | +------------------------------------------------------------+
**Comments:**
The SDP offer is similar to the offer in Table A.12.xx. The line "a=rtcp-fb:*
nack ecn" is included to indicate that the RTCP AVPF ECN feedback messages can
...
descriptor 'speech\'
* * *
# E.3 Bi-directional video (H.263, 128 kbps, IPv4, RTCP and MBR=GBR bearer)
The video bandwidth is assumed to be 128 kbps and the IP overhead 5 kbps
(assuming 15fps and 1 IP paket per frame), resulting in 133 kbps. The transfer
delay for video is different from other media.
Table E.2: QoS mapping for bi-directional video (H.263, 128 kbps, IPv4, RTCP
and MBR=GBR bearer)
* * *
Traffic class Conversational class Notes Delivery order No The application
should handle packet reordering. Maximum SDU size (octets) 1 400 Maximum size
...
to achieve an acceptable compromise between packet loss rate and speech
transport delay and delay variation. SDU error ratio 7*10^-3^ A packet loss
rate of 0.7 % per wireless link is in general sufficient for video services
Transfer delay (ms) 170 ms Indicates maximum delay for 95^th^ percentile of
the distribution of delay for all delivered SDUs between the UE and the PS
domain during the lifetime of a bearer service. Permits the derivation of the
RAN part of the total transfer delay for the radio access bearer. This
attribute allows RAN to set transport formats and H-ARQ/ARQ parameters such as
the discard timer. Guaranteed bitrate for uplink (kbps) 136 The total bit-rate
of a video codec (running at 128 kbps) adding IP/UDP/RTP overhead (assumed to
be 5 kbps) and RTCP (RS:0 and RR:5000 used in clause A.6 adds 2.5kbps). The
value is then rounded up to nearest 8 kbps value. Maximum bitrate for uplink
(kbps) 136 The same as the guaranteed bitrate. Guaranteed bitrate for downlink
(kbps) 136 The total bit-rate of a video codec (running at 128 kbps) adding
IP/UDP/RTP overhead (assumed to be 5 kbps) and RTCP (RS:0 and RR:5000 used in
clause A.6 adds 2.5kbps). The value is then rounded up to nearest 8 kbps
value. Maximum bitrate for downlink (kbps) 136 The same as the guaranteed
bitrate. Allocation/Retention priority subscribed value Indicates the relative
importance to other radio access bearers. It should be the same or next lower
value to the priority of a Conversational bearer with source statistics
descriptor 'speech\'. Source statistics descriptor 'unknown\'
* * *
...
(assuming 15fps and 2 IP pakets per frame), resulting in 202 kbps. The
...
statistics descriptor 'speech\'
* * *
# E.9 Bi-directional video (H.263, 128 kbps, IPv6, RTCP and MBR=GBR bearer)
The video bandwidth is assumed to be 128 kbps and the IP overhead 8 kbps
(assuming 15fps and 1 IP paket per frame), resulting in 136 kbps. The transfer
delay for video is different from other media. IPv6 is also assumed.
Table E.10: QoS mapping for bi-directional video (H.263, 128 kbps, IPv6, RTCP
and MBR=GBR bearer)
* * *
**Traffic class** **Conversational class** **Notes** Delivery order No The
application should handle packet reordering. Maximum SDU size (octets) 1 400
...
RAN part of the total transfer delay for the radio access bearer. This
attribute allows RAN to set transport formats and H-ARQ/ARQ parameters such as
the discard timer. Guaranteed bitrate for uplink (kbps) 144 The total bit-rate
of a video codec (running at 128 kbps) adding IP/UDP/RTP overhead (assumed to
be 8 kbps) and RTCP (RS:0 and RR:5000 used in clause A.6 adds 2.5kbps) rounded
up to nearest 8 kbps value. Maximum bitrate for uplink (kbps) 144 The same as
the guaranteed bitrate. Guaranteed bitrate for downlink (kbps) 144 The total
bit-rate of a video codec (running at 128 kbps) adding IP/UDP/RTP overhead
(assumed to be 8 kbps) and RTCP (RS:0 and RR:5000 used in clause A.6 adds
2.5kbps) rounded up to nearest 8 kbps value. Maximum bitrate for downlink
(kbps) 144 The same as the guaranteed bitrate. Allocation/Retention priority
subscribed value Indicates the relative importance to other radio access
bearers. It should be the same or next lower value to the priority of a
Conversational bearer with source statistics descriptor 'speech\'. Source
statistics descriptor 'unknown\'
* * *
...
(assuming 15fps and 2 IP pakets per frame), resulting in 208 kbps. The
...
* * *
# E.13 Bi-directional video (H.263, IPv4, RTCP and MBR>GBR bearer)
The video bandwidths used for defining MBR and GBR are assumed to be 128 kbps
and 64 kbps, respectively. The IPv4 overhead is 5 kbps (assuming 15fps and 1
IP paket per frame) for both MBR and GBR, resulting in 133 kbps and 69 kbps,
respectively. The transfer delay for video is different from other media.
Table E.14: QoS mapping for bi-directional video (H.263, IPv4, RTCP and
MBR>GBR bearer)
* * *
Traffic class Conversational class Notes Delivery order No The application
should handle packet reordering. Maximum SDU size (octets) 1400 Maximum size
...
RAN part of the total transfer delay for the radio access bearer. This
attribute allows RAN to set transport formats and H-ARQ/ARQ parameters such as
the discard timer. Guaranteed bitrate for uplink (kbps) 72 The total bit-rate
of a video codec (running at 64 kbps) adding IP/UDP/RTP overhead (assumed to
be 5 kbps) and RTCP (RS:0 and RR:5000 used in clause A.6 adds 2.5kbps). The
value is then rounded up to nearest multiple of 8 kbps. Maximum bitrate for
uplink (kbps) 136 The total bit-rate of a video codec (running at 128 kbps)
adding IP/UDP/RTP overhead (assumed to be 5 kbps) and RTCP (RS:0 and RR:5000
used in clause A.6 adds 2.5kbps). The value is then rounded up to nearest
multiple of 8 kbps. Guaranteed bitrate for downlink (kbps) 72 The total bit-
rate of a video codec (running at 64 kbps) adding IP/UDP/RTP overhead (assumed
to be 5 kbps) and RTCP (RS:0 and RR:5000 used in clause A.6 adds 2.5kbps). The
value is then rounded up to nearest multiple of 8 kbps. Maximum bitrate for
downlink (kbps) 136 The total bit-rate of a video codec (running at 128 kbps)
adding IP/UDP/RTP overhead (assumed to be 5 kbps) and RTCP (RS:0 and RR:5000
used in clause A.6 adds 2.5kbps). The value is then rounded up to nearest
multiple of 8 kbps. Allocation/Retention priority subscribed value Indicates
the relative importance to other radio access bearers. It should be the same
or next lower value to the priority of a Conversational bearer with source
statistics descriptor 'speech\'. Source statistics descriptor 'unknown\'
* * *
...
Source statistics descriptor 'speech\'
* * *
# E.17 Bi-directional video (H.263, IPv6, RTCP and MBR>GBR bearer)
The video bandwidths used for defining MBR and GBR are assumed to be 128 kbps
and 64 kbps, respectively. The IPv6 overhead is 8 kbps (assuming 15fps and 1
IP packet per frame) for both MBR and GBR, resulting in 136 kbps and 72 kbps,
respectively. The transfer delay for video is different from other media.
Table E.18: QoS mapping for bi-directional video (H.263, IPv6, RTCP and
MBR>GBR bearer)
* * *
Traffic class Conversational class Notes Delivery order No The application
should handle packet reordering. Maximum SDU size (octets) 1400 Maximum size
...
RAN part of the total transfer delay for the radio access bearer. This
attribute allows RAN to set transport formats and H-ARQ/ARQ parameters such as
the discard timer. Guaranteed bitrate for uplink (kbps) 80 The total bit-rate
of a video codec (running at 64 kbps) adding IP/UDP/RTP overhead (assumed to
be 8 kbps) and RTCP (RS:0 and RR:5000 used in clause A.6 adds 2.5kbps). The
value is then rounded up to nearest multiple of 8 kbps. Maximum bitrate for
uplink (kbps) 144 The total bit-rate of a video codec (running at 128 kbps)
adding IP/UDP/RTP overhead (assumed to be 8 kbps) and RTCP (RS:0 and RR:5000
used in clause A.6 adds 2.5kbps). The value is then rounded up to nearest
multiple of 8 kbps. Guaranteed bitrate for downlink (kbps) 80 The total bit-
rate of a video codec (running at 64 kbps) adding IP/UDP/RTP overhead (assumed
to be 8 kbps) and RTCP (RS:0 and RR:5000 used in clause A.6 adds 2.5kbps). The
value is then rounded up to nearest multiple of 8 kbps. Maximum bitrate for
downlink (kbps) 144 The total bit-rate of a video codec (running at 128 kbps)
adding IP/UDP/RTP overhead (assumed to be 8 kbps) and RTCP (RS:0 and RR:5000
used in clause A.6 adds 2.5kbps). The value is then rounded up to nearest
multiple of 8 kbps. Allocation/Retention priority subscribed value Indicates
the relative importance to other radio access bearers. It should be the same
...
# G.3 Session setup
An MTSI client offering a speech media session for speech and DTMF events
should include an offer for DTMF events according to the example in Table
G.3.1 when narrowband speech is offered and according to the example in Table
G.3.2 when both narrowband and wideband speech is offered. The answerer shall
select DTMF payload format(s) that match the selected speech codec(s).
...
\
\
\The 3GPP MTSI Service Network Profile Management Object\
\
\
\
\
...
\
\
\Identification for speech session parameter set\
\
\text/plain\
\
\
...
\
\
\Subnodes related to speech bandwidth\
\
\text/plain\
\
\
...
\
\
\RTCP bandwidth allocated to other participants in the RTP
session\
\
\text/plain\
\
...
\
\
\Identification for video session parameter set\
\
\text/plain\
\
\
...
\
\
\Subnodes related to video bandwidth\
\
\text/plain\
\
\
...
\
\
\Subnodes related to profile and level of video codec\
\
\text/plain\
\
\
...
\
\
\Subnodes related to profile and level of H.263\
\
\text/plain\
\
\
...
\
\
\Subnodes related to supported image size\
\
\text/plain\
\
\
...
\
\
\Supported image sizes for send direction\
\
\text/plain\
\
\
...
\
\
\Supported image sizes and their preferences for receive
direction\
\
\text/plain\
\
...
\
\
\Identification for text session parameter set\
\
\text/plain\
\
\
...
\
\
\Format of text conversation protocol\
\
\text/plain\
\
\
...
\
\
\Subnodes related to text bandwidth\
\
\text/plain\
\
\
...
\
\
\Shortest period between text transmissions\
\
\text/plain\
\
\
...
**\ **
**\ **
**\ 3GPP MTSI Media Adaptation Management Object\**
**\ **
**\ **
**\ **
**\ **
...
**\ **
**\ **
**\ Identification for video adaptation state machine\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Identification for speech adaptation state machine\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Subnodes related to packet loss rate\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Maximum packet loss rate tolerated when redundancy is not
employed\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Packet loss rate below which up-switch is attempted\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Packet loss rate above which the state is reversed\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Packet loss rate above which redundancy is
ineffective\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Duration for which MAX is computed\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Duration for which LOW is computed\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Duration for which STATE_REVERSION is computed\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Duration for which RED_INEFFECTIVE is computed\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Default duration for which unspecified PLRs are
computed\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Subnodes related to packet loss burst\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Number of packets lost during DURATION\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Duration for which LOST_PACKET is counted\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Subnodes related to explicit congestion notification to
IP\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Switch to enable or disable ECN-based adaptation\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Minimum bit rate to use during ECN-based adaptation\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Switch to enable or disable stepwise downswitch\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ List of bit rates to use during stepwise downswitch\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Minimum waiting time before upswitch is attempted\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Waiting time at each step during upswitch after session
start\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Minimum interval between detection of ECN-CE and
upswitch\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Waiting time at each step during upswitch after
congestion\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Subnodes related to initial codec mode\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Initial bit rate speech encoder should begin encoding speech
at\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Waiting time the sender should use before an up-switch is
attempted in the initial phase of session if no rate control information is
received\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Waiting time at each step during up-switch in the beginning of
session\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Probation period during which transition is prohibited after
ping-pong effects\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Measurement period multiplied by PLR/DURATION to substitute
undefined DURATIONs\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Maximum response time tolerated before the receiver considers
repairing actions\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Maximum packet loss rate tolerated before the receiver asks the
sender to reduce bit rate\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Minimum packet loss rate tolerated before the receiver asks the
sender to increase bit rate\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Subnodes related to minimum video quaity\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Subnodes related to minimum bit rate\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Minimum bit rate tolerated (kps)\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Minimum bit rate tolerated (proportion of the bit rate
negotiated)\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Subnodes related to minimum frame rate\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Minimum frame rate tolerated (fps)\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Minimum frame rate tolerated (proportion of maximum frame rate
specified by codec profile/level negotiated)\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Subnodes related to maximum luminance quantization
parameter\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Maximum luminance quantization parameter QUANT\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Maximum luminance quantization parameter
quantiser_scale\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Maximum luminance quantization parameter QPY\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Proportion of current estimated encoding rate video receiver
requests the sender to reduce by\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Increase in requested maximum encoding rate over current
rate\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Subnodes related to minimum bit rate to use during ECN-based
adaptation\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Minimum frame rate tolerated (fps) during ECN-based
adaptation\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ Minimum frame rate tolerated (proportion of maximum frame rate
specified by codec profile/level negotiated) during ECN-based
adaptation\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Maximum tolerated interval between packet arrivals\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Minimum interval between TMMBR messages increasing maximum rate
limit\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Minimum interval between TMMBR messages decreasing maximum rate
limit\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Duration of sliding window for
TARGET_PLAYOUT_MARGIN_HI\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Upper threshold of interval between video packet arrival and its
properly scheduled playout\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Lower threshold of interval between video packet arrival and its
properly scheduled playout\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Rate at which the sender should increase target bit rate to a
higher max rate limit\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Rate at which the sender should decrease target bit rate to a
lower max rate limit\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Time the receiver should command the sender to spend in
decongesting\**
**\ **
**\ text/plain\**
**\ **
...
**\ **
**\ **
**\ Tri-valued parameter to control how the sender should behave in
case video quality cannot meet requirements\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ Initial bit rate ((proportion of the bit rate negotiated) video
codec should begin encoding at\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
**\ **
**\ **
**\ X percentile point of packet arrival distribution used with
TARGET_PLAYOUT_MARGIN\**
**\ **
**\ text/plain\**
**\ **
**\ **
...
size (bytes) 11.88 12.88 14.75 16.75 18.5 19.88 25.5 30.5 10 Rounded-up speech
...
57.63 59.625 10 Rounded-up speech frame size (bytes) 17 23 32 36 40 46 50 58
60 5 Rounded-up speech frame size (bits) 136 184 256 288 320 368 400 464 480
40 Payload header and ToC 16 16 16 16 16 16 16 16 16 16 RTP payload (bits) 152
...
64 64 64 64 IPv4 header 160 160 160 160 160 160 160 160 Total bits per 20 ms
...
Payload header and ToC 24 24 24 24 24 24 24 24 RTP payload (bits) 216 232 264
296 328 344 440 520 RTP header 96 96 96 96 96 96 96 96 UDP header 64 64 64 64
64 64 64 64 IPv6 header 320 320 320 320 320 320 320 320 Total bits per 20 ms
...
header 96 96 96 96 96 96 96 96 96 UDP header 64 64 64 64 64 64 64 64 64 IPv4
header 160 160 160 160 160 160 160 160 160 Total bits per 20 ms 616 712 856
...
24 24 24 24 24 RTP payload (bits) 296 392 536 600 664 760 824 952 984 RTP
header 96 96 96 96 96 96 96 96 96 UDP header 64 64 64 64 64 64 64 64 64 IPv6
header 320 320 320 320 320 320 320 320 320 Total bits per 20 ms 776 872 1016
