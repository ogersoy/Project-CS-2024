...
example to reduce latency for certain Representations.
The MPD shall include URL information for all Segments with an availability
start time less than both the end of the presentation and the sum of the
latest time at which this version of the MPD is available on the server and
the **MPD** \@minimumUpdatePeriod.
...
> NOTE if the scheme or the value for this descriptor is not recognized the
> DASH client is expected to ignore the descriptor.
...
> if the **MPD** \@minimumUpdatePeriod attribute is present, then _PeriodEnd_
> is defined as the smaller value of _FetchTime +_ **MPD**
> \@minimumUpdatePeriod and **MPD** \@availabilityStartTime + **MPD**
...
associated.
Table C.1 --- Role\@value attríbute for scheme with a value
...
int(8) level_count;\ **for (j=1; j \  Security considerations:
>
...
> MPD. As such, any security considerations for an MPD may also be applicable
> to an MPD Delta. A MIME type handler would not launch a service with only an
> MPD Delta.
>
