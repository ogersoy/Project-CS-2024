...
[101] 3GPP TS 36.443: "Evolved Universal Terrestrial Radio Access Network
(E-UTRAN); M2 Application Protocol (M2AP)".
[102] 3GPP TS 36.101: \"Evolved Universal Terrestrial Radio Access (E-UTRA);
User Equipment (UE) radio transmission and reception\".
[103] 3GPP TS 36.304: \"User Equipment (UE) procedures in idle mode\".
[104] IETF RFC6681, \"Raptor FEC Schemes for FECFRAME,\" M.Watson, T.
Stockhammer and M. Luby, August 2012.
[105] IETF RFC6363, \"Forward Error Correction (FEC) Framework,\" M.Watson, A.
Begen and V. Roca, October 2011.
[106] Marine, A., Reynolds, J., and G. Malkin, \"FYI on Questions and Answers
...
address FQDN [106]. The network may distribute the SNTP request traffic load
...
Metadata management information consists of:
  * a metadata envelope object(s) allowing the identification, > versioning, update and temporal validity of a metadata fragment > objects (defined in sub-clause 11.1).
...
In the event a MBMS User Service carries DASH-formatted contents, the MBMS
User Service Description fragment shall contain a
_mediaPresentationDescription_ element. The _mediaPresentationDescription_
element shall in turn contain a reference to the MPD (Media Presentation
Description) as defined in [98], as a metadata fragment. Furthermore, the MPD
may refer to one or more Initialisation Segments as defined in [98], as the
Initialisation Segment Description (ISD) metadata fragment. Also, when DASH-
...
description instance. The UE can expect to receive MBMS data during the time
periods described in the schedule description instance. In the case of a file
download service, the schedule description instance may also include a file
transmission schedule for file objects associated with the User Service. The
UE may select which files to receive based on the file transmission schedules
in the schedule description instance.
It will also be possible for the schedule instance in multiple User Service
Description instances to reference the same schedule description instance. In
this case, the schedule description instance shall capture the file
transmission schedule for files belonging to each of these User Services.
...
MPD delivered via the USD.
  5. If any other resource in the MPD is delivered (e.g. xlinked resource, metrics, etc.) then
```{=html}
``` a. The Content-Location element in the FDT for the delivered object shall
...
are relevant to 3GP-DASH over MBMS.
...
perform file repair if enabled in the system.
...
The below XML Schema shall be used for the FDT Instance.
...
#### 8.2.2.0 General
The "MBMS FEC scheme" is the fully-specified FEC scheme defined in [104],
section 6 with ID 1.
...
Application Data Unit (ADU) as defined in RFC6363 [105]. The source data flow
...
UDP flow. The UDP payload constitutes and ADU according to RFC6363 [105] and
...
constructs a source block as specified in RFC6681 [104], section 5 to which
...
packet's flow is marked using the Flow ID. See RFC6681 [104], section 5 for
details.
...
in [104], section 5. The UDP port the packet was received on is used to
...
The packet format for FEC source packets as defined in RFC6363 [105], section
...
specifications.
The Source FEC Payload ID shall be constructed according to RFC 6681 [104],
section 6.2.2.
...
The packet format for FEC repair packets as defined in RFC6363 [105], section
...
This section provides an example how to use the methods in RFC6363 [105] and
RFC6681 [104] to generate a source block.
...
FEC Scheme Specific Information in section 6.2.1.2 of RFC6681 [104].
NOTE: This corresponds to Payload ID Format A in RFC6681 [104] as the last
...
[104], section 6.2.3. The FEC source block resides in the hypothetical FEC
decoding buffer.
...
Acnowledgement). The same as StaR-all with the exception that individual files
are not acknowledged. Only reception details are reported for the session for
both streaming and download delivery. StaR-only is equivalent to StaR-all for
streaming delivery. StaR-all is relevant to download delivery where session
...
sends one or more Reception Report messages to thereception report server URI.
All Reception Report requests and responses for a particular MBMS transmission
should take place in a single TCP session using the HTTP protocol (RFC 2616
[18]).
...
the time when the session was started and stopped, respectively. > The values
of each attribute corresponds to the 32 most > significant bits of a 64 bit
...
_serviceArea_ and shall contain one or more _radiofrequency_ elements. The
...
Area Identity_ as defined in 3GPP TS 36.443 [101] and 3GPP TS 23.003 [77].
According to 3GPP TS 36.443 [101], _MBMS Service Area Identity_ is frequency
agnostic and can be mapped onto one or more cells. The specific usage of the
_MBMS Service Area Identity_ , or its correlation to other network
identification information, is not defined in this specification. The
...
coded as EARFCN in 3GPP TS 36.101 [102]. The MBMS client shall forward the
...
element _scheduleeDescriptionURI_ in the _schedule_ element.
The start and stop time of a single fileSchedule is specified by the _start_
...
following this schema can be identifed with the MIME type
...
3GP-DASH as defined in clause 5.6 (of Release 10) 18
* * *
