...
NOTE: The procedures in clause 11 are related to scenarios in which the
collaborative session is set up by the controller UE when a end-to-end session
already has been established with the remote UE.
...
\- procedures for session discover in clause 15;
...
NOTE: The procedures in clause 17 are related to scenarios in which the
...
remote UE are specified in clause 19;
\- procedures for collaborative session handling upon loss of collaborative
session are specified in clause 22; and
...
24.229 [7]. The SCC AS shall construct the SDP information in the SIP re-
INVITE request as follows:
...
the controller UE as described in IETF RFC 3515 [32];
...
The controller UE shall handle any response to the SIP REFER request and the
subsequent SIP NOTIFY requests according to 3GPP TS 24.229 [7] and IETF RFC
3515 [32]. The controller UE shall examine the Contact header field included
in the sipfrag of the SIP 200 OK response in the SIP NOTIFY of the refer
events package and if that Contact header field contains the g.3gpp.current-
iut-controller media feature tag set to \"active\" then control has been
successfully transferred and the controller UE shall perform the role of a
controllee UE in the collaborative session, otherwise the controller UE shall
...
specified in IETF RFC 3515 [5] and include:
NOTE 1: Before requesting media flow transfer, the target UE of media transfer
...
dialog as specified in IETF RFC 3515 [5] and include:
NOTE 1: Before requesting media flow transfer, the target UE of media transfer
discovers the ongoing session with transferred media and gets information
about its media flow by subscribing to dialog event.
1) the Request-URI set to the SIP URI of the local UE that currently holds the
...
and
\- the SIP 202 (Accepted) response and SIP and SIP NOTIFY request to the
target UE instead of the controller UE.
...
\- a SIP INVITE request to the controllee UE to which the media component(s)
is to be transferred (i.e. the target UE), instead of a SIP re-INVITE request;
\- the SIP 202 (Accepted) response and SIP NOTIFY request to the target UE
...
the SIP 202 (Accepted) response and SIP NOTIFY request are sent to the target
UE instead of the local UE.
...
The SCC AS performs the steps as described in subclause 12.3.1, except that
the SIP 202 (Accepted) response and SIP NOTIFY request are sent the target UE
...
the SIP 202 (Accepted) response and SIP NOTIFY request are sent tothe UE that
sent the SIP REFER request instead of the controller UE.
...
a SIP REFER request as specified in IETF RFC 3515 [32] and include:
> 1) the Request-URI set to the inter UE Transfer SCC AS URI;
>
> 2) the Accept-Contact header field containing the g.3gpp.iut-focus media
> feature tag with explicit and require tags;
...
3515 [32] and 3GPP TS 24.229 [7]. The SC UE shall populate the SIP REFER
request as follows:
...
24.229 [7] and IETF RFC 3515 [32]. The UE shall populate the SIP MESSAGE
request with
...
shall send a SIP REFER request as specified in IETF RFC 3515 [32] and 3GPP TS
24.229 [7]. The SC UE shall populate the SIP REFER request as follows:
1\. the Request-URI set to the URI of the UE where the session is to be
replicated;
...
sent in the subscription) as specified in IETF RFC 3515 [5], so that the SIP
...
control. A user has a multimedia session on his device UE‑1 with voice (Media
...
Table A.7.2.1-1: SIP re-INVITE request (UE-1 to intermediate IM CN subsystem
entities)
> INVITE
> \
> SIP/2.0
...
It is assumed that UE-1 is the controller UE having collaborative session
control. A user has a multimedia session on his device UE‑1 with a voice
...
a user has a multimedia session on his device UE‑1 with voice (Media A) and
...
The SCC AS-1 serving UE-1 sends a SIP 202 (Accepted) response to UE-2 as
response to the SIP REFER request.
...
itself.
Table 4.11.2.2-1: SIP re-INVITE request (UE-2 to Intermediate IM CN subsystem
entities)
