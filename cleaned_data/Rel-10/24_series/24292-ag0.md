...
[47] IETF RFC 5626 (October 2009): \"Managing Client-Initiated Connections in
...
Vector header field, store the value of the received term-IOI parameter.
NOTE 2: Any received \"term-ioi\" header field parameter will contain a type 1
...
subclause 4.5 of IETF RFC 5626 [47] for new registration attempts. The MSC
...
of IETF RFC 5626 [47]. If no values of the parameters \"max-time\" and \"base-
time (if all failed)\" have been configured by operator policy, the default
values defined in in subclause 4.5 of IETF RFC 5626 [47] shall be used.
...
\-- an a=cs-correlation attribute set to \"callerid\" along with the MSISDN,
...
specified in 3GPP TS 24.229 [11]. In the same, the SDP payload may also be
proposing a CS video stream;
...
a SETUP message in accordance with 3GPP TS 24.008 [7] for 3GPP systems. The UE
...
containinga valid SCC AS PSI DN encoded as a tel URI (see IETF RFC 3966 [42])
...
Route header field of the outgoing initial SIP INVITE request; and
...
Upon receiving a SIP 200 (OK) response from the remote UE, the SCC AS shall
send the SIP 200 (OK) response towards the CS domain. If the SIP 200 (OK)
...
When the SCC AS has received a SIP ACK request on both the leg with the CS and
on the leg with the ICS UE, the SCC AS shall send a SIP ACK request to the
remote UE.
...
\- a CS media \"m=\" line\'s with the media portion set to \"audio\", port
...
NOTE 4: The SCC AS includes a Target-Dialog header field in the SIP INVITE
...
feature tag containing the value associated at registration with the selected
PS domain IP-CAN;
NOTE 5: The SCC AS can determine which g.3gpp.accesstype media feature tag
values to use by taking into account the access-type and access-class of the
...
NOTE 6: It is possible that a handover between different IP-CANs can take
place without a reregistration of the UE and corresponding update of access-
type and access-class (e.g. from \"3GPP-UTRAN\" to \"3GPP-E-UTRAN\"). The SCC
AS needs to take this possibility into account when determining the IP-CAN to
use.
NOTE 7: If the SCC AS wants to preclude the request to be sent to the UE by
the S-CSCF (through sequential forking) over a different IP-CAN than the one
it explicitly selected, it needs to include the parameters \"require\" and
\"explicit\" along with the g.3gpp.accesstype media feature tag. If it intends
to allow the S-CSCF to fork between different IP-CANs, it can do that by
...
a Target-Dialog header field containing the dialog parameters for that
existing dialog between the SCC AS and the UE; and
NOTE 8: The SCC AS includes a Target-Dialog header field in the SIP INVITE
...
IETF RFC 7195 [36] and may also include a CS video media stream according to
the initial SIP INVITE received by the SCC AS;
...
received from the ICS UE then, the SCC AS shall:
...
attribute contained in the SDP answer from the response to the SIP INVITE
request received from the ICS UE to with the SIP INVITE request from the
remote UE; and
...
a valid SCC AS PSI DN encoded as a tel URI (see IETF RFC 3966 [42]) or SIP URI
with the \"user\" SIP URI parameter set to \"phone\", in accordance with 3GPP
...
ecf=[5555::6aa:7bb:8cc:9dd]
P-Charging-Vector: icid-value=\"AyretyU0dm+6O2IrT5tAFrbHLso=023551024\"; orig-
ioi=\"type 3home1.net\"; orig-ioi=\"home1.net\"
P-Access-Network-Info:
Privacy: none
From: \;tag=171828
To: \
...
> The ISC UE generates an I1 Progress message based upon the received I1
> INVITE request.
Specifically for this signalling flow, the I1 Progress message includes:
\- Protocol Information = 0x11
\- Message Type = I1 Progress
