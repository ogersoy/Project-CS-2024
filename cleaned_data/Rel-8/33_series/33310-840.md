...
Version 1.7\".
[3] IETF RFC 3280: \"Internet X.509 Public Key Infrastructure Certificate and
CRL Profile\".
...
[13] IETF RFC 3280 (2002): \"Internet X.509 Public Key Infrastructure
...
Interconnection CA~A~. The path is verifiable in domain B, because the path
...
### 5.2.2 Establishment of secure inter-domain communications
#### 5.2.2.1 NDS/IP case
...
\- SEG A verifies the SEG B certificate using the cross-certificate for
Operator B\'s SEG CA by executing the following actions:
...
\- NE A receives the remote NE B (or SEG B) certificate and signature;
\- NE A verifies the remote NE B (or SEG B) signature;
...
be applied in addition to those contained within RFC3280 [3]. This applies for
the SEG, NE,the TLS entity, the SEG CA and the Interconnection CA.
...
\- Hash algorithm for use before signing certificate: Sha-1 mandatory to
support, MD-5 shall not be used.
...
dc=\, dc=\.
\- CRLv2 support with LDAPv3 [5] retrieval shall be supported as the primary
method of certificate revocation verification. HTTP shall also be allowed for
checking the revocation status of TLS and NE certificates.
\- Certificate extensions which are not mandated by this specification but
which are mentioned within RFC3280 [3] are optional for implementation. If
...
Editor\'s note: It is intended to align the SEG certificate profile with
draft-ietf-pki4ipsec-ikecert-profile-10 if/when it becomes an RFC. This draft
was moved to IETF last call in June 2006.
...
certificate;
\- if used for connections with elements having the the same root CA
certificate installed as used in the domain the SEG CA is located in, the
...
located in, the issuer name is the subject name of either this root CA or an
intermediate CA whose certificate has a valid certificate chain up to this
root CA;
\- Extensions:
\- Optionally non critical authority key identifier;
\- Optionally non critical subject key identifier;
...
cross-certificate from the (local) Interconnection CAa CR;
...
9\. SEGa verifies the status of the Interconnection CAa certificate if the
Interconnection CAa is not a top-level CA, otherwise Interconnection CAa is
implicitly trusted;
...
Extensions
According to RFC3280 [3], section 4.2 a certificate extension can be
...
marked as critical shall lead to an error according to RFC 3280.
...
TLS end points shall support the CipherSuite TLS_RSA_ WITH_3DES_EDE_CBC_SHA
and the CipherSuite TLS_RSA_WITH_AES_128_CBC_SHA, except those TLS endpoints
that have been defined for GBA in TS 33.220 [10] and for IMS in TS 33.203 [9].
Both TS 33.203 [9] and TS 33.220 [10] contain their own TLS CipherSuite
requirements.
...
Revocation Lists (CRLs) [13] published by the issuer of Cert~A~.
#