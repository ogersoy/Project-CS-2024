...
seconds of a 10 second QCT timer have passed when a CCR(U) is triggered, and
the CCA(U) returns 2 seconds later, then the QCT timer will expire 3 seconds
after the receipt of the CCA and the remaining unaccounted 5 seconds of usage
will be recorded against the new quota even though no packets were transmitted
with the new quota.
In the case of a new quota with the Quota-Consumption-Time AVP, or when
packets are blocked during the CCR(U)/CCA procedure then the Quota-
...
after the table and for the others can be found from IETF RFC 3588 [401], IETF
RFC 4006 [402] and and IETF RFC 4005 [407].
...
CC-Unit-Type | 454 | - | - | - | OC | Enumerated | M | P | - | V | Y  
...
NOTE: _Result-Code_ AVP is defined in Diameter Base Protocol [401]. However,
new values are used in offline and online charging applications. These
additional values are defined below.
...
as specified in IETF RFC 4006 [402]. It contains additional 3GPP specific
charging parameters.
...
The _Rating-Group_ AVP (AVP code 432), is defined in IETF RFC 4006 [402]. It
contains the charging key (defined in 3GPP TS 23.203 [218]). Each quota
...
document. The Result-Code AVP operates as described in RFC 3588 [401] and IETF
RFC 4006 [402].
...
The _Service-Context-Id_ AVP is defined in IETF RFC 4006 [402]. It is of type
...
The _Service-Identifier_ AVP (AVP code 439), is defined in IETF RFC 4006
[402]. For further details, please refer the middle-tier specification.
### 7.1.9 Used-Service-Unit AVP
The _Used-Service-Unit_ AVP (AVP code 446) is of type grouped as specified in
IETF RFC 4006 [402]. It contains additional 3GPP specific charging parameters.
It has the following ABNF grammar:
\ ::= \
> [ Reporting-Reason ]
>
...
IANA registered value for \'3GPP\' in 3GPP TS 29.230 [206].
### 7.1.12 User-Equipment-Info AVP
...
as described in IETF RFC 4005 [407] with the exception that the 'M' flag shall
...
Application-Server-ID | 2101 | X | - | X | - | refer[222] |  |  |  |  |   
Application-Server-Information | 850 | X | - | X | - | Grouped | V,M | P |  |  | N  
Application-Service-Type | 2102 | X | - | X | - | refer[222] |  |  |  |  |   
Application-Session-ID | 2103 | X | - | X | - | refer[222] |  |  |  |  |   
**Applic-ID** | 1218 | - | - | X | - | UTF8String | V,M | P |  |  | N  
**Associated-Party-Address** | 2035 | X | - | - | - | UTF8String | V,M | P |  |  | N  
...
Content-ID | 2116 | X | - | X | - | refer[222] |  |  |  |  |   
Content-Provider-ID | 2117 | X | - | X | - | refer[222] |  |  |  |  |   
...
IM-Information | 2110 | X | - | X | - | refer[222] |  |  |  |  |   
...
MMTel-SService-Type | 2031 | X | - | - | - | Unsigned32 | V,M | P |  |  | N  
MMTel-Information | 2030 | X | - | - | - | Grouped | V,M | P |  |  | N  
MSISDN | 701 | - | - | X | - | refer [221] |  |  |  |  |   
Next-Tariff | 2057 |  |  | X | X | Grouped | V/M | P |  |  | N  
Node-Functionality | 862 | X | -. | X | - | Enumerated | V,M | P |  |  | N  
Node-Id | 2064 | X | - | X | - | UTF8String | V,M | P |  |  | N  
Number-Of-Diversions | 2034 | X | - | - | - | Unsigned32 | V,M | P |  |  | N  
Number-Of-Messages-Sent | 2019 | X | - | X | - | Unsigned32 | V,M | P |  |  | N  
Number-Of-Messages-Successfully-Exploded | 2111 | X | - | X | - | refer[222] |  |  |  |  |   
Number-Of-Messages-Successfully-Sent | 2112 | X | - | X | - | refer[222] |  |  |  |  |   
...
Participant-Action-Type | 2049 | X | - | - | - | Enumerated | V,M | P |  |  | N  
...
Service-Generic-Information | 1256 | X | - | X | - | Refer[222] |  |  |  |  |   
Service-Id | 855 | X | - | X | - | UTF8String | V,M | P |  |  | N  
Service-Information | 873 | X | - | X | X | Grouped | V,M | P |  |  | N  
Service-Mode | 2032 | X | - | - | - | Unsigned32 | V,M | P |  |  | N  
...
Subscriber-Role | 2033 | X | - | - | - | Enumerated | V,M | P |  |  | N  
Supplementary-Service | 2048 | X | - | - | - | Grouped | V,M | P |  |  | N  
...
Tariff-Information | 2060 | - | - | X | X | Grouped | V,M | P |  |  | N  
...
Total-Number-Of-Messages-Exploded | 2113 | X | - | X | - | refer[222] |  |  |  |  |   
Total-Number-Of-Messages-Sent | 2114 | X | - | X | - | refer[222] |  |  |  |  |   
...
The _Additional-Content-Information_ AVP (AVPcode 1207) is of type Grouped and
identifies any subsequent content types. It is used to identify each content
(including re-occurrences) within an MM when the Type-Number AVP or
...
\"forwarding party\" for CDIV, the \"transferor\" for ECT.
...
the address (SIP URI or TEL URI) of the party (Public User ID or Public
Service ID) to whom the SIP transaction is posted. The Called Party Address
shall be populated with the SIP URI or TEL URI contained in the Request-URI of
the outgoing request.
For a registration procedure, this field holds the party (Public User ID) to
be registered. In this case, the Called Party Address field is obtained from
the \"To\" SIP header of the SIP Request. For a subscription procedure this
field holds the address of the resource for which the originator wants to
receive notifications of change of states. In this case, the Called Party
Address field is obtained from the outgoing Request-URI of the SIP Request.
...
cause code value from IMS node. It is used in Accounting-request[stop] and/or
Accounting-request[event] messages. It is also used in the Credit-Control-
request [Terminate] and/or Credit-Control-request [Event] messages.
...
> due to an IMS node receiving/initiating a 3xx response [405].
>
...
> terminated due to an IMS node receiving/initiating a 4xx error response
> [405].
>
> \"5xx Server failure\" 5xx
>
> The cause \"5xx Server failure\" is used when the SIP transaction is
> terminated due to an IMS node receiving/initiating a 5xx error response
> [405].
>
> \"6xx Global failure\" 6xx
>
> The cause \"6xx Global failure\" is used when the SIP transaction is
> terminated due to an IMS node receiving/initiating a 6xx error response
> [405].
>
...
> after reception of the 200OK final response and SIP ACK is not received)
> [202] [405].
>
> \"Internal error\" 3
>
...
message to CDF
### 7.2.25C Charge-Reason-Code AVP
...
services.
The AVP is defined in OMA-DDS-Charging_Data [222].
...
> [ LCS-Client-ID ]
>
> [ Location-Type ]\ [ Location-Estimate ]
>
> [ Positioning-Data ]
>
> [ 3GPP-IMSI ]
...
The _Location-Estimate_ AVP (AVP code 1242) is of type UTF8String and contains
an estimate of the location of an MS in universal coordinates and the accuracy
of the estimate.
...
The _MBMS-User-Service-Type_ AVP (AVP code 1225) is of type Enumerated and
indicates type of service the MBMS user service that is being delivered. The
...
>
> [ Subscriber-Role ]
### 7.2.88AA MMTel-SService-Type AVP
...
S-CSCF during number portability look-up (ENUM/DNS). This information is sent
over SIP in the Requested URI header.
...
populating this parameter [404] in a SIP request [202].
...
``` \- Type 3 IOI:
    -   IOI of the home network (originating side or terminating side)
        ...
        \[202\] to an AS (proxy, terminating UA or redirect server or
        B2BUA).
...
For further details on the Type 1, Type 2 and Type 3 IOIs, please refer to
3GPP TS 32.240 [1].
...
>
> [ Address-Domain ]
### 7.2.97 Originator-Interface AVP
...
>
> [ Interface-Type ]
### 7.2.97A Originator-Received-Address AVP
...
AddressType disciminator [401] is set to value 8, E.164, and the address
...
> \ : : = \
>
> [ Called-Party-Address ]\ [ Participant-Access-Priority ]\ [ User-
> Participating-Type ]
...
Coding of this AVP is same as 3GPP-Charging-Id coding described in 3GPP TS
29.061 [207].
...
The PS-Append-Free-Format-Data AVP (AVP code 867) is of type enumerated and
...
The PS-Furnish-Charging-Information AVP (AVP code 865) is of type Grouped. Its
purpose is to add online charging session specific information, received via
the Ro reference point, onto the Rf reference point in order to facilitate its
inclusion in CDRs. This information element may be received in a CCA message
via the Ro reference point. In situations where online and offline charging
are active in parallel, the information element is transparently copied into
...
> [ 3GPP-GGSN-MCC-MNC ]\ [ 3GPP-NSAPI ]\ [ Called-Station-Id ]\ [ 3GPP-
> Session-Stop-Indicator ]\ [ 3GPP-Selection-Mode ]\ [ 3GPP-Charging-
> Characteristics ]\ [ Charging-Characteristics-Selection-Mode ]\ [ 3GPP-SGSN-
> MCC-MNC ]\ [ 3GPP-MS-TimeZone ]
>
...
AddressType discriminator [401] is set to value 8, E.164, and the address
information is UTF8 encoded.
...
The _Remaining-Balance_ AVP (AVPcode 2021) is of type Grouped and provides
...
IMS it holds the address (SIP URI or TEL URI) of the party (Public User ID or
Public Service ID) to whom the SIP transaction was originally posted.\ The
Requested Party Address shall be populated with the SIP URI or TEL URI
...
content of the SDP \"m=\" line in a media description, as described in IETF
RFC 4566 [406].
...
service the MRFC is hosting or the AoC service. For conferences the conference
ID is used as the value of this parameter.
...
The _Service-Mode_ AVP (AVP Code 2032) is of type Unsigned32and provides the
...
The _SM-Protocol-ID_ AVP (AVP code 2013) is of type Octet String and holds an
...
### 7.2.161 SM-User-Data-Header AVP
...
Encoding is as described in 3GPP TS 23.040 [216]. Any padding bits are not
...
2 VAS4SMS Short Message Forwarding multiple subscriptions (as defined in 3GPP
TS 22.142 [217]
...
6 VAS4SMS Short Message to multiple destinations (as defined in 3GPP TS 22.142
[217]
7 VAS4SMS Short Message Virtual Private Network (VPN) (as defined in 3GPP TS
22.142 [217]
...
network as generated by the IMS network element which takes responsibility for
populating this parameter [404] in a SIP response [202].
...
The _Time-Stamps_ AVP (AVP code 833) is of type Grouped and holds the time of
...
PLMN id (MCC and MNC).
Coding of this AVP is same as 3GPP-SGSN-MCC-MNC coding described in 3GPP TS
29.061 [207].
...
The _WLAN-Technology_ AVP (AVP code 893) is of type Unsigned32. Actual content
of this AVP is tbd.
