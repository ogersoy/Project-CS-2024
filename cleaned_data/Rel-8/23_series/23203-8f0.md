...
Core Network Aspects\" September 2010.
...
data flow filter to a new or an existing PCC rule. If the PCRF received an SDF
filter identifier together with this traffic mapping information, the PCRF
shall modify the existing PCC rule if the PCC rule is authorized for a GBR
QCI.
...
can be a pre-defined PCC rule in the PCEF. In case the PCRF provides PCC rules
dynamically, authorised QoS information for the IP‑CAN bearer (combined QoS)
may be provided. For a predefined PCC rules within the PCEF the authorized QoS
information shall take affect when the PCC rule is activated. The PCEF shall
combine the different sets of authorized QoS information, i.e. the information
received from the PCRF and the information corresponding to the predefined PCC
rules. The PCRF shall know the authorized QoS information of the predefined
PCC rules and shall take this information into account when activating them.
This ensures that the combined authorized QoS of a set of PCC rules that are
activated by the PCRF is within the limitations given by the subscription and
operator policies regardless of whether these PCC rules are dynamically
provided, predefined or both.
...
During QoS rules enforcement, if packet filters are provided to the UE, the
network shall provide packet filters with the same content as that in the SDF
template filters received over the Gx/Gxx interface. The representation/format
of the packet filters provided by the network to the UE is access-system
dependent and may vary between accesses and also may be different from the
that of the SDF template filter on the Gx/Gxx interface.
After handover from one access-system to another, if the UE needs to determine
the QoS provided in the target access to the pre-existing IP flows in the
source access, the UE may do IP filter matching between IP flow filter and
those provided by the target access during QoS resource activation.
NOTE: If UE initiated procedures are supported and handover between access
...
conditionally the UE Identity and PDN ID towards open Gateway Control
Sessions.
...
filters are provided to the UE, the BBERF shall provide packet filters with
the same content as that in the SDF template filters received over the Gx/Gxx
interface.
...
also includes provisioning the UE with traffic mapping information. See Annex
A and Annex D for details.
NOTE 3: In this Release the PCEF uses all SDF filters for the generation of
traffic mapping information. The default bearer is established without any
explicit traffic mapping information, and thus transfers any traffic that is
not directed to any other IP-CAN bearer. If the PCEF receives PCC rules for
the default bearer with SDF filters restricting the traffic, the PCEF is
required to add traffic mapping information to the default bearer
subsequently.
...
filters;
\- Service data flow filters are unidirectional, so that the detection is
applied independently for the downlink and uplink directions.
NOTE 1: A service data flow template may include service data flow filters for
...
NOTE 2: The details about the IPSec Security Parameter Index (SPI), the Type
...
NOTE 3: Such filters may be used to support filtering with respect to a
...
NOTE 4: For the uplink direction, discarding packets due to no matching
...
NOTE 5: To avoid the PCEF discarding packets due to no matching service data
...
A and Annex D).
NOTE: In addition, the GW may maintain IP‑CAN bearer level measurement if
...
NOTE 3: The H-PCRF may inform the AF of that event reporting is restricted due
to limited PCC deployment. For detail see Annex L.
...
control functions. It is a functional entity within the Online Charging
System.
...
NOTE 1: The definition of the mapping between QCI and Non 3GPP access specific
...
During IP‑CAN bearer QoS enforcement, if packet filters are provided to the
UE, the BBERF shall provide packet filters with the same content as that in
the SDF template received over the Gx/Gxx interface.
NOTE 2: In this Release the BBERF uses all SDF filters for the generation of
traffic mapping information. The default bearer is established without any
explicit traffic mapping information, and thus transfers any traffic that is
not directed to any other IP-CAN bearer. If the BBERF receives QoS rules for
the default bearer with SDF filters restricting the traffic, the BBERF is
required to add traffic mapping information to the default bearer
subsequently.
...
PCEF, the charging information supersedes the Primary OFCS/OCS address and
Secondary OFCS/OCS address in the charging characteristics profile.
...
the PCEF, charging method supersedes the Default charging method in the
...
identifier (e.g. APN), the IP‑CAN type and the IP address(es) and, if
...
to the PCRF and, if changed, the new IP‑CAN bearer establishment modes
supported. If there is a limitation or termination of the transmission
resources for a PCC Rule, the GW(PCEF) reports this to the PCRF.
...
NOTE: Multiple PDN connections to the same APN are not supported for the case
2b in clause 7.1 in this release of the specification.
...
NOTE: How subsequent interactions work is described in TS 29.213 [21].
...
and, if available, the IP‑CAN bearer establishment modes supported. The IP‑CAN
Type identifies the type of access used by the UE. The UE\'s identity and PDN
Identifier requested are used to identify the subscriber and in PCRF selection
to locate the PCRF function with the corresponding IP‑CAN session established
by the PDN GW. The BBERF may also include the Default Bearer QoS and APN-AMBR
(applicable for case 2b, as defined in clause 7.1). An indication that leg
linking shall be deferred is included to inform the PCRF that linking the
Gateway Control Session to a Gx session shall occur when a matching Gx message
is received as described clause 6.2.1.0. Further information is supplied on an
access specific basis, as described in the IP‑CAN specific Annexes.
...
bearer establishment mode, QoS Rules and Event Triggers. In case 2a, a
charging ID may be provided together with QoS rules. The QoS policy rules are
employed by the GW(BBERF) to perform Bearer Binding. The Event Triggers
indicate events that require the GW(BBERF) to report to the PCRF.
...
Identity, PDN Identifier (if known), IP address(es) (if known) and, if
available, the IP‑CAN bearer establishment modes supported. The IP‑CAN Type
identifies the type of access used by the UE. The UE\'s identity and PDN
Identifier requested are used to identify the subscriber and in PCRF selection
to locate the PCRF function with the corresponding IP‑CAN session established
by the PDN GW. The BBERF may also include the Default Bearer QoS and APN-AMBR
(applicable for case 2b, as defined in clause 7.1). If the handover state is
unknown to the GW(BBERF), as described in TS 23.402 [18], the GW(BBERF)
includes an indication to inform the PCRF that linking the Gateway Control
Session to a Gx session shall be deferred as described clause 6.2.1.0
(applicable to case 2b, as defined in clause 7.1). Further information is
supplied on an access specific basis, as described in the IP‑CAN specific
Annexes.
...
\- to a candidate PDP context with a matching QoS class that, after
modification of the bitrates, fulfils the PCC rule QoS demands;
\- to a new PDP context with a matching QoS class, if there is no suitable
candidate PDP context present.
...
When the PCEF performs the bearer binding the ARP information in the PCC rule
shall be ignored.
...
communicated as the radio access type, has changed. Location change (routeing
area) The routeing area of the UE has changed. Location change (CGI/SAI) The
CGI/SAI of the UE has changed.
* * *
...
the trigger according to the procedures described in TS 23.060 [12]. If
credit-authorization triggers and event triggers require different levels of
reporting of location change for different PDP contexts for a single UE, the
SGSN reports location changes to the highest level of detail required.
However, the GGSN should not trigger a credit re-authorization if the report
received is more detailed than requested by the OCS.
...
change (CGI/SAI) The CGI/SAI of the UE has changed. NOTE 1: Available only
when the bearer binding mechanism is allocated to the PCRF.
* * *
...
NOTE: The operator\'s configuration should reserve QCI values that map to
\"speech\" for service data flows consisting of speech (and the associated
RTCP) only.
* * *
NOTE 1: This table defines the mapping for GPRS QCI to/from UMTS QoS
...
GGSN.
NOTE 2: Any change of the ARP parameter by the PCEF may get overwritten by the
SGSN due to subscription enforcement.
...
PDP context is deleted, is sent to the SGSN.
...
QoS negotiated or the required CGI/SAI/RAI change reporting. The Update PDP
Context Response message, indicating that a PDP context is modified, will be
...
In the 3GPP Access (GTP-based) architecture, see TS 23.401 [17] and TS 23.060
[12],
...
NOTE: For GERAN/UTRAN in UE-only mode the detection of the uplink part of the
service data flow may be active, in parallel, on any number of EPS bearers.
...
NOTE: This enables a modification of the EPS bearer ARP without impacting the
service assignment after a handover to E-UTRAN.
...
APN‑AMBR to/from pre‑Rel‑8 QoS parameter values of PDP‑contexts as specified
in Annex E of TS 23.401 [17]. The PDN GW shall mediate Gn/Gp procedures so
that the PCRF experiences no difference compared to S5/S8 procedures.
Only the GBR per bearer is used for resource reservation (e.g. admission
control in the RAN).
...
\- Default EPS Bearer QoS is provided by the BBERF.
...
in clause 7.7.1): User Location Information, Serving-GW address, Serving
Network, RAT Type, Default EPS Bearer QoS and if available the APN-AMBR are
provided to the PCRF.
...
7.7.3) the BBERF includes the modification of any the 3GPP specific
information listed in clause A.5.4.2.
...
clause 7.2): Authorized APN‑AMBR.
...
Provision to the PDN GW (in addition to the parameters in clause 7.4.2):
Authorized APN‑AMBR.
#### A.5.4.6.3 IP‑CAN Session Modification; GW(BBERF) initiated
The PCRF may provide the following parameters in the Acknowledgement of the
IP‑CAN Session Modification to the PDN GW (in addition to the parameters in
clause 7.4.3): Authorized APN‑AMBR.
...
be generated.
\- When the H-PCRF detects the limited PCC deployment, the H-PCRF shall inform
the AF that the event reporting is restricted due to limited PCC deployment.
