...
a=connection attribute in an SDP offer. For file transfer, the SDP shall also
include media level attributes in accordance with RFC 5547 [15], with the
exception that it shall include the file selector attribute (a=file-selector)
...
transfer, by sending a SDP offer where the m line port value for the file
...
intermediate node concludes that all available userson the distribution list
