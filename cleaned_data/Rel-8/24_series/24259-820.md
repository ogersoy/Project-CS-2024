...
> Based upon the IARI value PN controller UE invokes the PN controller
...
> target URI-parameter in the Request-URI to the user and provides the option
> to the user to:
>
...
> PN controllee UE sends a SIP 302 Moved Temporarily response to
...
> P-CSCF#2a forward the 302 Moved Temporarily response to S-CSCF#2.
...
> S-CSCF#2 forwards the 200 OK to PNM AS.
...
> Upon receiving the 302 Moved Temporarily response from the controller UE,
> the PNM AS determines the session is to be redirected to the controllee UE.
**18\. INVITE#3 (PNM AS to S-CSCF#2)**
> PNM AS forwards the Originating INVITE message to S-CSCF#2.
...
procedure in CS domain. PN access control services deal with a user having
access control over its personal network.
Figure A.3.5.2-1 Signalling flow for PN access control in the CS domain
**1\. ISUP IAM**
Call Request containing an IAM message arrived at the GMSC.
...
establishment and send a MAP-UNSTRUCTURED-SS-REQUEST message to the HSS. The
...
> String))
**11-13.** When the timer expires, the gsmSCF generates play announcement
...
String)) which inform that call is accepted.
**15-16. MAP-UNSTRUCTURED-SS-REQUEST-Ack (MSC/VLR to HSS)**
After receiving the FACILITY message from the controller UE, the MSC/VLR send
an USSD message towards the gsmSCF via the HSS. It contains the following
information.
Facility (ReturnResult=UNSTRUCTURED-SS-REQUEST(USSD-Data-Coding-Scheme, USSD
String))
**17.** As user responds, the gsmSCF stops playing announcement by sending
DisconnectForwardConnection message to GMSC.
...
On receipt of call continue message, the GMSC queries again the HSS for the
routing information for the called party but this time with the parameter
Suppress T-CSI.
**21\. MAP SRI Ack**
...
> UE initiates the INVITE request with the request-URI of the UE-1 to the IM
...
> route the call to the default UE located in CS domain.
>
...
> In this example, a call request (ISUP IAM) makes an entry from the PLMN of
> the originating user (calling party) into the home PLMN of the terminating
> user (called party). The first entry point of the PLMN of the terminating
> user is the Gateway MSC (GMSC). The call request is a form of an ISUP IAM
...
> On receipt of incoming call request, GMSC queries the HSS for routing
> information.
**3\. Retrieval of PNM subscriber information**
> The HSS provides information including the T-CSI information element that
> contains information configured for the PNM subscriber, identifying the
...
> \- the type of call; and
>
> \- information from the T-CSI IE received by the GMSC in the SRI ACK from
> the HSS. This includes the CAMEL service key.
**6\. Reroute to IMS determination**
> The gsmSCF invokes PNM redirection service logic to determine whether the
> call needs to be rerouted to default UE located in IM CN subsystem for UE
> redirection.
...
> GMSC will generate the IAM#2 containing the MSISDN of default UE at which
> the call is to be redirected and signals to the MGCF.
>
> MSISDN of default UE: 12134567897
**9\. SIP INVITE (MGCF to S-CSCF)**
> The MGCF interworks the IAM into the appropriate SIP message and initiates
> the SIP INVITE request towards the S-CSCF via the intermediate IM CN
> subsystem entities.
>
...
defined in annex B. All line feeds within element contents are for display
purposes only.
...
XCAP requires application usage to fulfil a number of steps in the definition
of such application usage. The reminder of this annex specifies the required
...
by the PNM application usage falls into the vendor‑proprietary namespace of
...
application/pnm+xml
**Naming conventions:** By default, PNM XML document is stored under the
user\'s Home Directory (which is located under the \"users\" sub‑tree). In
...
with a user name, called XCAP User Identify (XUI), which is pre-configured at
the PN UE. The XUI for PNM can be the shared public user identity used of the
PN user.
#