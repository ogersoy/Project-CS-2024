...
reselection, handover and manual CSG selection according to the information
provided by the NAS.
...
\- Limited service (emergency calls and ETWS if supported on an acceptable
cell)
...
```{=html}
``` \- For a CSG cell, the CSG ID broadcast by the cell is present in the
allowed CSG list associated with the PLMN for which the above condition is
satisfied;
...
#### 5.2.3.4 CSG cells in Cell Selection
...
set of priorities shall apply are defined in [4]. UE specific priorities are
only valid in the RPLMN, and shall be cleared when the validity time of the UE
specific priorities expires, or when leaving the RPLMN. UE specific priorities
