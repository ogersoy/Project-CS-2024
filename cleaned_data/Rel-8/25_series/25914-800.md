...
used under the \"speech mode\" conditions that correspond to predefined
...
uncertainty also occurs from the positioning of the DUT against the phantom,
as the DUT cannot be attached exactly in the same way every time. This
uncertainty depends on how much the DUT\'s position against the SAM phantom
varies from the specified \"cheek\" position. It is noted that the uncertainty
of the phone positioning depends on the phone holder and the measurement
operator and is in fact difficult to distinguish from random uncertainty.
...
To estimate this uncertainty, it is suggested to perform at least five
evaluations of TRP/TRS for the cheek right position whereby the device shall
be dismounted and newly positioned with a fully charged battery before each
tests. This measurement set has to be carried out in mid channel of each
frequency band, for at least three phones with different type of mechanical
design. The values have to be normalized by the mean for each measurement set.
As a result the uncertainty contribution entered to uncertainty budget is the
difference between the maximum and minimum normalized value.
...
reverberation chamber, such as the one shown in Figure E.1.
...
certain speed, as obtained by continuous mode stirring in the chamber by
moving the plate stirrers with certain spped.
...
phone in [70]. The measurements in the reverberation chamber are in comparison
fast, and repeatable even in other reverberation chambers of equal or larger
size, provided the chambers have efficient stirring methods. Also, the
measurements in many such reverberation chambers can be done by using only one
single channel receiver, because we can repeat exactly the same environment in
the chamber as many times as we like, and therefore we do not need to
simultaneously measure the two channels and the single isolated antenna
reference. In a real environment we need in comparison three measurement
receivers to do diversity measurements, one for each branch and one for the
reference, or a fast switch to select the three signals one after the other.
These advantages are even bigger when measuring MIMO systems, because the MIMO
antenna has many ports at which the signals have to be measured
simultaneously, or otherwise under the same conditions. In the following, we
will only describe how the chamber can be used for measurements of TRP.
## E.1.1 Chamber size
The reverberation chamber shall have a volume large enough to support the
number of modes needed for the stated accuracy. If platform stirring is
implemented the reverberation chamber can have an inner volume as small as a
minimum of 1.25 m3, but must be larger than 8 m3 if there is no such stirring
facility [11]. To obtain an expanded uncertainty (2σ) in the result of less
than 1 dB, it is necessary in the smallest chamber to perform frequency
stirring of 25 MHz [12]. This will reduce the resolution of the results
correspondingly. For antenna measurements, it is possible by appropriate
signal processing to improve frequency resolution to better than the 25 MHz
from the frequency stirring. If the chamber is larger than about 5 m3,
frequency stirring is not necessary, so that full frequency resolution can be
obtained.
...
necessary if the chamber is sufficiently large as mentioned above.
{width="4.770138888888889in" height="2.5840277777777776in"}
Figure E.1: Illustration of reverberation chamber, showing a setup for
measuring a six-element monopole circular MIMO array.
The chamber is equipped with two mechanical plate-shaped stirrers. The six-
element monopole array and the reference dipole are located on a platform and
rotated inside the chamber (platform stirring) [11]. Three fixed wall antennas
are used for polarization stirring [13]. The drawing also shows a head phantom
inside the chamber.
## E.1.3 Loading of chamber with lossy objects
The reverberation chamber should not be loaded to such an extent that the mode
statistics in the chamber is destroyed. It is important to keep the same
...
Make the setup in the chamber by locating the head phantom and the UE/MS in
one of the specified positions inside the chamber. The objects placed inside
the chamber should be the same as in the calibration measurement.
Measure and save pulse power levels for all the positions of the platform
stirrer and mechanical stirrer and for all the frequency points used. Do this
for each of the fixed measurement antennas. Average the saved pulse power
levels over all stirrer positions. Note that all averaging must be done on
power values. The result of the averaging of the power samples originating
from antenna A is P~A~. Repeat this for fixed measurement antennas B and C to
get P~B~ and P~C~. The TRP in W of the phone is then obtained by
$P_{\text{TRP}} = \frac{\left( \frac{P_{A}}{C_{A}\left( 1 - R_{A} \right)} +
\frac{P_{B}}{C_{B}\left( 1 - R_{B} \right)} + \frac{P_{C}}{C_{C}\left( 1 -
R_{C} \right)} \right)}{\left( P_{\text{ref},A} + P_{\text{ref},B} +
P_{\text{ref},C} \right)}$ (E.1)
where C~x~, R~x~ and P~ref,x~ for each antenna A, B and C is defined in
section E.3 (Calibration of reverberation chamber).
# E.3 Calibration of reverberation chamber
This section treats the necessary calibration of the reverberation chamber in
order to get absolute values for the measured parameters.
...
Make the setup in the chamber by locating the head phantom and the calibration
dipole inside the chamber. Use a phantom that is filled with the correct
liquid for the band to be measured, see Section 5 and Appendix B. Exactly the
same material must be present inside the chamber when the phones are measured
as when the calibration measurement is performed.
Position the calibration dipole inside the chamber in such a way that all its
parts are not closer to any wall, ceiling or floor than 0.5 times the
measurement wavelength and at least 0.7 wavelengths away from the phantom. The
phone shall be located outside the chamber, but the phone holder must be
located inside the chamber.
Connect the network analyzer and perform a full two-port calibration so that
the transmission between the calibration dipole and fixed measurement antenna
A can be measured. Measure the S-parameters for all the chosen positions of
the mechanical stirrer and the platform stirrer (if used), and over the whole
frequency band of interest plus an appropriate amount on each side if
frequency stirring will be used in the data processing. Spacing between the
frequency points shall be 1 MHz.
The measured reflection coefficient S~11~ has two additive contributions: One
due to the reflection from the antenna port itself (deterministic) and the
other due to the chamber (statistic). The same applies to S~22~. Therefore,
calculate the averages of the complex values of S~11~ and S~22~, and average
them further over a 5 MHz bandwidth. The remaining parts are the free space
reflection coefficients of the fixed receiving antenna A and the calibration
dipole, respectively. Denote by R~A~ the reflection coefficient of the fixed
measurement antenna A, so that
$R_{A} = \left| \frac{1}{N}\sum_{i = 1}^{N}S_{\text{11},A}^{i} \right|^{2} =
\left| \overline{S_{\text{11},A}} \right|^{2}$ (E.2)
where N denotes the total number of power samples achieved for the different
positions of the mechanical stirrers and the platform stirrer, and the
summation is over these samples.
Take the measured S~21~-values for all the stirrer positions N and calculate
the reference power transfer function of the chamber when using antenna A
$P_{\text{ref},A} = \frac{1}{N}\sum_{i = 1}^{N}\frac{\left|
S_{\text{21},A}^{i} \right|^{2}}{\left( 1 - R_{A} \right)\left( 1 - \left|
\overline{S_{\text{22}}} \right|^{2} \right)} \cdot \frac{1}{e_{\text{ref}}}$
(E.3)
where $\overline{S_{\text{22}}}$ is the complex average of the S~22~-samples
and e~ref~ is the efficiency (internal losses) of the reference dipole
antenna. For frequency stirring, include the power samples within a frequency
band centered around the measurement frequency in the summation above. A
frequency stirring bandwidth of 25 MHz is recommended for calculation of the
reference power transfer function.
Repeat the above for the other fixed antennas B and C to obtain R~B~, P~ref,B~
and R~C~, P~ref,C~, respectively.
Measure with the network analyzer also the path losses in the cables
connecting the power sampling equipment (used in the test case measurement) to
each of the three fixed measurement antennas A, B and C. Denote the obtained
transmission parameters C~A~, C~B~ and C~C~, respectively. Note that these
coefficients are always smaller than 1.
The measurement uncertainty is discussed in detail in [12].
