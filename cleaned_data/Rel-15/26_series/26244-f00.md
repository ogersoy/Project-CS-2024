...
Video Coding (\'avc1\') where each value in addition to the four-character
code includes a second element (referred to as \'avcoti\' in the formal
syntax), which is the hexadecimal representation of the following three bytes
in the (subset) sequence parameter set Network Abstraction Layer (NAL) unit
specified in [29]: (1) profile_idc,(2) the byte containing the constraint_set
flags (currently constraint_set0_flag through constraint_set5_flag, and the
...
\"CHAR\", \"CTL\", \"SP\", \"WSP\" and \"DQUOTE\" are defined in RFC 5234
[31].
