...
2) Up to 2 active temporal reference pictures are used for inter prediction of
each coding picture.
3) Cross-layer picture type aligned where the base layer and enhancement layer
share the same RAP distance (approximately 2 seconds).
...
\- Case A: the current active speaker sends two video resolutions (one
\"high\" @ 1080p + one \"medium\" \@720p), coded using either SHVC or HEVC
simulcast. The previous active speaker sends one video resolution (\"medium\"
\@720p) and one thumbnail (\@240p) video, coded using HEVC simulcast. All
other UEs send only one thumbnail video, coded using HEVC.
