...
NOTE 1: 3GPP hypermedia format is derived from Hypertext Application Language
...
7807 [19] that provides additional details of the error.
...
specification file, the TS version number included in the \"url\" field of the
\"externalDocs\" field in the OpenAPI specification file is not updated.
NOTE 2: If a new TS version is provided with changes to the OpenAPI
specification file, the TS version number included in the \"url\" field of the
\"externalDocs\" object is updated. However, if the API version in a given
release is still the same as in a previous release, the first TS version in
the first release that contains that API version is provided as TS version
within the \"externalDocs\" object also in the TS versions of the subsequent
releases.
...
### 5.3.5 Servers
As defined in clause 4.4, the base URI of an API consists of
**{apiRoot}/{apiName}/{apiVersion}**. It shall be encoded in the corresponding
OpenAPI specification file as \"servers\" field with **{apiRoot}** as
variable.
Example:
servers:
...
The referenced file names for other 3GPP-defined Open API specification
filesshall comply with the following convention, unless a specific file name
is indicated in the Annex of a 3GPP specification defining an OpenAPI
...
ii) the name of the security schema for oAuth2, as defined in the subsequent
bullet, and in the subsequent array the name of the API as only scope; and
...
contains:
1) a \"tokenUri\" field pointing to the Access Token Request service provided
by the NRF (see 3GPP TS 29.510 [18]); and
2) a \"scopes\" field defining the name of the corresponding API (using the
format used within URIs of that API) as only scope since the same security
applies to the entire API.
Example:
security:
\- {}
...
upper bound of required ressources. It also allows validation of incoming
...
suffix string \"/{custOpName}\" from the custom operation URI template in
clause 4.4.2.
