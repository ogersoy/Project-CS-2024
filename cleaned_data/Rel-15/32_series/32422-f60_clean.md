...
same term, if any, in TR 21.905 [4] , 23.501 [40] or 38.300 [42].
...
In NG-RAN the Cell Traffic trace functionality can be deactivated when the NG-
RAN node receives the Trace Session Deactivation message from the management
system. At this time the NG-RAN node shall deactivate the Trace Session for
those NG-RAN Cells that have been indicated in the Trace Session Deactivation
message received from the management system.
...
The format of the file sent to the TCE from the AMF is defined in 3GPP TS
32.423, clause A.2.2.
The figure 4.2.2.10.1 illustrates the procedure.
{width="6.166666666666667in" height="6.583333333333333in"}
Figure 4.2.2.10.1: NG-RAN trace starting mechanism
...
+----------------------+----------------------+----------------------+ | SMF | Start triggering | Stop triggering | | | events | events | +----------------------+----------------------+----------------------+ | UE initiated PDU | NAS signalling from | Sending of | | Session | the UE, N16 | Nsmf_PDUSess | | Establishment | signalling from VSMF | ion_CreateSMContext | | procedure | | response | | | Receipt of | | | | Nsmf_PDUSess | | | | ion_CreateSMContext | | | | request from AMF | | +----------------------+----------------------+----------------------+ | UE initiated PDU | NAS signalling from | Sending of | | Session Modification | the UE, N16 | Nsmf_PDUSess | | procedure | signalling from VSMF | ion_UpdateSMContext | | | | response | | | Receipt of | | | | Nsmf_PDUSess | | | | ion_UpdateSMContext | | | | request from AMF | | +----------------------+----------------------+----------------------+ | UE initiated PDU | NAS signalling from | Sending of | | Session Release | the UE, N16 | Nsmf_PDUSessi | | procedure | signalling from VSMF | on_ReleaseSMContext | | | | response | | | Receipt of | | | | Nsmf_PDUSessi | | | | on_ReleaseSMContext | | | | request from AMF | | +----------------------+----------------------+----------------------+ | UE initiated PDU | N11 request from AMF | Sending of N11 | | Session UP | (may come from a NAS | response to AMF | | activation / | SR or Registration) | | | deactivation | | | +----------------------+----------------------+----------------------+ | Mobility of a PDU | NAS signalling from | Sending of NAS | | Session between 3GPP | the UE (Request Type | signalling to UE | | and N3GPP access to | = \"Existing PDU | | | 5GC | Session\") | | +----------------------+----------------------+----------------------+ | Mobility of a PDU | N11 request from AMF | Sending of | | Session from EPC | (comes from a N26 | Nsmf_PDUSess | | | forward relocation | ion_UpdateSMContext | | | request) | response | +----------------------+----------------------+----------------------+
* * *
...
\- Service identification (M);
\- Trace reference (M);
\- Service level tracing counter (M);\ The service level tracing counter is
incremented on a hop-by-hop basis to indicate the sequence of trace records
...
MGW MSC-S gNB-CU-UP gNB-CU-CP UPF PCF SMF AMF eNB PDN GW spare spare spare
spare SMSF AUSF ng-eNB gNB-DU
* * *
...
* * *
SMF  
Bit 8 Bit 7 Bit 6 Bit 5 Bit 4 Bit 3 Bit 2 Bit 1 spare spare spare S5-C N11 N10
N7 N4
* * *
* * *
UPF  
...
\- M1: CPICH RSCP and CPICH Ec/No measurement by UE with Periodic or event 1F
as reporting triggers.
\- M2: For 1.28 Mcps TDD, P-CCPCH RSCP and Timeslot ISCP measurement by UE
with event 1I as reporting triggers.
...
\- M6: Packet Delay measurement, separately for DL and UL, per QCI per UE, UL
PDCP Delay, by the UE, and Packet Delay in the DL per QCI, by the eNB.
...
* * *
LTE  
Bit 8 Bit 7 Bit 6 Bit 5 Bit 4 Bit 3 Bit 2 Bit 1 M7 M6 logging of M1 from event
triggered measurement reports according to existing RRM configuration M5 M4 M3
M2 M1 spare  
UMTS  
...
logging they would generate in a highly loaded network. The selection of a
specific subset of supported values at the eNB is vendor-specific.
### 5.10.28 Collection period M7 in LTE
This parameter is mandatory if the job type is set to Immediate MDT or
...
specification.
Alternative 1 illustrated in Figure 7.2-1 may be used when supported by the
...
Figure 7.2-2 may be used when supported by the implementation and both
