...
affect some of the steps between Step 1 and Step 12 in Subclause 7.5.
...
at delay at time _t_ is given by:
(7.6-3)
...
unequal ray powers are considered,_ _are generated according to Subclause
...
vector respectively with $\left\| {\overline{v}}_{\text{tx}}\left( t_{k}
\right) \right\|_{2} = v_{\text{tx}}$ and $\left\|
{\overline{v}}_{\text{rx}}\left( t_{k} \right) \right\|_{2} =
v_{\text{rx}}$_;_
is the 3D distance between _Rx_ antenna and _Tx_ antenna at in [m];
...
generated in Step 4 of Subclause 7.5._ Normalize the cluster powers so that
...
coefficient generation described in Subclause 7.5 are performed twice, once
...
coefficients are denoted as and respectively, where is generated with the LOS
path loss formula and channel model parameters while is generated using the
NLOS path loss formula and channel model parameters. The channel matrix with
soft LOS state is determined from a linear combination of and as:
(7.6-19)
...
of [14] with the square root matrixbeing generated using the Cholesky
