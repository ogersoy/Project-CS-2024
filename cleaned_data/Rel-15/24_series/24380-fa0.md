...
of Timer T1 is running per talker that is granted the floor.
...
Timer T20 is running per talker that is granted the floor.
...
to support multi-talker floor control, one instance of Timer T20 is running
per talker that is granted the floor.
...
floor is granted;
5\. shall enter the \'G: Floor Taken\' state; and
...
distributing RTP media packets received from the participant for which T1 has
expired (with the exception of RTP media packets already in the buffer (if RTP
media buffering is ongoing)) to other MCPTT clients, 4. if mutli-talker floor
control is not supported; shall enter the \'G: Floor Idle\' state as specified
in the subclause 6.3.4.3.2; and
...
d. shall start the T1 (End of RTP) timer for the particpant to which the floor
is granted;
