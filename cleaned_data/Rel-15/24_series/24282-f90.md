...
specified in clause 8.2.2.2.15
...
affiliation information constructed according to clause 9.2.3.2. The MCData
...
request type IE shall follow the procedures in clause 9.2.1.3.
...
> a Warning header field as specified in clause 4.9, and shall not continue
...
\ element of the MIME body, as specified in clause 9.2.2.4.1;
...
j) if the procedures in clause 6.3.4 result in no affiliated members found in
...
a SIP 403 (Forbidden) response with the warning text set to \"206 short data
service not allowed for this group\" in a Warning header field as specified in
clause 4.x and shall not continue with the rest of the steps;
e) if the \ element is not present in the group document
or is present and contains a \ element containing an \"enabler\"
attribute which is not set to the value \"urn:urn-7:3gpp-
service.ims.icsi.mcdata.sds\", shall send a SIP 488 (Not Acceptable) response
...
in the incoming SIP INVITE request as specified in the clause 9.2.3.4.2;
9) shall interact with the media plane as specified in 3GPP TS 24.582 [15]
clause 6.3.1; and
...
11.1;
7) if the procedures in clause 11.1 indicate that the user identified by the
MCData ID is not allowed to send MCData communications as determined by step
1) of clause 11.1, shall reject the \"SIP INVITE request for SDS session for
originating participating MCData function\" with a SIP 403 (Forbidden)
...
SIP INVITE request;
8) shall include in the SIP 200 (OK) response an SDP answer to the SDP offer
in the incoming SIP INVITE request as specified in the clause 9.2.4.4.2;
9) shall interact with the media plane as specified in 3GPP TS 24.582 [15]
clause 6.3.2; and
10) shall send a SIP 200 (OK) response to the inviting MCData client according
to 3GPP TS 24.229 [5].
...
#### 9.3.1.2 Message transport to an MCData Group
In order to transmit an off-network SDS message or SDS notification to an
...
node present in group configuration as specified in 3GPP TS 24.483 [42] and on
...
procedures in clause 10.2.4; and
...
IE, shall follow the procedures in clause 10.2.1.2.3.
...
NOTIFICATION by following the procedures in clause 12.2.1.1.
...
following the procedures in clause 10.2.1.3.
...
6) if the FD SIGNALLING PAYLOAD message or FD HTTP TERMINATION messagedoes not
contain only one Payload IE, shall reject the SIP MESSAGE request with a SIP
403 (Forbidden) response, with warning text set to \"210 Only one File URL
...
by file URL does not exist\" in a Warning header field as specified in clause
4.9, and shall not continue with the rest of the steps in this clause;
...
field as specified in clause 4.9, and skip the rest of the steps below; and
b) if the application/vnd.3gpp.mcdata-signalling MIME body contains an FD
...
following the procedures in clause 6.3.4;
k) if the procedures in clause 6.3.4 result in no affiliated members found in
the selected MCData group, shall return a SIP 403 (Forbidden) response with
the warning text set to \"198 no users are affiliated to this group\" in a
Warning header field as specified in clause 4.9, and skip the rest of the
steps below; and
...
availability timer) to the Conversation ID, Message ID and Application ID (if
included) contained in the FD SIGNALLING PAYLOAD message;
...
and \"FILE URL\". If any ongoing transmission exist then execute procedure
described in clause 12.4.2.1 withfollowing clarifications:
i) shall set FD notification type IE as \"FILE DELETED UNAVAILABLE TO
...
MCData ID of the originating user mentioned in the \ element of the application/vnd.3gpp.mcdata-info+xml MIME body of the
...
A) fill \ element from \
element of the application/vnd.3gpp.mcdata-info+xml in received SIP MESSAGE;
...
in the incoming SIP INVITE request according to 3GPP TS 24.229 [5] with the
clarifications given in clause 10.2.5.2.2; and
...
403 (Forbidden) response with the warning text set to \"213 file distribution
not allowed for this group\" in a Warning header field as specified in clause
4.x and shall not continue with the rest of the steps;
e) if the \ element is not present in the group document
or is present and contains a \ element containing an \"enabler\"
attribute which is not set to the value \"urn:urn-7:3gpp-
service.ims.icsi.mcdata.fd\", shall send a SIP 488 (Not Acceptable) response
...
clause 7.3.
Upon receiving a SIP 200 (OK) response for a SIP INVITE request as specified
in clause 10.2.5.4.3 and if the MCData ID in the SIP 200 (OK) response matches
to the MCData ID in the corresponding SIP INVITE request the controlling
MCData function:
1) shall generate SIP 200 (OK) response to the SIP INVITE request according to
3GPP TS 24.229 [5];
...
SIP INVITE request;
8) shall include in the SIP 200 (OK) response an SDP answer to the SDP offer
in the incoming SIP INVITE request as specified in the clause 10.2.5.4.2;
9) shall interact with the media plane as specified in 3GPP TS 24.582 [15]
clause 7.3; and
10) shall send a SIP 200 (OK) response to the inviting MCData client according
to 3GPP TS 24.229 [5].
...
procedures are applicable for FD using HTTP.
...
2) shall send SIP 200 (OK) response towards MCData server according to 3GPP TS
24.229 [5]; and
...
15.1.12, the MCData client:
1) shall set the number of payloads IE to the number of FD using HTTP
communication which are deferred as per the stored file list:
...
notification\" in a Warning header field as specified in clause 4.4, and shall
not continue with the rest of the steps;
6) if:
...
the MCData group\" in a Warning header field as specified in clause 4.4, and
shall not continue with the rest of the steps;
...
1) shall generate a SIP MESSAGE request as specified in clause 12.x.2.1;
...
specified in clause 15.2.7; and
...
a SIP MESSAGE towards participating MCData function:
...
in clause 15.2.10;
3) may set the Application ID IE ID to the stored value if applicable; and
4) shall include a Payload IE with:
a) Shall set the Payload content type set to \"FILEURL\" as specified in
clause 15.2.13; and
b) Shall set the URL of the file same as of FD transmission.
...
according to 3GPP TS 24.229 [5];
3) if FD HTTP TERMINATION message contains an Application ID, shall deliver
the FD HTTP TERMINATION message to the application; and
...
1) shall decode contents of application/vnd.3gpp.mcdata-signalling MIME body;
2) if application/vnd.3gpp.mcdata-signalling MIME body contains a FD HTTP
TERMINATION message as specified in clause 15.1.11 and Termination Information
Type IE set to \"TRANSMISSION STOPPED\", then:
a) shall generate SIP 200 OK response according to 3GPP TS 24.229 [5];
b) shall send SIP 200 (OK) response towards MCData server according to 3GPP TS
24.229 [5]; and
3) shall notify MCData user about file transmission stopped by identifying
corresponding file transmission local database based on conversation id,
message id and FILE URL received in FD HTTP TERMINATION message along with
reason.
###### 13.2.3.3.2.2 MCData client terminating procedure
On receipt of a SIP MESSAGE request containing an application/vnd.3gpp.mcdata-
signalling MIME body with a FD NETWORK NOTIFICATION message, the MCData client
shall follow the procedures as described in clause 12.4.4
##### 13.2.3.3.3 Particiapting MCData function procedures
Upon receipt of a \"SIP MESSAGE request for FD using HTTP for terminating
...
MCDATA CARRIER message defined in 3GPP TS 24.379 [10].
...
0 0 DEFERRED LIST ACCESS RESPONSE x x 0 0 1 1 0 1 FD HTTP TERMINATION
...
0 1 0 0 FILEURL 0 0 0 0 0 1 0 1 LOCATION 0 0 0 0 0 1 1 0 ENHANCED STATUS
All other values are reserved.
* * *
...
15.2.24-1, table 15.2.24‑1, table 15.2.24‑2 and table 15.2.24-3.
