...
PDN connections per UE caused by the number of EPS bearer identities (see 3GPP
TS 24.007 [48]). Not complying with this limitation when accessing non-3GPP
access can lead to unexpected consequences, e.g. connectivity loss in case of
...
5996 [28]. The UE indicates a request for the default APN by omitting the
...
protocol as defined in IETF RFC 5996 [28]. If the previously allocated home
...
NOTE: In case of IPv6 address, the UE performs the match only on the IPv6
...
IKEv2 security association that was established.
...
attempt to establishment additional PDN connections if one or more existing
PDN connections are released.
...
shall send an INFORMATIONAL request with no payloads as per IETF RFC 5996
...
IKE SA as specified in IETF RFC 5996 [28].
...
RFC 5996 [28]) if it is configured by the RekeyingTime node as specified in
...
The UE may support NAT keep alive handling as defined in IETF RFC 5996 [28]
...
5996 [28]) to disconnect one or more IPsec tunnels to the ePDG. The UE shall
...
b) consider that the ePDG is no longer responding (see RFC 5996 [28]) and not
...
addresses as specified in IETF RFC 5996 [28];
...
to ID_FQDN as defined in IETF RFC 5996 [28]. Handling of \"IDr\" payload in
...
with no payloads IETF RFC 5996 [28]. If an INFORMATIONAL response is not
...
were negotiated using that IKE SA as specified in IETF RFC 5996 [28] and shall
...
If the UE included the N1_MODE_SUPPORT Notify payload (see subclause 8.1.2.3)
within the IKE_AUTH request message, the ePDG may include the
N1_MODE_INFORMATION Notify payload within the IKE_AUTH response message. The
ePDG shall set the S-NSSAI Value field of the N1_MODE INFORMATION Notify
payload to the S-NSSAI for the PDU session associated with the IKEv2 security
association if the S-NSSAI information for the PDU session indicated in the
PDU session ID field of the N1_MODE_SUPPORT Notify payload is available.
...
5996 [28]) to disconnect one or more IPsec tunnels to the UE. The ePDG shall
close the incoming security associations associated with the tunnel and
...
a) close the related IKEv2 security association for the IPsec tunnel
associated with this PDN connection; and
b) consider that the UE is no longer responding (see RFC 5996 [28]) and not
...
ID_FQDN ID Type, as described in IETF RFC 5996 [28].
...
* * *
Octet 1 is defined in IETF RFC 5996 [28] Octet 2 is SPI Size field. It is set
...
* * *
Octet 1 is defined in IETF RFC 5996 [28]. Octet 2 is the SPI Size field. It is
...
* * *
Octet 1 is defined in IETF RFC 5996 [28] and is set to 3 to indicate ESP.
...
and as specified in IETF RFC 4306 [70A] and updated by IETF RFC 5996 [28] and
IETF RFC 7296 [70B].
Which registry are you requesting this assignment/registration be made in?
...
Attribute Types\". IETF RFC 7296 [70B] and IETF RFC 5996 [28] refer to IETF
...
field used in the Emergency Call Number ANQP-element specified in IEEE
802.11-2012 [57].
NOTE: According to IEEE 802.11-2012 [57] an ANQP server is not necessarily
